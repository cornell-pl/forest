...
\cd{\{c \kw{is} "c.txt" :: C, d \kw{is} "d.txt" :: D c\}} is encoded in the calculus as
$\SPair{x}{(\SPath{\texttt{"}\mathtt{c.txt}\texttt{"}}{C})}{(\SPath{\texttt{"}\mathtt{d.txt}\texttt{"}}{D\; x})}$.  Similarly, \forest{}'s comprehension 
...
\SSetComp{s}{x}{e} & \repty{s}~\typ{list} & \md{\mdty{s}~\typ{list}}    \\
...
% \mdty{\SSetComp{s}{x}{e}} & = & \md{\mdty{s}~\typ{list}}  \\
