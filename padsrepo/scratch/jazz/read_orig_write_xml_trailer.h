...
int main(int argc, char** argv) {
   ...
   */
  while (!P_io_at_eof(pads) && P_OK!=Pcstr_lit_match(pads, "<h4",0)) {
  ...
  if (!P_io_at_eof(pads)) {
    if (P_OK != PADS_TRL_TY(_read)(pads, &trl_m, &trl_pd, &trl_rep EXTRA_TRL_READ_ARGS)) {
      error(ERROR_FATAL, "Note: trailer read returned error");
    } else {
      error(2, "Note: trailer read returned OK");
    }
    if (P_ERR == PADS_TRL_TY(_write_xml_2io)(pads, io, &trl_pd, &trl_rep, (const char*)0, 0 EXTRA_TRL_READ_ARGS) ) {
