...
if ($_status == "OK") then
  set ast_arch = `$PADS_HOME/scripts/package`
  if (! $?INSTALLROOT) then
    ...
    echo "# and then use NMAKE_SETENV.tcsh again."
    echo "##############################################################################"
    echo " "
  endif
  ...
  setenv PATH            `echo ${pads_bin_dir}:${pads_script_dir}:${ast_bin_dir}:${PATH} | $remove_dups`
