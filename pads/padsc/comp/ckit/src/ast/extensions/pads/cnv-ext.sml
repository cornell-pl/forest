...
(*  Typedef case *)
	      ...
	      fun cnvPStruct ({name: string, isRecord, containsRecord, largeHeuristic, isFile, params: (pcty * pcdecr) list, 
			...
			| mungeFields f b m (x::xs) = (mungeField f b m x) @ (mungeFields f b m xs)
			 ...
			 case postCon of NONE => ([],[])  (* get begin loc for struct, check constraint*)
		      ...
		      val _ = pushLocalEnv()                                        (* create new scope *)
		      val () = ignore (insTempVar(gMod rep, P.ptrPCT canonicalPCT)) (* add modrep to scope *)
		      val () = ignore (insTempVar(gMod m,  P.ptrPCT mPCT))        (* add modm to scope *)
		 ...
		 in
             ...
             fun cnvPArray {name:string, params : (pcty * pcdecr) list, isRecord, containsRecord, largeHeuristic, isFile : bool,
