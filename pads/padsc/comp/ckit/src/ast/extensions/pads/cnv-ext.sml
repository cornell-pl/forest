...
(* Conversions ***************************************************************)
	  ...
	  let (* Some useful names *)
	      ...
	      fun modFieldX (bsName, fName) = P.arrowX(PT.Id(gMod bsName), PT.Id fName)
		...
		| TyProps.Dynamic => (SOME (initSuf name),
	      ...
	      fun lookupLitWrite s = (iSuf o bufSuf o writeSuf) s
	      ...
	      fun genReadFun (readName, cParams:(string * pcty)list, 
			      csmPCT,edPCT,canonicalPCT, csmFirstPCT, hasNErr, bodySs) = 
		  let val iReadName = iSuf readName			  
		      val (cNames, cTys) = ListPair.unzip cParams
                      val paramTys = [P.ptrPCT PL.toolStatePCT, P.ptrPCT csmPCT]
			             @ cTys
			             @ [P.ptrPCT edPCT, P.ptrPCT canonicalPCT]
                      val paramNames = [pdc, csm] @ cNames @ [ed,rep]
		      val iParamNames = [pdc, gMod csm] @ cNames @ [gMod ed, gMod rep]
		      ...
		      val decls =   genLocTemp(canonicalPCT, rep, NONE) 
			          @ genLocTemp(csmPCT, csm, csmFirstPCT) 
			          @ genLocTemp(edPCT, ed, NONE)
		      val wrapperinitDecls =  [genLocInit rep, genLocInit csm, genLocInit ed]
...
*)
		  ...
		  let val writeIOName = ioSuf writeName
		      ...
		      val returnTy =  PL.ssizePCT
                      ...
                      (* -- write2buf_internal *)
		      val iBufDeclSs = [P.varDeclS(P.ptrPCT PL.bytePCT, bufCursor, PT.Id buf),
				        P.varDeclS(PL.ssizePCT, length, P.zero),
				        P.varDeclS'(PL.ssizePCT, tlen)]
		      val iBufIntroSs = [P.assignS(P.starX (PT.Id bufFull), P.zero)]
		      val (iBufRecordIntroSs,iBufRecordCloseSs)  = 
                           if isRecord then
			       (([P.assignS(PT.Id tlen, 
		      ...
		      val iBufCloseSs = [PT.Return (PT.Id length)]
		      val iBufBodySs  = iBufDeclSs @ iBufIntroSs @ iBufRecordIntroSs @ iBodySs 
			                @ iBufRecordCloseSs @iBufCloseSs
		      val iWriteBufFunED = 
			  P.mkFunctionEDecl(iWriteBufName, BufFormalParams, PT.Compound iBufBodySs, returnTy)
		      ...
		      val introSs = [P.varDeclS'(P.ptrPCT PL.bytePCT, buf),
				     ...
				     P.varDeclS(PL.sizePCT, bufLen, P.arrowX(PT.Id pdc, PT.Id PL.outBufRes))]
			...
			val chkResS = PT.IfThen(PT.Id bufFull,
		      ...
		      val bodySs = introSs @ [ whileLoopS, chkResS, abortS, returnS]
		      val bodyS = PT.Compound bodySs
		      ...
		      val bodySs = introSs @ checkParamsSs @ [ checkEdS, returnS]
		      val bodyS = PT.Compound bodySs
		      val writeBufFunED = P.mkFunctionEDecl(writeBufName, BufFormalParams, bodyS, returnTy)
		  in
		      [iWriteBufFunED, iWriteIOFunED, writeIOFunED, writeBufFunED]
		  end
		  ...
		  [PL.sfprintf(PT.Id tmpstr, PT.String ("%s."^fieldDescriptor),[PT.Id prefix]@extraArgsXs),
		   chkPrint bodyX]
              ...
              fun genPrintPiece(reportName, fieldDescriptor, whatX, modFieldX, extraArgsXs) = 
		...
		| SOME a => (
		  ...
		  let val base = "base"
		      ...
		      val baseCSMPCT = P.makeTypedefPCT(lookupTy(baseTy,csmSuf, #csmname))
                      val csmFields  = [(base, baseCSMPCT, SOME "Base CheckSet mask"),
				       (user, PL.base_csmPCT, SOME "User constraint")]
		      val csmED      = P.makeTyDefStructEDecl (csmFields, csmSuf name)
			  ...
			  let val resDeclSs = [P.varDeclS'(PL.toolErrPCT, result)]
				  ...
				  [PT.IfThen(P.andX(P.lteX(modFieldX(csm,user), PL.CSM_CHECK),
						    P.notX modPredX),
		      ...
		      val readFunEDs = genReadFun(readName, cParams, csmPCT,edPCT,canonicalPCT, 
						  NONE, true, bodySs)
	      ...
	      fun cnvPStruct ({name:string, isRecord, isFile, params: (pcty * pcdecr) list, 
			...
			| mungeFields f b m (x::xs) = (mungeField f b m x) @ (mungeFields f b m xs)
			  ...
			  [(name,P.makeTypedefPCT(lookupTy (pty,csmSuf,#csmname)), NONE)]
		      fun genCSMBrief e = []
		      fun genCSMMan m = []
		      val csmFieldsNested = mungeFields genCSMFull genCSMBrief genCSMMan fields
		      val auxCSMFields = [(all, PL.base_csmPCT, NONE)]
		      val csmFields = auxCSMFields @ csmFieldsNested

		      val csmFirstPCT = getFirstEMPCT csmFields
		      val csmStructED = P.makeTyDefStructEDecl (csmFields, csmSuf name)
		      val csmDecls = cnvExternalDecl csmStructED 
                      val csmPCT = P.makeTypedefPCT (csmSuf name)			  
			  ...
			  let val readFieldName = lookupTy(pty, iSuf o readSuf, #readname)
			      ...
			      val (locDecls, locX) = if isVirtual then ([P.varDeclS'(PL.locPCT,tloc)],PT.Id tloc) 
					   ...
					   then addSub(name, modFieldX(rep,name))  (* record additional binding *)
				       else ()
				   ...
				   PL.getLocBeginS(PT.Id pdc, P.addrX locX),
				     ...
				     PT.Compound(* else no error reading field *)
					       ...
					       fun swap reportErrSs = 
                                                   [PL.swapBytesS(modFieldX(rep, name)),
						    PT.IfThenElse(
						       exp,
                                                       PT.Compound
							[P.assignS(d_endianX,
					   ...
					   in
					       [PT.IfThen(
                                                 P.andX(P.lteX(getEMExp(modFieldX(csm,name)), PL.CSM_CHECK),
							P.notX exp),
						 PT.Compound
					           (if isEndian then
						     swap reportErrSs
					 ...
					 [PT.IfThen(
				      ...
				      PT.Compound(reportBriefErrorSs (PL.PDC_MISSING_LITERAL,
                              ...
                              val notPanicSsNoScan = 
                                  [(* base_csm tcsm = Check *)
				   P.varDeclS(PL.base_csmPCT, tcsm, PL.CSM_CHECK),
                                   PT.IfThen( (* PDC_ERROR == readFieldName(pdc, &tcsm, &ted, e) *)
			 ...
			 case postCon of NONE => ([],[])  (* get begin loc for struct, check constraint*)
				...
				val condSs = 
					...
					PT.IfThen(
                                           P.andX( P.lteX(modFieldX(csm,all), PL.CSM_CHECK), P.notX expr),
		      ...
		      val _ = pushLocalEnv()                                        (* create new scope *)
		      val () = ignore (insTempVar(gMod rep, P.ptrPCT canonicalPCT)) (* add modrep to scope *)
		      val () = ignore (insTempVar(gMod csm,  P.ptrPCT csmPCT))        (* add modcsm to scope *)
		      ...
		      val reportFields = (mungeFields genAccReportFull genAccReportBrief 
			                  (genAccReportMan (reportSuf, iSuf)) fields)
		 ...
		 let (* Some useful names *)
		     ...
		     fun genCSMBrief e = []
		     fun genCSMMan m = []
		     val csmFields = mungeVariants genCSMFull genCSMBrief genCSMMan variants
		     val csmFirstPCT = getFirstEMPCT csmFields
		     val csmStructED = P.makeTyDefStructEDecl (csmFields, csmSuf name)
		     val csmPCT = P.makeTypedefPCT (csmSuf name)			  
			 ...
			 case #memChar unionProps of TyProps.Static => []
		       | TyProps.Dynamic => 
				 [PT.IfThen(
					    P.neqX(modFieldX(rep,tag), PT.Id (!firstTag)),
			 ...
			 [P.assignS(PT.Id result, PL.PDC_OK),
			  PT.Goto (findEORSuf unionName)]
 		          ...
 		          case predOpt of NONE => foundSs
                          | SOME constraint => 
			    let val predX = (PTSub.substExp (name, P.dotX(modFieldX(rep,value),PT.Id name), constraint))
				 ...
				 in
				     PT.Compound[
                                      PT.IfThenElse(
                                         P.andX(P.lteX(modFieldX(csm,name), PL.CSM_CHECK),
						P.notX predX),
					   notFoundSs,
					   foundSs
					  )]
			 ...
			 in
				 ...
				 PL.readFunChkX(
				     ...
				     P.addrX(modFieldX(csm,name)), args, 
			 ...
			 let val ctNoptEs = cnvDeclaration decl
				 ...
				 let val name = case nameOpt of NONE => "bogus"
			       | SOME n => n  
				     val repX = P.dotX(modFieldX(rep, value), PT.Id name)
		 ...
		 val element = "element"
                 ...
                 val elemCsmPCT  = P.makeTypedefPCT(lookupTy(baseTy, csmSuf, #csmname))
		 ...
		 val tlocES     =  PL.getLocEndS(PT.Id pdc, tLocX, ~1) 
			  ...
			  PT.Compound[P.postIncS(modFieldX(ed,nerr))])]
                 ...
                 fun amCheckingE(SOME testE) = 
                     P.andX(P.lteX(modFieldX(csm,array), PL.CSM_CHECK), testE)
                   | amCheckingE(NONE) = P.lteX(modFieldX(csm,array), PL.CSM_CHECK)
		 ...
		 val csmFields = [(element, P.makeTypedefPCT(lookupTy(baseTy, csmSuf, #csmname)),
				 ...
				 (array,   PL.base_csmPCT, SOME "entire array checks")]
		 val csmStructED = P.makeTyDefStructEDecl (csmFields, csmSuf name)
		 val csmStructDecls = cnvExternalDecl csmStructED 
		 val csmPCT = P.makeTypedefPCT (csmSuf name)			  
		 ...
		 val resBufferX   = modFieldX(rep, elts)
		 val indexX       = P.minusX(modFieldX(rep,length), P.intX 1)
				 ...
				 in
				   (NONE, SOME (exp,readFun,scanFun,writeFun), NONE, NONE)
                              ...
                              |  PX.Forall (r as {index,range,body}) => (
                                 let val subList = [(length, modFieldX(rep,length)), 
						    (name, modFieldX(rep,elts)),
						    (elts, modFieldX(rep,elts))]
				     val (lower, upper) = 
					(case range 
					 of PX.ArrayName n => (
		 ...
		 val () = checkParamTys(name, elemReadName, args, 2, 2)
                 val termTmpDecls = case termXOpt of NONE => [] | SOME _ =>
		                    [P.varDeclS(PL.base_csmPCT, tcsm, PL.CSM_CHECK), (* base_csm tcsm = CHECK; *)
          		             P.varDeclS'(PL.base_edPCT, ted)]
		       ...
		       PT.IfThen(
                          PL.readFunChkX(PL.PDC_OK, readFun, PT.Id pdc, P.addrX(PT.Id tcsm),  [], 
		 ...
		 val (chkLenSs, bufSugX) = case maxOpt of NONE => ([], P.zero)
	             | SOME sizeX => 
		        ([P.assignS(PT.Id reachedLimit, P.gteX(modFieldX(rep,length), Option.valOf maxOpt))],
			 sizeX)
                 val readElementSs = 
                       [P.postIncS(modFieldX(rep,length)),
			locBS]
                     @ chkLenSs
		     ...
		     @ (PL.chkReserveSs(PT.Id pdc, readName, edRBufferX, 
			   ...
			   PT.IfThen(PL.getSpecLevelX(PT.Id pdc),
				     PT.Return PL.PDC_ERROR),
			   PT.IfThen(P.lteX(modFieldX(csm,array), PL.CSM_CHECK),
			     PT.Compound[
		 ...
		 fun genPanicRecoveryS (sepXOpt, termXOpt, maxOpt) = 
		     ...
		     let fun insLengthChk bdyS = 
			    case (maxOpt,maxConstOpt) 
                            of (SOME maxX, NONE) => (
				PT.IfThenElse(
                                 P.gteX(modFieldX(rep,length), maxX),
			 ...
			 fun insTermChk bdyS = 
			     ...
			     | SOME (termX, termRead, _,_) => (
						...
						P.addrX (PT.Id tcsm), [], P.addrX (PT.Id ted),
						termX),
				 PT.Compound[
				  P.assignS(PT.Id foundTerm, P.trueX)
                                 ],
		 ...
		 val readEORSs = if isRecord then genReadEOR (readName, reportStructErrorSs) () else []
		      ...
		      P.assignS(modFieldX(ed,length), modFieldX(rep,length))
                 ...
                 fun genMinReachedConstraintSs minX =  
                     let val lengthTestX = P.ltX(modFieldX(rep,length), minX)
		     ...
		     in
			  ...
			  recordArrayErrorS([tlocES],tLocX,PL.PDC_ARRAY_SIZE_ERR, true, readName,
			    ...
			    [modFieldX(rep,length), minX], false))]
		     end
		     ...
		     [P.mkCommentS "Checking user's array constraint.",
		       ...
		       PT.IfThen(P.notX(P.andX(P.lteX(P.zero, lower),
				 	P.ltX(upper, modFieldX(rep,length)))),
			...
			recordArrayErrorS([tlocES],tLocX,PL.PDC_ARRAY_USER_CONSTRAINT_ERR, true, readName,
			 ...
			 [P.mkCommentS "Checking constraints",
			  PT.IfThen(amCheckingE(SOME(P.notX (modFieldX(ed,panic)))),
				    PT.Compound bdySs)]
		     in
	  ...
	  fun cnvPEnum  {name:string, params : (pcty * pcdecr) list, isRecord, isFile,
		  ...
		  val readFunEDs = genReadFun(readName, cParams, 
					      csmPCT,edPCT,canonicalPCT, NONE, false, bodySs)
