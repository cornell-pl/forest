...
int main(int argc, char** argv) {
  ...
  */
  ...
  if (argc != 2) { error(2, "Usage: test_load_dns <dns-data-file>\n"); exit(-1); }
  ...
  /* init -- must do this! */
  P_INIT_ALL(pads, dns_msg_array, rep, m, pd, P_CheckAndSet);

  /* make the top-level node */
  PDCI_MK_TOP_NODE_NORET (doc_node, &dns_msg_array_vtable, pads, "doc", &m, &pd, &rep, "main");

  /* Try to read entire file */
  dns_msg_array_read(pads, &m, &pd, &rep);
  if (!P_PS_isPanic(&pd)) { 
    ...
    error(0, "dns_msg_array_read raised panic error");
  }

  P_CLEANUP_ALL(pads, dns_msg_array, rep, pd);
  P_io_close(pads);
  P_close(pads);
  return 0;
}
