...
int try_galax() { 
  ...
  err = galax_load_standard_library(pc, &sc); 

  err = galax_eval_statement_from_string(sc, "<a/>", &items);
  err = galax_serialize_to_string(items, &str);
  error(0, "%s\n", str); 
  return err;
}
...
int main(int argc, char** argv) {
  ...
  */
  ...
  if (P_OK == myfile_read(pads, &m, &pd, &rep)) {
    ...
    err = galax_serialize_to_string(docitems, &str);
    error(0, "%d: %s\n", strlen(str), str);  
    exit_on_error(galax_serialize_to_output(docitems));
