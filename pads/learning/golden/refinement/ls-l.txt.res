Source file to process: data/ls-l.txt
Output directory: gen/
Max depth to explore: 50
Print line numbers in output contexts: false
Print ids and output type tokens: true
Print Entropy: false
Histogram comparison tolerance (percentage): 0.01
Struct determination tolerance (percentage): 0.1
Noise level threshold (percentage): 0.0
Minimum width threshold for array: 2
Junk threshold (percentage): 0.1
Starting on file data/ls-l.txt
35 records.
Histogram of number of tokens per record:
	3:	1
	15:	15
	16:	2
	17:	2
	18:	13
	19:	1
	20:	1

Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [Date]
Total number of token occurrences: 34.
Number of records with at least one token occurrence: 34.
StructScore: 1.
	1:	34	0.971428571429


Cluster 1:
Token: [white]
Total number of token occurrences: 240.
Number of records with at least one token occurrence: 35.
StructScore: 4.
	1:	1	0.0285714285714
	7:	33	0.942857142857
	8:	1	0.0285714285714


Cluster 2:
Token: [int]
Total number of token occurrences: 97.
Number of records with at least one token occurrence: 35.
StructScore: 16.
	1:	1	0.0285714285714
	2:	7	0.2
	3:	26	0.742857142857
	4:	1	0.0285714285714


Cluster 3:
Token: [other](-)
Total number of token occurrences: 18.
Number of records with at least one token occurrence: 18.
StructScore: 17.
	1:	18	0.514285714286


Cluster 4:
Token: [string]
Total number of token occurrences: 153.
Number of records with at least one token occurrence: 35.
StructScore: 23.
	1:	1	0.0285714285714
	4:	19	0.542857142857
	5:	14	0.4
	6:	1	0.0285714285714


Cluster 5:
Token: [Time]
Total number of token occurrences: 7.
Number of records with at least one token occurrence: 7.
StructScore: 28.
	1:	7	0.2


Cluster 6:
Token: [Host]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 34.
	1:	1	0.0285714285714

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 34.
	1:	1	0.0285714285714


Cluster 7:
Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 61.
	1:	14	0.4
	2:	1	0.0285714285714


Junk Tolerance Threshold: 4
Coverage: 34
Num Tokens: 1
Struct
Coverage:34
Token count:1
[Date]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [int]
Total number of token occurrences: 68.
Number of records with at least one token occurrence: 34.
StructScore: 0.
	2:	34	1.0

Token: [string]
Total number of token occurrences: 102.
Number of records with at least one token occurrence: 34.
StructScore: 0.
	3:	34	1.0

Token: [white]
Total number of token occurrences: 170.
Number of records with at least one token occurrence: 34.
StructScore: 0.
	5:	34	1.0


Cluster 1:
Token: [other](-)
Total number of token occurrences: 18.
Number of records with at least one token occurrence: 18.
StructScore: 16.
	1:	18	0.529411764706


Junk Tolerance Threshold: 4
Coverage: 34
Num Tokens: 10
Struct
Coverage:34
Token count:10
[int]	Occurrences:2
[string]	Occurrences:3
[white]	Occurrences:5
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [other](-)
Total number of token occurrences: 18.
Number of records with at least one token occurrence: 18.
StructScore: 16.
	1:	18	0.529411764706


Cluster 1:
Token: [empty]
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 16.
StructScore: 18.
	1:	16	0.470588235294


Junk Tolerance Threshold: 4
Coverage: 18
Num Tokens: 1
Struct
Coverage:18
Token count:1
[other](-)	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [empty]
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 16.
StructScore: 0.
	1:	16	1.0


Junk Tolerance Threshold: 2
Coverage: 16
Num Tokens: 1
Struct
Coverage:16
Token count:1
[empty]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [white]
Total number of token occurrences: 69.
Number of records with at least one token occurrence: 34.
StructScore: 1.
	2:	33	0.970588235294
	3:	1	0.0294117647059


Cluster 1:
Token: [string]
Total number of token occurrences: 50.
Number of records with at least one token occurrence: 34.
StructScore: 17.
	1:	19	0.558823529412
	2:	14	0.411764705882
	3:	1	0.0294117647059


Cluster 2:
Token: [int]
Total number of token occurrences: 28.
Number of records with at least one token occurrence: 27.
StructScore: 22.
	1:	26	0.764705882353
	2:	1	0.0294117647059


Cluster 3:
Token: [Time]
Total number of token occurrences: 7.
Number of records with at least one token occurrence: 7.
StructScore: 27.
	1:	7	0.205882352941


Cluster 4:
Token: [Host]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059


Cluster 5:
Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 58.
	1:	14	0.411764705882
	2:	1	0.0294117647059


Junk Tolerance Threshold: 4
Coverage: 33
Num Tokens: 2
Struct
Coverage:33
Token count:2
[white]	Occurrences:2
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [int]
Total number of token occurrences: 27.
Number of records with at least one token occurrence: 27.
StructScore: 7.
	1:	27	0.794117647059


Cluster 1:
Token: [Time]
Total number of token occurrences: 7.
Number of records with at least one token occurrence: 7.
StructScore: 27.
	1:	7	0.205882352941


Junk Tolerance Threshold: 4
Coverage: 27
Num Tokens: 1
Struct
Coverage:27
Token count:1
[int]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [Time]
Total number of token occurrences: 7.
Number of records with at least one token occurrence: 7.
StructScore: 0.
	1:	7	1.0


Junk Tolerance Threshold: 1
Coverage: 7
Num Tokens: 1
Struct
Coverage:7
Token count:1
[Time]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [string]
Total number of token occurrences: 50.
Number of records with at least one token occurrence: 34.
StructScore: 17.
	1:	19	0.558823529412
	2:	14	0.411764705882
	3:	1	0.0294117647059


Cluster 1:
Token: [Host]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059

Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059


Cluster 2:
Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 58.
	1:	14	0.411764705882
	2:	1	0.0294117647059


Junk Tolerance Threshold: 4
Coverage: 34
Num Tokens: 0
Clusters sorted by array criteria:
Cluster 0:
Token: [string]
Total number of token occurrences: 50.
Number of records with at least one token occurrence: 34.
StructScore: 17.
	1:	19	0.558823529412
	2:	14	0.411764705882
	3:	1	0.0294117647059


Cluster 1:
Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 58.
	1:	14	0.411764705882
	2:	1	0.0294117647059


Cluster 2:
Token: [Host]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059

Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 33.
	1:	1	0.0294117647059


Possible array tokens:
[string]
Records in possible array context:34
Total:50
Coverage:34
Width:3
ARRAY NOT CHOSEN
Union
BUILDING UNION TY
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [Host]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0

Token: [string]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0


Junk Tolerance Threshold: 1
Coverage: 1
Num Tokens: 2
Struct
Coverage:1
Token count:2
[Host]	Occurrences:1
[string]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [string]
Total number of token occurrences: 49.
Number of records with at least one token occurrence: 33.
StructScore: 17.
	1:	18	0.545454545455
	2:	14	0.424242424242
	3:	1	0.030303030303


Cluster 1:
Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303


Cluster 2:
Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 55.
	1:	14	0.424242424242
	2:	1	0.030303030303


Junk Tolerance Threshold: 4
Coverage: 33
Num Tokens: 0
Clusters sorted by array criteria:
Cluster 0:
Token: [string]
Total number of token occurrences: 49.
Number of records with at least one token occurrence: 33.
StructScore: 17.
	1:	18	0.545454545455
	2:	14	0.424242424242
	3:	1	0.030303030303


Cluster 1:
Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 55.
	1:	14	0.424242424242
	2:	1	0.030303030303


Cluster 2:
Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303


Possible array tokens:
[string]
Records in possible array context:33
Total:49
Coverage:33
Width:3
ARRAY NOT CHOSEN
Union
BUILDING UNION TY
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [string]
Total number of token occurrences: 33.
Number of records with at least one token occurrence: 33.
StructScore: 0.
	1:	33	1.0


Junk Tolerance Threshold: 4
Coverage: 33
Num Tokens: 1
Struct
Coverage:33
Token count:1
[string]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [empty]
Total number of token occurrences: 17.
Number of records with at least one token occurrence: 17.
StructScore: 16.
	1:	17	0.515151515152


Cluster 1:
Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 32.
	1:	1	0.030303030303


Cluster 2:
Token: [string]
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 55.
	1:	14	0.424242424242
	2:	1	0.030303030303

Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 55.
	1:	14	0.424242424242
	2:	1	0.030303030303


Junk Tolerance Threshold: 4
Coverage: 17
Num Tokens: 1
Struct
Coverage:17
Token count:1
[empty]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [string]
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 4.
	1:	14	0.875
	2:	1	0.0625

Token: [other](.)
Total number of token occurrences: 16.
Number of records with at least one token occurrence: 15.
StructScore: 4.
	1:	14	0.875
	2:	1	0.0625


Cluster 1:
Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 15.
	1:	1	0.0625

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 15.
	1:	1	0.0625

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 15.
	1:	1	0.0625


Junk Tolerance Threshold: 2
Coverage: 14
Num Tokens: 2
Struct
Coverage:14
Token count:2
[string]	Occurrences:1
[other](.)	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [empty]
Total number of token occurrences: 13.
Number of records with at least one token occurrence: 13.
StructScore: 2.
	1:	13	0.866666666667


Cluster 1:
Token: [string]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 14.
	1:	1	0.0666666666667

Token: [other](.)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 14.
	1:	1	0.0666666666667

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 14.
	1:	1	0.0666666666667


Junk Tolerance Threshold: 2
Coverage: 13
Num Tokens: 1
Struct
Coverage:13
Token count:1
[empty]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [string]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 1.
	1:	1	0.5

Token: [other](.)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 1.
	1:	1	0.5

Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 1.
	1:	1	0.5


Junk Tolerance Threshold: 1
Coverage: 1
Num Tokens: 3
Struct
Coverage:1
Token count:3
[string]	Occurrences:1
[other](.)	Occurrences:1
[other](~)	Occurrences:1
converting false struct into union
BUILDING UNION TY
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [string]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0

Token: [other](.)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0


Junk Tolerance Threshold: 1
Coverage: 1
Num Tokens: 2
Struct
Coverage:1
Token count:2
[string]	Occurrences:1
[other](.)	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [other](~)
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0


Junk Tolerance Threshold: 1
Coverage: 1
Num Tokens: 1
Struct
Coverage:1
Token count:1
[other](~)	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0


Junk Tolerance Threshold: 1
Coverage: 1
Num Tokens: 2
Struct
Coverage:1
Token count:2
[int]	Occurrences:1
[white]	Occurrences:1
Building histograms...
THRESHOLD for histogram equality: 1.
Computed clusters
Cluster 0:
Token: [int]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0

Token: [string]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0

Token: [white]
Total number of token occurrences: 1.
Number of records with at least one token occurrence: 1.
StructScore: 0.
	1:	1	1.0


Junk Tolerance Threshold: 1
Coverage: 1
Num Tokens: 3
Struct
Coverage:1
Token count:3
[int]	Occurrences:1
[string]	Occurrences:1
[white]	Occurrences:1

Before reduction:
Punion(Id = BTy_44 35, 1b, 0b)
	Pstruct(Id = BTy_43 34, 1b, 0b)
		Punion(Id = BTy_1 34, 1b, 0b)
			[other](-)(Id = BTy_0 18, 1b, 0b);
			[empty](Id = BTy_2 16, 1b, 0b);
		End Punion;
		[string](Id = BTy_3 34, 1b, 0b);
		[white](Id = BTy_4 34, 1b, 0b);
		[int](Id = BTy_5 34, 1b, 0b);
		[white](Id = BTy_6 34, 1b, 0b);
		[string](Id = BTy_7 34, 1b, 0b);
		[white](Id = BTy_8 34, 1b, 0b);
		[string](Id = BTy_9 34, 1b, 0b);
		[white](Id = BTy_10 34, 1b, 0b);
		[int](Id = BTy_11 34, 1b, 0b);
		[white](Id = BTy_12 34, 1b, 0b);
		[Date](Id = BTy_14 34, 1b, 0b);
		[white](Id = BTy_15 34, 1b, 0b);
		Punion(Id = BTy_17 34, 1b, 0b)
			[int](Id = BTy_16 27, 1b, 0b);
			[Time](Id = BTy_18 7, 1b, 0b);
		End Punion;
		[white](Id = BTy_19 34, 1b, 0b);
		Punion(Id = BTy_41 34, 1b, 0b)
			Pstruct(Id = BTy_22 1, 1b, 0b)
				[Host](Id = BTy_20 1, 1b, 0b);
				[string](Id = BTy_21 1, 1b, 0b);
			End Pstruct;
			Pstruct(Id = BTy_23 33, 1b, 0b)
				[string](Id = BTy_24 33, 1b, 0b);
				Punion(Id = BTy_26 33, 1b, 0b)
					[empty](Id = BTy_25 17, 1b, 0b);
					Pstruct(Id = BTy_36 15, 1b, 0b)
						[other](.)(Id = BTy_27 15, 1b, 0b);
						[string](Id = BTy_28 15, 1b, 0b);
						Punion(Id = BTy_30 15, 1b, 0b)
							[empty](Id = BTy_29 13, 1b, 0b);
							Pstruct(Id = BTy_33 1, 1b, 0b)
								[other](.)(Id = BTy_31 1, 1b, 0b);
								[string](Id = BTy_32 1, 1b, 0b);
							End Pstruct;
							[other](~)(Id = BTy_34 1, 1b, 0b);
						End Punion;
					End Pstruct;
					Pstruct(Id = BTy_40 1, 1b, 0b)
						[white](Id = BTy_38 1, 1b, 0b);
						[int](Id = BTy_39 1, 1b, 0b);
					End Pstruct;
				End Punion;
			End Pstruct;
		End Punion;
	End Pstruct;
	Pstruct(Id = BTy_48 1, 1b, 0b)
		[string](Id = BTy_45 1, 1b, 0b);
		[white](Id = BTy_46 1, 1b, 0b);
		[int](Id = BTy_47 1, 1b, 0b);
	End Pstruct;
End Punion


After final reduction:
Punion(Id = BTy_44 35, 1b, 0b)
	Pstruct(Id = BTy_43 34, 1b, 0b)
		Switch(BTy_5)(Id = BTy_1 34, 1b, 0b):
		case [IntConst] [1]:
			[StringConst] "-"(Id = BTy_0 18, 1b, 0b);
		case [StringConst] "*":
			[StringConst] ""(Id = BTy_2 16, 1b, 0b);
		End Switch;
		[string](Id = BTy_3 34, 1b, 0b);
		[StringConst] "   "(Id = BTy_4 34, 1b, 0b);
		[Int] [1...21](Id = BTy_5 34, 1b, 0b);
		[StringConst] " dpw fac      "(Id = BTy_6 34, 1b, 0b);
		[Int] [0...274595840](Id = BTy_11 34, 1b, 0b);
		[StringConst] " "(Id = BTy_12 34, 1b, 0b);
		[Date](Id = BTy_14 34, 1b, 0b);
		[StringConst] "  "(Id = BTy_15 34, 1b, 0b);
		Switch(BTy_14)(Id = BTy_17 34, 1b, 0b):
		case [StringConst] "*":
			[Int] [2002...2006](Id = BTy_16 27, 1b, 0b);
		case [Enum] {[StringConst] "Jan  2", [StringConst] "Jan  5", [StringConst] "Jan 14", [StringConst] "Jul 25", [StringConst] "Nov 15", [StringConst] "Sep 11", }:
			[Time](Id = BTy_18 7, 1b, 0b);
		End Switch;
		[StringConst] " "(Id = BTy_19 34, 1b, 0b);
		Switch(BTy_11)(Id = BTy_41 34, 1b, 0b):
		case [IntConst] [145920]:
			Pstruct(Id = BTy_22 1, 1b, 0b)
				[StringConst] "e-DARPA60.doc"(Id = BTy_20 1, 1b, 0b);
			End Pstruct;
		case [StringConst] "*":
			Pstruct(Id = BTy_23 33, 1b, 0b)
				[string](Id = BTy_24 33, 1b, 0b);
				Punion(Id = BTy_26 33, 1b, 0b)
					[StringConst] ""(Id = BTy_25 17, 1b, 0b);
					Pstruct(Id = BTy_36 15, 1b, 0b)
						[StringConst] "."(Id = BTy_27 15, 1b, 0b);
						[string](Id = BTy_28 15, 1b, 0b);
						Punion(Id = BTy_30 15, 1b, 0b)
							[StringConst] ""(Id = BTy_29 13, 1b, 0b);
							Pstruct(Id = BTy_33 1, 1b, 0b)
								[StringConst] ".gz"(Id = BTy_31 1, 1b, 0b);
							End Pstruct;
							[StringConst] "~"(Id = BTy_34 1, 1b, 0b);
						End Punion;
					End Pstruct;
					Pstruct(Id = BTy_40 1, 1b, 0b)
						[StringConst] " "(Id = BTy_38 1, 1b, 0b);
						[IntConst] [8](Id = BTy_39 1, 1b, 0b);
					End Pstruct;
				End Punion;
			End Pstruct;
		End Switch;
	End Pstruct;
	Pstruct(Id = BTy_48 1, 1b, 0b)
		[StringConst] "total "(Id = BTy_45 1, 1b, 0b);
		[IntConst] [275528](Id = BTy_47 1, 1b, 0b);
	End Pstruct;
End Punion

Complexity of inferred type:
	numAlt = 9  numTBD = 0  numBtm = 0
Outputing partitions to directory: gen/

Completed data/ls-l.txt
Overall type complexity = 35.170b
Overall data complexity = 728.983b

