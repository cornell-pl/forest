structure AI = struct
    open Model
    val aiAux: AuxInfo = {coverage = 3000, label = NONE, typeComp = zeroComp, datacomp = zeroComp }
    val aiLoc : location = {lineNo = 0, beginloc = 0, endloc = 0, recNo = 0}
    val ip : Ty = Base(aiAux, [(Pip "135.207.23.32", aiLoc)])
    val host : Ty = Base(aiAux, [(Phost "www.cnn.com", aiLoc)])
    ...
    val intbase:Ty = Base(aiAux, [(Pint 1, aiLoc)])
    ...
    val response:Ty = RefinedBase(aiAux, Int (100, 600), [(Pint 100, aiLoc)])
