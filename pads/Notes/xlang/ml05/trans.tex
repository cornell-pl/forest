...
data with a semantic error, \ie{} data that violates a constraint written in a set type.  The final description \pdunk{} describes data for which a top-level descriptor is incomplete.  This case arises when parsing a potentially infinite stream, for example.  In this case, the user must query the descriptor of individual elements rather than of the stream itself. Note that in the actual language,
