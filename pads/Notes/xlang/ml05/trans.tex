...
To preserve soundness, we do not permit programmers to construct
descriptors or to pair them with representations to create \pvalue{}s.
However, we do provide a
generic pattern for all \pvalue{}s, \patreadpd {pat} {pdpat}, to
extract and read a value's descriptor.  In this pattern, the right subpattern matches only the descriptor, while the left subpattern matches the entire \pvalue{}.  
There are currently five possible
values for a descriptor: \pdgood{}, \pdbad{},\pdnest{}, 
...
data with a semantic error, \ie{} data that violates a constraint
written in a constrained type.  The final description \pdunk{} describes data for which a top-level descriptor is incomplete.  This case arises when parsing a potentially infinite stream, for example.  In this case, the user must query the descriptor of individual elements rather than of the stream itself. Note that in the actual language,
