...
fun removePPempty ty =
case ty of
  PPstruct (a, tys) =>
    let		
	val tys' = map removePPempty tys
    in
	case tys' of
	  nil => ty
	| _ =>
		...
		case ty of
		  PPBase (_, ltokens) => 
		    (case (hd ltokens) of 
		     ...
		     | _ => true)
		 | _ => true 
	    val nonEmpties = List.filter isNotPPempty tys' 
          in
	    case nonEmpties of
	      nil => hd tys'
	    | _ => PPstruct (a, nonEmpties)
          end
    end
  | PPunion (a, tys) => PPunion (a, map removePPempty tys)
  | PParray (a, {tokens=t, lengths = l, first=f, body=b, last=la}) =>
	PParray (a, {tokens=t, lengths=l, first=removePPempty f, 
			body=removePPempty b, last=removePPempty la})
  | _ => ty
