...
open Ast
  ...
  fun newfieldToPADSC isEnum f =
    ...
    | CompField (t, (v, NONE, NONE, SOME (IntConst x))) => 
		if x = 0 then "\tPempty" else
		"\tPcompute " ^ newtyNameToPADSCString t ^ " " ^ v ^ " = " ^ (largeIntToStr x)
    | FullField (v, t, sw, c) => 
	let val tyname = newtyNameToPADSCString t in
	"\t" ^ tyname ^ " " ^  
	(case sw of 
	   SOME swv => "(:" ^ swv ^ ":)"
	 | NONE => ""
  ...
  fun newirToPADSC irTy = 
    let val (isRecord, tyVar, tyDef) = irTy
	val tyVarStr = newtyNameToPADSCString tyVar
  	val precord = if isRecord then "Precord " else ""
    in
      precord ^
      (
      case tyDef of
        TyBase (tyName, cons_op) =>
	  let val tyNameStr = newtyNameToPADSCString tyName
	  in
	   case cons_op of
	     NONE => "Ptypedef " ^ tyNameStr ^ " " ^ tyVarStr ^";\n"
	   | SOME (var, NONE, NONE, SOME (IntConst i)) =>
	   ...
	   | _ => raise TyMismatch
    	...
    	| TyStruct fields => "Pstruct " ^ tyVarStr ^ " {\n" ^ 
				(String.concatWith ";\n" (map (newfieldToPADSC false) fields)) ^ ";\n};\n"
	| TyUnion fields => "Punion " ^ tyVarStr ^ " {\n" ^ 
				(String.concatWith ";\n" (map (newfieldToPADSC false) fields)) ^ ";\n};\n"
	| TyEnum fields => "Penum " ^ tyVarStr ^ " {\n" ^ 
				(String.concatWith ",\n" (map (newfieldToPADSC true) fields)) ^ "\n};\n"
	| TySwitch (swVar, swTyName, branches) => 
		let
		  val swTyNameStr = newtyNameToPADSCString swTyName 
		  fun branchtoStr (e, f) = 
		  case e of
		    EnumInt i => "\tPcase " ^ (largeIntToStr i) ^ ": " ^ (newfieldToPADSC true f) ^ ";\n"
		  | EnumVar v => "\tPcase " ^ v ^ ": " ^ (newfieldToPADSC true f) ^ ";\n"
		  | EnumDefault => "\tPdefault: " ^(newfieldToPADSC true f) ^ ";\n"
		in
		  "Punion " ^ tyVarStr ^ " (:" ^ swTyNameStr ^ " " ^ swVar ^ ":) {\n" ^
		  "  Pswitch (" ^ swVar ^ ") {\n" ^
		  (lconcat (map branchtoStr branches)) ^
	...
	let
	  val bodyLabel = 
		...
		case ty of
		  PPunion (_, tys) => 
		    let
		      val body = List.nth (tys, numHeaders)
		    in (newtyNameToPADSCString (getNewTypeName body)) 
		    end
		  | _ => raise TyMismatch
	    else
		newtyNameToPADSCString (
		case ty of
		  PPBase _ => getNewBaseTyName ty
	  ...
	  val headerLabel = 
		...
		case ty of
		  PPunion (_, tys) => (newtyNameToPADSCString (getNewTypeName (List.nth (tys, 0)))) 
		  | _ => raise TyMismatch
	    else "Header"
	  val footerLabel = 
	    if numFooters = 0 then ""
		...
		case ty of
		  PPunion (_, tys) => (newtyNameToPADSCString (getNewTypeName (List.nth (tys, (numHeaders+1))))) 
		  | _ => raise TyMismatch
	    else "Footer"
	  ...
	  val (pads, topLabel) =
		(if numHeaders=0 andalso numFooters=0 then
		    let val irTys = newtyToIR true nil ty
			val body = (lconcat (map newirToPADSC irTys))
		    in
		       (incString ^
			body ^
			 "Psource Parray entries_t {\n" ^
		  ...
		  case ty of 
			...
			raise Fail "Header Footer incorrect!"
				...
				val body = List.nth (tys, numHeaders)
				val footers = List.drop (tys, (numHeaders+1))
				val headerIRs = List.concat (map (newtyToIR true nil) headers)
				val bodyIRs = newtyToIR true nil body
				val footerIRs = List.concat (map (newtyToIR true nil) footers) 
				val l = getLabelString (getNAuxInfo ty)
	    			val topLabel = "Struct_" ^ (String.extract (l, 4, NONE))
			      in
			        (incString ^
				 (case headerIRs of
					nil => ""
					| _ => (lconcat (map newirToPADSC headerIRs)) 
				 ) ^
				(case headerIRs of
					nil => ""
					| [_] => ""
					| _ => "Pstruct Header {\n" ^
					  (String.concat (map 
					  (fn t => ("\t" ^ newtyNameToPADSCString (getNewTypeName t) ^ " " ^
						newgetVar t ^ ";\n")) 
					  headers)) ^ "};\n"
				) ^
				(lconcat (map newirToPADSC bodyIRs)) ^
				(case footerIRs of
					nil => ""
					| _ => (lconcat (map newirToPADSC footerIRs))
				) ^
				(case footerIRs of
					nil => ""
					| [_] => ""
				...
				(case headerIRs of
					nil => ""
					| [_] => "\t" ^ headerLabel ^ " " ^ newgetVar (hd headers) ^ ";\n"
					| _ => "\tHeader v_header;\n"
				) ^
				("\t" ^ (newtyNameToPADSCString (getNewTypeName body)) ^ 
					"[] " ^ (newgetVar body) ^" : Plongest;\n") ^
				(case footerIRs of 
					nil => ""
					| [_] => "\t" ^ footerLabel ^ " " ^ newgetVar (hd footers) ^ ";\n"
					| _ => "\tFooter v_footer;\n"
				) ^
				"};\n",
				topLabel)
