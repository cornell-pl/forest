...
struct
    ...
    type NTokenOrder = BSToken list

    type DerivedContexts = Context list
    type Partition = (TokenOrder * (DerivedContexts list)) list * (Context list)

    type NDerivedContexts = NewContext list
    type NPartition = (NTokenOrder * (NDerivedContexts list)) list * (NewContext list)
    ...
    datatype NewTy = PPBase    of AuxInfo * BSLToken list (* list will never be empty *)
	        (* TBD is introduced when maxdepth limit kicks in *)
                | PPTBD     of AuxInfo *
                             int     * (* Sequence number on tbds *) 
                             NewContext list
                ...
                | PPBottom  of AuxInfo *
                             int     * (* Sequence number on bottoms *)
                             NewContext list 
                | PPstruct of AuxInfo * NewTy list 
                | PPunion  of AuxInfo * NewTy list
                | PParray  of AuxInfo * { tokens:(BSToken * int) list, (* list of tokens and freq count in 
								       cluster identified as an array cell *)
					 lengths: (int * int) list, (* list of array (lengths,linenumbers) *)
					 first : NewTy, (* inferred type for first cell of list of arrays *)
					 body  : NewTy, (* inferred type for middle cells of list of arrays *)
					 last  : NewTy} (* inferred type for last cell of list of arrays *)

                | PPRefinedBase of AuxInfo * Refined * BSLToken list
                | PPSwitch  of AuxInfo * Id * (Refined (* switch value *) * NewTy) list
                | PPRArray  of AuxInfo * Refined option (* separator *)
                                     * Refined option (* terminator *)
	                             * NewTy             (* body type *)
                                     * Refined option (* fixed length *) 
                                     * (int*int) list (* (length, linenumber) list*)
                | PPoption of AuxInfo * NewTy (* a Ty which is optional *)
    ...
    fun getNAuxInfo ( ty : NewTy ) : AuxInfo = 
	case ty 
        of PPBase (a,t)                   => a
        |  PPTBD (a,i,cl)                 => a
        |  PPBottom (a,i,cl)              => a
        ...
        |  PPunion (a,tys)               => a
        |  PParray (a, _)                => a
        |  PPRefinedBase (a,r,tl)         => a
        |  PPSwitch(a,id,branches)        => a
        |  PPRArray (a,sep,term,body,len,lengths) => a
        |  PPoption (a, _)               => a
    ...
    fun sumNCoverage ( tys : NewTy list ) : int =
        foldl ( fn (t:NewTy,n:int) => getNCoverage t + n ) 0 tys
    fun minNCoverage ( tys : NewTy list ) : int = 
	case tys of [] => Option.valOf Int.maxInt
        | (ty::tys) => Int.min(getNCoverage ty, minNCoverage tys)
...
(*
	...
	in
*)
	    "[Line#"^(Int.toString lineNo)^"Rec#"^(Int.toString recNo)^"Begin#"^(Int.toString beginloc)^"End#"^(Int.toString endloc)^"]  "
(*
	end
*)
...
*)
    ...
    fun bstokenTyToString ( (b, s) : BSToken ) : string = "["^(BTokenToName b)^"]" 
    ...
    fun printBSTokenTy ( (b,s) : BSToken ) : unit = print ("["^(BTokenToName b)^"]")

    fun LTokensToString [] = "\n"
      | LTokensToString (t::ts) = (ltokenToString t) ^ (LTokensToString ts)

    fun BSLTokensToString [] = "\n"
      | BSLTokensToString (t::ts) = (bsltokenTyToString t) ^ (BSLTokensToString ts)
    ...
    fun newcontextsToString ( contexts : NewContext list ) : string = 
	((case contexts 
	  of [] => "<no records matched context>\n"
	  | _ => (lconcat(
		  List.map (fn tl => 
    ...
    fun NewTyToStringD (prefix:string) (longTBDs:bool) (longBottom:bool)
                    (suffix:string) (ty:NewTy) : string = 
    let val aux = getNAuxInfo ty
        val { tc = tcomp, adc = acomp, dc = dcomp } = #tycomp aux
        val stats = ( "(" ^  (covToString aux)  ^
		      (if print_complexity then (
                      ", tc: " ^ (showBits tcomp)  ^
        ...
        val partialD = NewTyToStringD (prefix^"\t") longTBDs longBottom (";\n")
    in ( prefix ^
         ( case ty of
               PPBase (aux, ts)  =>
                 ...
                 in ( case ts of nil =>
                         "[NULL]"
                       | _ => (bsltokenTyToString (hd ts)) (*(BSLTokensToString ts)*)
                    ) ^ " " ^ stats ^
		    (if print_complexity then 
			(" (avg: " ^ Real.fmt (StringCvt.FIX (SOME 2)) avg ^
                                      ", tot: " ^ LargeInt.toString tot ^ ")")
             ...
             | PPTBD (aux,i,cl) =>
                "TBD_" ^ (Int.toString i) ^ stats ^
                ( if longTBDs then ( "\n"^ (newcontextsToString cl) ^ prefix ^ "End TBD" ) else "" )
             | PPBottom (aux,i, cl) =>
                "BTM_" ^ (Int.toString i) ^ stats ^
                (if longBottom then ("\n"^(newcontextsToString cl)^prefix^"End Bottom") else "")
             | PPstruct (aux, tys) =>
                "Pstruct" ^ stats ^ "\n" ^
                (lconcat (List.map partialD tys)) ^ prefix ^ "End Pstruct"
             | PPunion (aux, tys)  =>
             ...
             | PParray (aux, {tokens=tkns, lengths, first=ty1,body=ty2,last=ty3}) =>
                "Parray" ^ stats ^ 
                "("^(lconcat(List.map (fn (t,loc) => (bstokenTyToString t) ^" ")tkns)) ^")\n"^
                prefix ^ "First:\n" ^ (partialD ty1) ^ prefix ^ "Body:\n" ^ (partialD ty2) ^
                prefix ^ "Tail:\n" ^ (partialD ty3) ^ prefix ^ "End Parray"
             | PPRefinedBase (aux, refined, tl) =>
                 let val avg = avgBSLTokenLength tl
                     val tot = sumBSLTokenLength tl
                 in ( refinedToString refined ) (*^ (LTokensToString tl)*)
		      ^ " " ^ stats ^ 
		    (if print_complexity then 
			(" (avg: " ^ Real.fmt (StringCvt.FIX (SOME 2)) avg ^
             ...
             | PPSwitch(aux ,id, retys) =>
                ...
                (lconcat (List.map (fn (re, ty) => (prefix^"case "^(refinedToString re)^":\n"^ 
                (partialD ty))) retys)) ^ prefix ^ "End Switch"
             | PPRArray (aux, sep, term, body, len, lengths) => 
                "RArray" ^ stats ^ "\n" ^
                ( case sep of
                       SOME septok =>
                         prefix ^ "\tSeparator: "^ refinedToString(septok) ^ "\n"
    ...
    fun NewTyToString (ty:NewTy): string = NewTyToStringD "" false false "" ty 
...
*)
    ...
    fun printSSLoc (ss: Seqset) : unit =
      let
        ...
        fun printOneTS ((tl, f): Tokenseq) = List.app printOneBSL tl
      in
