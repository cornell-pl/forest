...
toKind kind = case kind of
   Hs.KindStar     -> StarK
   Hs.KindFn k1 k2 -> ArrowK (toKind k1) (toKind k2) 
   a@Hs.KindBang       -> error $ errorMsg "toType" a
   a@(Hs.KindParen k)  -> error $ errorMsg "toType" a
   a@(Hs.KindVar n)    -> error $ errorMsg "toType" a

toTyVarBind (Hs.KindedVar n k)  = KindedTV (toName n) (toKind k)
toTyVarBind (Hs.UnkindedVar n ) = PlainTV  (toName n)

toPred asst = 
  case asst of
    Hs.ClassA   qname tys       -> ClassP (toName qname) (map toType tys)
    a@(Hs.InfixA ty1 qname ty2) -> error $ errorMsg "toPred" a
    a@(Hs.IParam ipName ty)     -> error $ errorMsg "toPred" a
    Hs.EqualP ty1 ty2           -> EqualP (toType ty1) (toType ty2)
...
instance ToType Hs.Type where
  toType (Hs.TyForall tvbM cxt t) = ForallT (maybe [] (fmap toTyVarBind) tvbM) (fmap toPred cxt) (toType t)
...
instance ToDec Hs.Decl where
  toDec (Hs.TypeDecl _ n ns t)
    = TySynD (toName n) (fmap toTyVarBind ns) (toType t)
...
qualConDeclToCon (Hs.QualConDecl _ ns cxt cdecl) = ForallC (fmap toTyVarBind ns)
                                                    (fmap toPred cxt)
                                                    (conDeclToCon cdecl)
