...
sub inc_lrn
{
  (my $file, my $isize, my $lsize, my $doparse, my $adcw, my $uc, my $initpxml) = @_;
  my $score = 0;
  my $time = 0;
  my $rptime = 0;
  my $exectime = 0;
  ...
  for (my $j = 0; $j < $numTimes; $j++)
  {
    $exectime = 0;
    if ($doparse) {
      $inctimedout = timed_exec ("increment -f $file $initpart -l $lsize -output gen -reparse true $adc_str $uc_str > $filename.inc", 2*$tmout);
      #system ("increment -f $file -i $isize -l $lsize -output gen -reparse true $adc_str $uc_str > $filename.inc"); 
    } else
    {
      $inctimedout = timed_exec("increment -f $file $initpart -l $lsize -output gen $adc_str $uc_str > $filename.inc", $tmout);
...
if (!@ARGV) {
  print "Usage: wasl-run.pl init inc LARGE_FILES [-small] [-scale] [-incsize] 
         [-single] [-noparse] [-initdesc PATH] [-w ADC_WEIGHT] [-u FLOAT]
...
-initdesc: the dir where descriptions can be found
...
if ($otherargs =~ /.*-incsize.*/) {
  ...
  if ($otherargs =~ /.*-initdesc ([^ ]+)/) {
   my $initdesc = "$1/$largefile.pxml";
   if (-e $initdesc) {
   for (my $j = 25; $j <= 6400; $j*=4)
   { 
   ($tc, $adc, $score, $dist, $rate, $time, $rptime, $padstime, $blobtime, $inctimedout) = 
	inc_lrn ($largefile, 0, $j, 0, $adc_weight, $uc, $initdesc);
    if ($score==0) {
        print "$fname (init=$initdesc, inc=$j): timed out\n";
	if (!$inctimedout) {$timeout=1; last}
	else {}
    }
    elsif ($score==-1) {
        print "$fname (init=$initdesc, inc=$j): exception raised\n";
    }
    else {
        $dist = getGoldDist ($largefile);
        output("$fname (init=$initdesc, inc=$j)", $tc, $adc, $score, $dist, $rate, $time, $rptime, $padstime, $blobtime, 0);
    }
   }
   } 
  }
