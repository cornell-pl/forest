The \padsd{} language allows users to describe {\em feeds}, which is
...
The \padsd{} specification in \figref{fig:simplecomon} describes a
simple CoMon statistics feed \cd{simple\_comon}
using the \kw{base} feed constructor.  This constructor contains three fields.
The \kw{sources} field 
...
With replicated distributed systems, many sites may contain all the
desired information.   
...
\cd{comon\_1} feed defined in \figref{fig:comon_1} specifies this behavior
by using the \kw{any} constructor in the \kw{sources} field.
The schedule for \cd{comon\_1} indicates the system should fetch data
every minute for two hours, using the \cd{lasting} clause to indicate
...
behavior of replicated systems.
\cut{%%%%%%%%
, such as those using
