...
\label{sec:tools}
\cut{
\reviewer{The generic tools [...] don't seem to provide any value over
  ...
  structures improve efficiency in practice?}

\reviewer{It's good to have these tools, but it's easy to imagine how
  they work.  They don't need an entire page.}
}
...
metadata class \cd{ForestMD}) and a predicate on \cd{FileInfo}
structures, and returns the list of all \cd{FilePath}s anywhere in the
input metadata whose associated \cd{FileInfo} satisfies the
predicate.  For example, if \cd{cs_md} is the metadata associated
...
\cd{FileInfo}s in the metadata.  We then convert this list into a
...
the argument metadata structure using \cd{listify} and then calling
...
be passed to \cd{ls}. For \cd{grep}, the second argument is the search
string and result is the output of the shell version of \cd{grep}. For
...
uses the \cd{tar} tool to move the files mentioned in the metadata to
the location specified by the second argument \textit{while retaining
  the same directory structure}.  The module that implements these
tools is approximately 80 lines of Haskell code.
