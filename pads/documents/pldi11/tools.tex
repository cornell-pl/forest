...
\cd{other} to all the directories and files not owned by \cd{"dpw"}.
...
\graphviz{}~\cite{Gansner+:graphviz} attributes.  By appropriately setting the
...
Finally, we have implemented analogs of many shell tools that work
over only the portion of the file system that match the given
\forest{} description:
\begin{code}
...
search and result is the output of the shell version of \cd{grep}. For
