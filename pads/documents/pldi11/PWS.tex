...
[pads| 
 \mkred{-- Configuration file for learning demo web site; contains paths to various web site components.}
  \kw{data} Config_f =  \{
                      header      :: [Pstringln] \kw{with} term length of 13,
   ...
   \}
\mbox{}
  \kw{type} Config_entry_t = Line (" \\"",  Pstring '\\"',  "\\";")   
  \kw{type} Header_t = [Pstringln] \kw{with} term length of 13
\mbox{}
  \mkred{\{- File listing data sources for web site -\}}
  \kw{type} SourceNames_f = [Pstringln]
\mbox{}
  \mkred{\{- Information related to a single user's use of the web site -\}}
  \kw{type} UserEntries_f = [Line UserEntry_t] \kw{with} term Eor
\mbox{}
  ...
  \kw{data} UserEntry_t =  \{
     "id.",   usrId :: Pint,               
    ",id.",   dirId :: (Pint, '.', Pint)    \kw{where} <| usrId == fst dirId |> 
  \} 
\mbox{}

  \mkred{\{- Log of requests.  Used to prevent denial of service attacks. -\}}
  \kw{type} LogFile_f = [LogEntry_t]
\end{code}
\vfill
...
\mbox{}
  \mkred{\{- Request entry.  -\}}
  \kw{data} LogEntry_t = \{
    userId :: Pint,        ',',   \mkred{ --user making request}
    ip     :: IP_t,        ',',   \mkred{ --IP address of requestor}
    script :: Pstring ' ', ' ',   \mkred{ --script to be executed}
    userDir:: Pstring ' ', ' ',   \mkred{ --directory to put results, corresponds to user}
    padsv  :: Pstring ' ', ' ',   \mkred{ --version of PADS used}
    sml    :: PstringSE(RE " "),  \mkred{ --version of SML used}
    msg    :: Maybe Pstringln     \mkred{ --optional message}
  \}
\mbox{}
  \kw{type} IP_t = (Pint, '.', Pint, '.', Pint, '.', Pint)
|]
\end{code}
...
[forest|
  ...
  \kw{type} BinaryRO    = Binary        \kw{where} <| get_modes this_att ==  "-rw-r--r--" |>
  \kw{type} BinaryRX    = Binary        \kw{where} <| get_modes this_att ==  "-rwxr-xr-x" |>
  \kw{type} TextRX      = Text          \kw{where} <| get_modes this_att ==  "-rwxr-xr-x" |>
  \kw{type} TextRO      = Text          \kw{where} <| get_modes this_att ==  "-rw-r--r--" |>
\mbox{}  
...
\mkred{\{- Collection of files named by sources containing actual data.  -\}}
 \kw{type} DataSource_d(sources :: [String]) =  [ s :: Text | s <- sources ]
\mbox{}
...
\mkred{\{- Directory of optional links to source data files -\}}
 \kw{type} Data_d ((root,sources) :: (FilePath, [String])) = \kw{Directory} \{
     datareps  \kw{is }[s :: Maybe Text                            | s <- sources],
...
\mkred{\{- Directory that stores the generated output for data source named "source". -\}}
 \kw{type} Example_d (source :: String) = \kw{Directory} \{
   ...
   makefile       \kw{is }"GNUmakefile"                   :: Text,           \mkred{ --Makefile}
...
\mkred{\{- Top-level of PADS website. -\}}
 \kw{type} Website_d(config::FilePath)  = \kw{Directory} \{
  c               \kw{is }config               :: Config,       \mkred{ --Configuration file with locations of other components}
  static_content  \kw{is }<| gstatic_dst c  |> :: Static_d,     \mkred{ --Static web site content}
  dynamic_content \kw{is }<| gcgi_path c    |> :: Cgi_d,        \mkred{ --Dynamic web site content}
  scripts         \kw{is }<| gscript_path c |> :: Scripts_d,    \mkred{ --Shell scripts invoked by cgi to run learning system}
  admin_info      \kw{is }<| gstatic_dst c  |> :: Info_d,       \mkred{ --Administrative information about website}
  data_dir        \kw{is }<| (glearn_home c)++"/examples/data" |>    \mkred{ --Stock data files for website}
                                          :: DataSource_d <|(getSources admin_info)|>,     
  usr_data        \kw{is }<| gtmp_root c |>    :: Users_d <|(get_fullpath data_dir_md, admin_info)|>   \mkred{ --User info}
 \}
|]
\end{code}
...
\mkred{\{- Helper functions to convert a \cd{Config} entry to a \cd{FilePath} -\}}
