...
\begin{code}
\kw{data} PrincetonCS_1 = \kw{Directory}
...
\begin{code}
\kw{data} PrincetonCS_2 = \kw{Directory}
  \{ notes \kw{is} "README" :: Text
  , classof11 :: Class 11
  , classof12 :: Class 12
  , graduates :: Grads
...
\begin{code}
\kw{data} PrincetonCS_3 = \kw{Directory}
...
\kw{data} PrincetonCS (y::Integer) = \kw{Directory}
  ...
  \}
\end{code}
The brackets \cd{<|...|>} provide an escape so that we may use
...
\cd{Grads} directory from \figref{fig:student-file-example}:
...
type \cd{Forest_md} (\figref{fig:forest-md}).
...
any directory:
\begin{code}
\kw{data} Universal = \kw{Directory}
  \{ asc \kw{is} [ f :: Text
           | f <- \kw{matches} (GL "*"), 
