...
\reminder{should mention any ``contractiveness'' constraint here as well...}

Complex \ipads{} descriptions are built by gluing a collection of base types
...
next object.}  Similarly, \padskw{Puint16$\_$FW}{\tt (3)} is an 
...
\figref{fig:ipads-clf}, describes the
Common Web Log Format~\cite{wpp}, which 
web servers use to log the requests they receive.  
\figref{fig:ipads-clf-data} shows a couple of sample records.
...
% \Palt{} \{\vec {\nont{field}}\} \|
  ...
  \name{Fields} \meta{field}  \::= \var : \nont {pty}
    \\
  \mname{Base Type Constructor}{2} \meta{C}  \::= ... \\ \\
