...
\item $\trans[\pmu \ptyvar \ty,,] =$ \\
  ...
  {\trans[\ty,,] \iappi \ictup{\data,\off}} \sapp \ictup{\off',r,p} \\
  \end{array}$
...
The parsing semantics of a type is a function that takes a bit string
as input and produces a representation and parse descriptor as ouptut. 
\figref{fig:parser-types} specifies the \implang{} language types of the parsers generated from well-kinded \ddc{} types.  Parameterized \ddc{} types require their arguments before they can parse the input.

The semantics
of each type is shown in \figref{fig:ddc-sem}.  Each parsing function
takes a bit string and an offset into the string and returns a new
