...
type for each \ddc{} type. We can prove that (for well formed types)
all PD types share the same basic shape of a pair of header and body.
This common shape allows us to define functions that polymorphically
process PDs based on their headers. Each header records the number of
...
\ispty$.  Each body consists of subdescriptors corresponding to
subcomponents of the rep (if any) and any meta-data particular to the
type. For types with neither subcomponents nor special meta-data, we
