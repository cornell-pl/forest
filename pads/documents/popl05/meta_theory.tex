...
One of the most difficult, and perhaps most interesting, parts of our
work on developing the theory presented above was determining what
properties we wanted it to satisfy. What are the ``correct''
...
meta-theory of data description language has been, until now,
unchartered waters.

We present the following three properties as critical invariants of
our theory. We feel that the last property is of particular importance
and should hold, in some form, for any data description language.
\begin{itemize}
...
\item {\bf Parser Type Correctness}: For a \ddc{}-type $\ty$, the
  Rep and PD output by the parsing function of $\ty$ will
  have exactly the types specified by $\itsem[\ty]$ and
  $\itpdsem[\ty]$, respectively.
  
\item {\bf Parser Error Correlation}: For any Rep and PD output by a
  parsing function of the semantics, the errors reported in the PD
  will be correllated with the errors present in the Rep.
\end{itemize}

Before we present the formal statement of these properties, we specify
...
the properties. In essence, we assume that the properties we desire of
the rest of the calculus hold for the various facets of the base
types.
...
Note that by condition~\ref{cond:closed-op}, the base type parser must
be closed.
...
$\rctxt$, decscribed in the next section. 
...
Next we present the type correctness theorem, followed by a simpler
statement for closed types. Note that we need to add $\ptyc \rctxt$ to
$\ctxt$ in order to give types to any free function variables in
$\trans[\ty,,]$. We also apply $\rctxt$ to $\ty$ in order to close any
open references to recursive types before determining its parser type.
We don't we explicitly require that $\ctxt$ be well formed as it
follows from the fact that $\ty$ is well formed in $\ctxt$.

\begin{theorem}[Type Correctness]
\label{thm:type-correctness}
  If  $\wfd \ctxt \rctxt$ and
   $\ddck[\ty,{\rctxt;\ctxt},\gk,\mcon]$ then
  $\stsem[{\trans[\ty,,]},\cdot;{\ctxt,\ptyc \rctxt},
...
We now turn to the final property, that of parser error-correllation.
We begin by defining what it means for a Rep and PD to be correllated.
Informally, it means that the number of errors recorded in the PD is
at least as many as the number of errors that appear in the Rep. It
also ensures that semantic errors - that is, constraint violations -
are properly reported. Formally, we define two mutually recursive
relations $\corrkl \ty r p$ and $\corr \tyval r p$ that check whether
a Rep and PD are correllated at a given type. The first relation is
just a ``helper function'' that normalizes types before checking for
Rep and PD correllation. We define {\it type normalization} in
\figref{fig:revised-ddc-syntax}, with a revised syntax chart of the
\ddc{} and the reduction rules used to normalize types.
...
\]
  \caption{Revised \ddc{} Syntax and Reduction Rules}
...
\begin{definition}[Rep and PD Corellation Relation]
$\corr \ty r p$ iff exactly one of the following is true:
  ...
  \item $\ty = \pand {\ty_1} {\ty_2}$, $r = \ipair {r_1} {r_2}$ and $p =
    \ipair h {\ipair {p_1}{p_2}}$, 
    and $h.{nerr} = \mathsf{pos}(p_1.{nerr}+p_2.{nerr})$, 
    $\corrkl {\ty_1} {r_1} {p_1}$ and $\corrkl {\ty_2} {r_2} {p_2}$.
  \item $\ty = \pset x {\ty'} e$, $r = \iinld \ity {r'}$ and $p =
    \ipair h {p'}$, 
    and $h.{nerr} = \mathsf{pos}(p'.{nerr})$, $\corrkl {\ty'}{r'}{p'}$
    and $e[(r',p')/x] \kstepsto\itrue$.
  \item $\ty = \pset x {\ty'} e$, $r = \iinrd \ity {(\ierror {r'})}$
    and $p = \ipair h {p'}$,
    and $h.{nerr} = 1 + \mathsf{pos}(p'.{nerr})$,
  ...
  \item $\ty = \pseq {\ty_e}{\ty_s}{\pterm {e,\ty_t}}$, 
    ...
    \mathsf{pos}(p_i.{nerr})$, $\corrkl {\ty_e}
    {r_i} {p_i}$, (for $i=1 \ldots {len}$), and
    $h.{nerr} \geq \mathsf{pos}({neerr})$.
...
\begin{proof}
  By inspection of code. Array case is most complicated, in particular
  proving the clause $h.{nerr} \geq \mathsf{pos}({neerr})$. To do so,
  you must prove that $H_A$ maintains this invariant. The first case
  of the match is the hard one, as ${nerr}$ is 0 (if its $1$, then it
  must be greater than or equal to $\mathsf{pos}(n)$, for any $n$).
  First, as $h.{nerr} = 0$, so too must $neerr$. Next, note that in
  this first case, $h1.{nerr} = 0$. Now, the new value of ${neerr}$ is
  just the sum of the original ${neerr}$ and
  $\mathsf{pos}(h1.{nerr})$, that is, $0 + 0$.
\end{proof}

Definition~\ref{def:err-corr} specifies the exact property we require
of parsing functions. At base kind, we require that any Rep and PD
returned by the parser must be correllated. At higher kind, we require
that the function preserve the property of error correllation.
...
\end{definition}
¯
Now, we state that any well-formed type of base kind is error-correllated.
...
Finally, we present the error correllation theorem itself.
...
We conclude this section with a useful property of correllated Reps
and PDs. If the PD reports no errors, then there are no syntactic
errors in the Rep datastructure {\it at any level}. We formally define
error-free values next, followed by the statement of the property
itself.
...
\begin{lemma}
  If $\corrkl \ty r p$ and $p.nerr = 0$ then $\noerr r$.
\end{lemma}

\begin{proof}
  By induction on the structure of r. {\em Need to ensure that compute
