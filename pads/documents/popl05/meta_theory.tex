...
statement for closed types. In the conclusion of the former, as any
free recursive variables in $\ty$ will become free function variables
in $\trans[\ty,,]$, we need to extend $\ctxt$ with the types of those
variables. We do so using the function $\ptyc \rctxt$, defined in
\figref{fig:rec-parser-types}, which maps recursive variable contexts
$\rctxt$ to typing contexts $\ctxt$. We also apply $\rctxt$ to $\ty$
in order to close any open references to recursive types before
determining its parser type.  We don't explicitly require that $\ctxt$
