...
\caption{Selected Rules for Encoding \ipads{} in \ddc{}}
  \label{fig:encode-ipads}
\end{figure}
}
...
stream. A termination predicate cannot encode this check as
they \trversion{are limited to the in-memory representation and PD
  and} cannot perform lookahead. Therefore, we must use the terminator
type to look ahead for an element parse error. For this purpose, we
design a type which succeeds where $\ty$ fails and fails where $\ty$
succeeds:
{\small
\[
