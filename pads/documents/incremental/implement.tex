...
\subsection{Reducing errors}
In the algorithm presented in section \ref{sec:algo}, 
parsing a sync token results
in one of three parse nodes: {\tt Good}, {\tt Fail} and {\tt Recovered}. 
...
To handle such cases, we created a fourth type of node, {\tt Partial}, 
to indicate that the input is ``partially'' correct. 
During aggregation, such partial nodes in the parse
...
from an initial chunk of data using the original \learnpads{} algorithm. 
In addition, the {\tt update\_desc} function uses the 
original \learnpads{} algorithm to learn 
sub-descriptions from all the accumulated data at the various learn nodes 
in the aggregate structure. One of the rewriting rules used in 
the original \learnpads{}
merges adjacent constant strings together into one string. As a result, we
could end up with constant strings like ``$\backslash${\tt "GET} '' 
(see Fig \ref{fig:ai.p}), instead of ``$\backslash${\tt "}'', 
``{\tt GET}'' and `` ''. 
...
after skipping too many characters (as a factor of the number of characters in
...
In practice, {\tt parse} returns a list of
