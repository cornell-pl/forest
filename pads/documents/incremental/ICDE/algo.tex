...
the receiver already possesses the description \cd{D}.
...
rule is used if and only if \cd{MDL(D)} $\leq$ \cd{MDL(D')}.
Rewriting continues until no further rule can be used.
...
The old system contained many MDL-based rewriting rules such as rules
to flattening nested structs and unions, to remove degenerate cases and
empty nodes, and a number of others.
One important new rewriting is called {\em BlobFinding}. 
This rule takes a given sub-description \cd{D} and uses a heuristic to 
determine if it is overly verbose and needs to be simplified. 
...
to mark the termination of a string. This rule is tremendously helpful
in controlling the size and
complexity of the learned description.  Without it, descriptions sometimes
grow quite large and complex, data processing slows, and the algorithms
fail to scale.
