...
number lines and brief descriptions. Our experiments are
...
We are generally interested in two kinds of performance measures: 
\begin{enumerate}
...
of the learned description.  MDL score was the
score we use to refine a description during the local search refinement 
procedure. It provides a ``mechanical way'' to quantify both the
precision and the compactness of a description. A smaller MDL score is better.
However, a description with
good MDL score may not be so ideal from human perspective. 
As a more important and intuitive metric, 
we measure how different the learn description is 
from the ``ideal'' description, also known as the gold description, using
a {\em normalized edit distance} score:
\[normal\_dist(D) = \frac{edit\_dist(D, D_{gold})}{|D_{gold}|}\]
where $|D|$ denotes the total number of nodes in $D$.
Of course, there maybe
many ways to craft a gold description depending on the expert's 
knowledge of the given data source, but this subjective measure could give
us a hint on how readable and document-worthy the generated description is.
Finally, our system would not be very useful if the learned description 
does not {\em describe} the original data correctly. Therefore,
...
The rest of this section presents the results of four sets of experiments.
...
\tblref{tab:eventstream.current} and \tblref{tab:messages.sdb}.
...
for \cd{message.sdb} is learned with $N=16000$ and $M=400$. 
...
the performance. In \cd{eventstream.current}, both MDL and distance scores seem to be
going up (worse) as $N$ increases. But in \cd{messages.sdb}, the quality
of the description goes down and then up again as $N$ increases. To find out
why description quality was particularly bad when $N$ is 1000 and 2000 for
\cd{message.sdb}, we perturb the order of 1000th to 2000th record in the data
three times and re-run the algorithm on the perturbed data with $N$ set at
1000 or 2000 and $M$ fixed at 25. The result in \tblref{tab:perturb}
shows that such perturbations does not have significant effects on the learning
in this occasion.

To illustrate the quality of learned description and the difference with the
gold description, we show the gold description and the best
...
In this experiment, we evaluate how the algorithm scales with increasing data size.
For each of the small files, using $N$/$M$ that produces the best result, 
we record the total running time for learning 20\%, 40\%, up to 100\% of the data
file and the results are depicted in \figref{fig:scale}. The algorithm enjoys
...
fairly simply one due to \cd{BlobFinding}, the parsing the remaining data gives no error
and takes negligible amount of time. 
...
Our last experiment learns a description with $N=2000$
...
\tblref{tab:large}. In addition, \cd{Reparse time} is the time it takes
the \cd{parse} function in the algorithm to parse the entire data using the
learned description; \cd{PADS time} is the time it takes the generated \pads{}
...
learning algorithm can learn the format of a 30GB file in a few hours, which
previously takes days and weeks for a person to produce manually. Furthermore,
