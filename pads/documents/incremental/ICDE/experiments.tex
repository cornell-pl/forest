...
{\em large files} with 6 sources each larger than 1GB, 
and {\em small files} with 10 sources each under 1GB. 
...
conducted on an AT\&T cluster xxx.
...
vshkap2.log & 10.33 & 89662433	& Syslog format \\ \hline
...
eventstream.current & 0.50 & 1579920 &	Event streams on Cosmos \\ \hline
...
HALO\_have2impression.log.1 & 0.36 &  210034 & Server side impression records of
iPhone applications\\ \hline
smallrace.log & 0.0019 & 3000 &	Log file from an autonomous vehicle \\ \hline
\end{tabular}
\end{table*}

We are generally interested in two kinds of performance measures: 
{\em time to learn a description} and 
{\em quality of the learned description}. The time to learn can be further
...
the MDL score of the description, the edit distance between the learned
description and a gold description written by human expert, and the correctness
...
precision and the compactness of a description. However, a description with
good MDL score may not be so ideal from human perspective. To complement
the MDL score, we measure how far the learn description is from the ``ideal''
description, also known as the gold description. Of course there maybe
...
us a hint on how readable and document-worthy is the generated description.
Finally, our system would not be very useful if the learn description 
does not {\em describe} the original data correctly. Therefore,
the correctness measures the percentage of the original data that
can be parsed without any error using the learned description.


We ran five sets of experiments altogether.

\subsection{Union clustering or not?}
 
\subsection{Optimal initial batch size and incremental batch size}
