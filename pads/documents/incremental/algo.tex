...
records, the algorithm transforms \cd{D} to accomodate the places where
...
the data structures that the algorithm uses for descriptions \cd{D}, data
...
\caption{Data structures for descriptions, representations, and aggregates}
...
integers and strings that matches a regular expression. Synchronizing
...
\cd{Pstruct}s and \cd{Punion}s in \padsc{} descriptions.
An array has a element type which is described by \cd{D}, a separator
string $s$ that appears between array elements, and a
terminator string $t$. \cd{Option D} indicates \cd{D} is optional.

A term with type \cd{R} is a parse tree obtained from parsing some
data using a description \cd{D}.  Parsing a base type can result in a
string, an integer or an error.  In contrast, parsing a sync token
...
\cd{s} is not a substring in the current input, or \cd{Recovered s'},
meaning \cd{s} is not found at the current position in the input, but
...
Then for each data record $x$, we use the \cd{parse} function to
...
line contains an error for \cd{Pint} and some unparsable data ``{\tt
  abc}'', and the third line contains a \cd{Fail} node because the
sync token was missing.  \figref{fig:aggregate} shows the aggregation
