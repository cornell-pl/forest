...
and punctuation (\cd{Pother(}$a$\cd{)}) for some punctuation 
...
strings (\cd{PstringFW(}$i$\cd{)}) or string constants 
(\cd{PstringConst(}$s$\cd{)}).  
...
enumerations (\cd{enum \{s_1,...,s_k\}}) or options (\cd{option T}).  In
addition, later phases of the algorithm 
bind variables to the results of parsing 
base types ({\em i.e.}, 
\cd{x:b(p_1,...,p_k)} as opposed to \cd{b(p_1,...,p_k)})
...
(\cd{switch x of \{s_1 => T_1 ... s_k => T_k\}}), where the
data is described by \cd{T_1}, ..., \cd{T_k} depending on the value
associated with \cd{x}, be it \cd{s_1}, ..., or \cd{s_k}.
