...
remark: cvs version: $Revision: 1.5 $
...
the current \pads{}\ system unable to process certain kinds of
...
semantic information across tools.  Third, while we have already begun a
...
Here, {\tt Tphys} is the PADS type of the external or {\tt con}crete
...
with \pads{} parse descriptors and error representations correctly
and conveniently.
...
drag a small data file from each of the machine to a centralized repository once every
five minutes and presumably will continue to do so for eternity.  In the
centralized repository, the data is split into two new formats, each designed to accumulate
information about a certain aspect of the system.

The current \pads{} implementation is limited to processing a single data source.
We propose to extend our specification language to enable automatic generation of tools
that process multiple data sources, either on one local machine or distributed across
a wide area network.  Doing so will require investigation of the following features.

\begin{enumerate}
\item {\bf Multi-source Specifications}  
We propose to extend the PADS specification language with a new first-class
type constructor, which may appear anywhere within a specification.  As a first cut,
the type constructor will have the form \texttt{Pget \{T;e\}} where \texttt{e} specifies
...
spatial specifications \texttt{Pget}, we will strive for design which combines simplicity with
...
data the format in question.  As control traverses the ad hoc data,
...
in order to create a new tool, tool implementers will merely need to link their tool-specific
code against a given tool-independent traversal.  Our revised design will take advantage of
...
We have already reaped a number of important benefits from this calculus:
\begin{itemize}
...
from one language may be co-opted by another.
...
data errors.  We have proven DDC-produced parser satisfy this principle
...
What general criteria and principles will use in the design?  The
...
we have identified at intuitive level is that input and output
...
a {\em Galois connection}.  This would allow parsers and printers to
``improve'' data as they work.  The properties of {\em Gallois connections}
...
generates a collection of value-added tools for the particular format includingan XML converter, a statistical analyzer, a histogram generator and
...
these higher-level tools.  It would seem to doing so will require to devise
...
obtained semantics for these different tools.  We and do not yet know how to
construct the semantics but we believe we may be able to do so by 
extending our work to date on parsing.  However, this element will take 
substantial additional thinking and research.
...
In fact, several of \pads{}\ features have been motivated directly by
our investigation of Troyanskaya's data sources.
If this research is
...
runs into in the sciences.   No one we have talked to uses these tools for
ad hoc scientific data.
...
useful in processing ad hoc data nor do they good support for generating
...
automatically generate an XML converter, a query engine and a
histogram generator, for instance.  The fact that all of these tools, and in
the future more, can be generated automatically from PADS descriptions provides
scientists with great added value over more general libraries.
...
descriptions to build additional useful tools.  These languages have
no formal semantics.
...
XDTM~\cite{zhao+:sigmod,xdtm} uses XML Schema to describe the locations of a collection
of sources spread across a local file system or distributed
across a network of computers.
However, neither METS nor XDTM has no means of specifying the contents of files,
...
including the hierarchy such objects. 

Commercial database products do provide support for
