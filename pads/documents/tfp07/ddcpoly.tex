...
\figref{fig:ddc-syntax} summarizes the syntax of \ddc{}.  The syntax
...
%error.   
The simplest description is a base type $\pbase{e}$.  The base type's
...
we have proven two key metatheoretic results, one concerning {\em type 
correctness} and the other concerning {\em canonical forms}.  
%First, we show that
...
We discuss these and other issues in more detail in the companion 
technical report~\cite{fisher+:popl-sub-long}; below we present the 
final theorem on type correctness.

% Hence, to prove type correctness, we first need to characterize the shape 
% of
...
% }
% prove the theorem using a general induction
...
% \end{proof}
% }{
%   We discuss these and other issues in more detail in the companion technical report~\cite{fisher+:popl-sub-long}.  
%   Below we present just the final theorem on type correctness.
% }
...
applications.  
%The result is a normal type $\tyval$ and the
