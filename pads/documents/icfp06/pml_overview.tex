...
The type \cd{Source} (Line~320) describes a complete \dibbler{} data
...
The type \cd{Orders} (Line~30) uses the list type
constructor \cd{Plist} to describe a homogenous sequence of
values in a data source.  The \cd{Plist} constructor 
...
and the type of the list terminator.~\footnote{The \cd{Plist} 
constructor itself is not built-in, but instead 
is defined as a recursive, polymorphic datatype.} 
In this example, the type \cd{Orders} is a list 
of \cd{Order} elements, separated by an
end-of-record \cd{Peor} marker~\footnote{%
...
and terminated by \cd{Peof}, a 
type that describes the special \emph{end-of-file marker}.  
Similarly, the \cd{Events} type (Line~27)
denotes a sequence of \cd{Event} values separated by vertical bars
...
example, the \cd{Event} type (Line~26) is a string terminated by a
...
The last key feature of \padsml{} is parameterized types.
Type parameterization makes descriptions more concise and allows
programmers to define convenient libraries of reusable components. For
example, the \cd{Plist} type that appears in both the \dibbler{} and
Newick formats is a recursive, parameterized datatype:
\begin{code}
\kw{pdatatype} (Elem, Sep, Term) Plist = 
  Elem * Sep * (Elem, Sep, Term) Plist
| Term\end{code}
This type is so useful that it is provided by default.

The description in Figure~\ref{fig:darkstar-ml} illustrates types
parameterized by both types and values.  This description specifies the format of
alarm data recorded by a network-link monitor used by the \darkstar{}
project at AT\&T.  Sample \darkstar{} data is in \figref{fig:darkstar-records1}.
...
definition of the type \cd{Pnvp} (Line~2) takes both type and value
...
definition of \cd{Pnvp}, there is one type parameter named \cd{Alpha}
and one value parameter named \cd{p}.  Informally, \cd{Alpha Pnvp(p)}
is a name-value pair where the value is described by \cd{Alpha} and
the name must satisfy the predicate \cd{p}.
...
example, the switched datatype \cd{Info} (Lines~18--21) chooses a
...
qualifier \cd{omit}.  In the \cd{Service} datatype (Lines~22--25),
