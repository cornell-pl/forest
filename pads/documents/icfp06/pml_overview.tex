...
dates (\cd{Pdate}), strings (\cd{Pstring}), and IP addresses
(\cd{Pip}).  Semantic conditions for such base types include checking
...
The last type definition, \cd{Source}, is intended to be a definition
of an entire \dibbler{} data source.  It uses the tuple type
constructor \cd{*}, which describes a possibly-heterogenous,
fixed-length sequence of objects in the data source. \cd{Source},
therefore, is defined as a \cd{Summary\_header} followed by
\cd{Orders}. The definition of \cd{Orders} is based on the list type
constructor, \cd{Plist}, which specifies a homogenous sequence of
objects in a data source.  \cd{Plist} -- defined in \padsml{} itself
...
our example, is the type \cd{Order}, described below. The second
parameter describes the syntactic separators that may be found between
elements of the list.  In this case \cd{Peor}, the
...
terminator for the list.  In this case, the terminator is \cd{Peof}, a
