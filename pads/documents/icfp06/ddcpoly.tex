...
In the process of studying these new functions, which we will call
{\em type abstractions} (as opposed to 
{\em value abstractions}, which are functions 
from values to types), we were able to simplify our overall semantics 
by making a couple of subtle technical changes.  This, for instance,
allowed us to eliminate the complicated ``contractiveness'' constraint from our
earlier work.

The main practical benefit of this calculus to us so far has been as
a guide for our implementation  --- before working through the formal details,
it was difficult to disentangle the important invariants from one another.
Once the calculus had been defined it took us approximately a week
to implement it in O'Caml using functors to implement the type 
abstractions.  We hope the calculus can serve as a guide for 
implementations of {\sc PADS} in other strongly typed 
functional languages.

In the remainder of this section we describe the
calculus to the extent possible given the space constraints, 
focusing primarily on the type abstractions.  The appendix
...
\figref{fig:ddc-syntax} presents the main syntactic elements of the \ddc.
...
The interpretation of a description with kind $\kty$ 
is a parser that maps data in an external form
into an internal, programmatic form.  The interpretation of a description with
kind $\ity \rightarrow \gk$ is a function that maps values
with host language type $\ity$ to the interpretation of
descriptions with kind $\gk$.
A description with kind $\kty \rightarrow \gk$ is a function mapping 
a parser to the interpretation of a description with kind $\gk$.

The basic descriptions include $\ptrue$ (the empty data fragment)
and $\pfalse$ (no data fragment).  
$\pbase{e}$ is a base description parameterized
by an expression $e$ from the host language, which, in our case
will be \fomega. \padsml's {\tt Pstring} is
an example of such a base type.

Any description in the language may be parameterized by a value using
value abstraction 
$\plam{\var}{\ity}{\ty}$.  We do not include the type $\sigma$ of
the variable --- the reader will have to reconstruct it from context.
...
not and matches the second type.  
...
Well-formedness of descriptions is specified by a kinding judgment with the form $\ddck[\ty,\pctxt;\ctxt,\kind,\mcon]$ where $\pctxt$ maps type variables
to kinds and $\ctxt$ maps host language value variables to host language 
types.  In our original work~\cite{fisher+:700ddl} these kinding rules
were somewhat unorthodox, but due to some technical simplifications to the
overall semantics,
they now hold almost no surprises.  See the appendix for further details.
...
and the span of data (location in the source) 
...
In \figref{fig:ddc-sem}, we specify the parsing semantics of the
...
understood. The interpretation of dependent sums parses the data with
