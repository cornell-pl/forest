...
\emph{Terminology: abstraction of types of values is ``value
  abstraction,'' and over types is ``type abstraction.''}
...
\ddc{}, an extension of \fomega.  We use this host language both to
write the expressions that can appear within \ddc{} itself and to
encode the parsing semantics of \ddc{}. We will discuss
in~\secref{sec:ddc-sem} our choice of \fomega in place of the
polymorphic lamda calculus used in \ddcold{}.
...
host-language type for the body.  This common shape enables us to
write polymorphic functions that treat parse descriptors in a generic
mannner. Indeed, the parsing semantics of types rely on this structure
heavily, as discussed in \secref{sec:parse-sem}. 
...
types.  However, type abstractions and applications are translated in
a subtly different manner. Type abstractions correspond to parse
decriptor-type constructors in \fomega much like they correspond to
representation-type constructors. The major difference is that PD-type
constructors are parameterized over the PD \emph{body} type, rather
than over the PD type itself. Correspondingly, type application
applies the PD interpretation to the ...
and type variables become ...

The reason for this difference can be appreciated only once we have
discussed the parsing semantics of type abstractions and applications.


% For example, a
% function to split a list of representation and parse-descriptor pairs
% into two lists, one for valid representations and one for invalid
% representations, could be written


% The basic goal is that each PD should have a header and body.
% However, this is only guaranteed to be true for well-formed types.
% Type abstractions parameterize over pd_bodies. Now, should have
% background to explain difference. In previous system, definition of
% type variables and recursive types was $blah$. Then, we proved that
% all well formed types had headers. However, the new system requires
% that pd variables be for the body, not the whole pd type. Otherwise,
% can't type type abstractions becuase depend on meta property that
% can't be expressed in the type. Forall a, s.t. a == pd_hdr * b'.


{\em Discuss change to rec. types and types variabls and explain new PD types here.}
...
The parsing semantics of a type $\tau$ with kind $\kty$ (\emph{Say the
previous in less technical terms. This is an important, general point,
not a technical detail.})is a
...
{\em Discuss translation of poly types here. Also, note somewhere that
we don'e use explicit type asbtraction and instantiation unless they
will clarify things.}
