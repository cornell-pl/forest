...
\ocaml{} module \cd{IntTriple}.)
\begin{code}\scriptsize
\kw{open} Pads
\kw{let} ((i1,i2,i3),pd) = parse_source IntTriple.Source.parse  ``input.dat''
...
average the values. The function \cd{parse_source} takes a parsing
...
In order to ensure that the data is valid, the program projects the
parse descriptor header from the parse descriptor \cd{pd} and checks
that the error code is set to \cd{Good}.  This error code is defined
in the \cd{Pads} module, and indicates that the data is syntactically
...
\figref{fig:ex-data-clean} gives such an example. The \cd{splitEntry}
function checks its argument \cd{entry} to determine whether it is
\cd{Good}. Because the function needs to access \cd{e}'s descriptor
