...
\begin{itemize}
\item {\bf Chunking:}  Dividing the data (either line-by-line, paragraph-by-paragraph,
file-by-file) into chunks with similar structure
\item {\bf Tokenization:}  The contents of every chunk of data must be divided up
into {\em tokens} such as numbers, words, punctuation, dates, times, 
ip addresses, phone numbers, etc.
\item {\bf Structure Discovery:}  After tokenization, the structure discovery phase uses
the tokenized data to produce a
{\em candidate format} --- a good ``guess'' as to what the optimal format may look like.
\item {\bf Format Scoring and Optimization:}  Once a candidate format has been produced,
it must be {\em scored} ({\em ie,} evaluated according to some metric).  A set of rewriting 
rules will be applied to the format for the purpose of transforming it into a 
format with a superior score.  The scoring-rewriting loop will be repeated until
no rewriting rule that increases the format score can be found.
\end{itemize}

We will assume that the chunking specification is given by the user.  However,
