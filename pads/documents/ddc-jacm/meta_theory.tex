...
of \ddc{} base types. Note that that the interface $\Iopty$ specifies the types of base-type parsers.
...
The first interesting lemma we prove about the \ddc\ 
is that evaluation commutes with semantic interpretation.
This property allows
reasoning about the semantics of \ddc{} functions directly in terms of
...
semantic domain. 
The premise of the lemma involves parser
evaluation because that is what is needed for later use. We posit without
proof that this lemma would also hold if the second
...
\begin{lemma}[Commutativity of Evaluation and Semantic Interpretation]
  ...
  there exists normal type $\tyval$ such that
  \begin{enumerate}
