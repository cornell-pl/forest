...
desire to hold of the rest of the calculus.  Note that that the interface $\Iopty$ specifies the types of base-type parsers.
...
\begin{lemma}[\ddc{} Inversion]
  All kinding rules are invertable. That is, given a proof of any rule's conclusion we have a proof of the rule's premises.
...
$\corr \tyval r p$ iff exactly one of the following is true:
  ...
  \item $\tyval = \psig x {\ty_1} {\ty_2}$ and $r =\ipair {r_1} {r_2}$ and $p =
    ...
    {\ty_1} {r_1} {p_1}$ and $\corrkl {\ty_2[(r_1,p_2)/x]} {r_2} {p_2}$.
