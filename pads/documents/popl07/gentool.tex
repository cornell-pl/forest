...
To leverage the many useful tools that exist for manipulating data in \xml{}, we built a translator that converts any data with a \padsml{} description into a canonical \xml{} format.  \figref{fig:gentool-xml-output} shows a sample portion of the output of this tool when run on the \dibbler{} data in \figref{fig:sample-data}(b).  
...
representation of the data rather than its original format, which may have delimiters \etc{} that are not present in the representation.
