...
We have used this framework to implement a variety of tools useful for processing ad hoc data, including a tool for generating statistical
overviews of the data, a data printer for debugging, and an XML
formatter.  We briefly describe these tools to illustrate the flexibility of the framework.

After receiving a new batch of data, analysts often want to understand the quality of the data quickly.  In particular, they might want 
to know the frequency of errors, or which fields are the most corrupted. For this purpose, an \emph{accumulator} tool provides a statistical summary of data
...
whose basic structure is a series of records of the same type. It
provides a summary for the record type based on viewing many records
of that type in the data source.  We implemented a simple version of this accumulator tool using the generic infrastructure.  It counts the number of errors and the total number of values for every element of a description.  A more complex accumulator program and a number of other statistical algorithms could be implemented in a similar manner.
...
for debugging descriptions. In contrast to the printer generated by the \padsml{} compiler, the output of both these tools corresponds to the in-memory
