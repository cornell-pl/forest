...
The \pads{} declarations in \figref{fig:switched-union-default} 
are similar to those in \figref{fig:switched-union} except that
...
%
\begin{figure}
\inputCode{code/union.switch.default}
...
such fields. 
There are six varieties of union fields, five borrowed from \Pstructs{}: 
...
the following \punion{} \cd{branches} 

\inputCode{code/union-void}

indicates that when the value of
the tag \cd{which} is \cd{0}, there is no more data associated with
the type \cd{branches} in the input source, while the tag value \cd{1}
indicates there is a \cd{Pint32} following.  
