...
\item[\cd{INIT\_scale}] is a \cd{Pint64} denoting scale factor for
  ...
  \pads{}. In this case, \cd{INIT\_scale} is needed. The default value
  is 100.       
...
\item[\cd{entry\_t\_toFloat}] is a function pointer, taking
  ...
  correctly by histograms. By default, only \cd{integer}, \cd{float} and \cd{char} in \pads{} have
  meaningful conversion functions to \cd{Pfloat64}. Users are allowed write their
  own conversion functions, and use \cd{entry_t_hist_setPara} function
...
\item[\cd{entry\_t\_fromFloat}] is a function pointer, taking
  ...
  default, only \cd{integer}, \cd{float} and \cd{char} in \pads{} have
  meaningful conversion functions from \cd{Pfloat64}. Users are allowed to write their
  own conversion functions, and use \cd{entry_t_hist_setPara} function
  to set them. If zero is assigned to this pointer, those default
  conversion functions will be used.
