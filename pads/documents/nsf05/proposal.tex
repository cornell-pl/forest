...
available.  There are vast amounts of useful data stored in
traditional databases and \xml formats, but there is just as much in
ad hoc formats.  For instance, in the networking and
telecommunications domain, we find ad hoc formats for web server
logs~\cite{wpp}, netflows capturing internet traffic~\cite{netflow},
log files characterizing IP backbone resource utilization, wire
formats for legacy telecommunication billing systems, and telephony
call detail data~\cite{hancock-toplas}, to name just a few.

%% Need to explain why this is important to CyberTrust here
...
serve as documentation and flexible enough to describe many ASCII, binary,
Cobol, and mixed data formats.  In addition, useful software
artifacts can be generated from the descriptions and this feature provides strong
...
source.  The result of a parse is a pair consisting of a canonical
...
\noindent
In addition, part of our mission will be to used \pads{} to provide
data processing tools for researchers in the natural sciences including
biology, chemistry and physics.  We have already developed a partnership with
Olga Troyanskaya in Princeton's Lewis-Sigler Institute for Integrative Genomics
and are studying her data processing problems.  We believe that we have a tremendous
opportunity to make a broad impact on the productivity of genomics researchers at Princeton
and elsewhere by supplying them with easy-to-use tools produced by our system.
To extend and support our relationship with the Genomics Institute,
we plan to develop interdisciplinary undergraduate research projects
on tool development.

Overall, our research combines novel language design, high-performance
systems engineering and theoretical analysis, all aimed at solving
important data processing problems.  However, before explaining the
details of our proposed research and its broader impacts, we give a
brief overview of the sorts of data sources that one can encounters in
practice and a brief summary of the current \pads{}\ system.
...
\subsubsection{Ad Hoc Data Sources}
...
\paragraph*{Sirius Provisioning Data: A Closer Look.}
...
a string literal that matches the characters \cd{no_li}.  Immediately after
...
\cd{dib_ramp_t} type in the CLF description indicates the possibility that
...
Our research agenda is divided into three broad subsections.
First, our experience with real-world data has revealed that
the current \pads{}\ system is unable to process many common
data formats.  We propose a number of extensions to \pads{} 
that will greatly improve its expressive power and enable
...
formalize the language and prove the critical \pads{} correctness
