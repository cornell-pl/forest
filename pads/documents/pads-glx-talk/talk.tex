...
\kw{Pstruct} http_clf_t \{
        clihost_t \blue{host};              /- IP address of client requesting service
   ' '; auth_id_t \blue{remoteID};          /- Remote identity; '-' means not obtained
   ' '; auth_id_t \blue{auth};              /- Name of authenticated user
   " [";
   Pdate(:']':) \blue{my_date};             /- Timestamp of request
   "] ";
   http_request_t \blue{request};           /- Request
   ' ';  Puint16_FW(:3:) \blue{response};   /- 3-digit response code
   ' ';  contentOpt_t \blue{contentLength}; /- Number of bytes in request response
\};
\end{alltt}}
