...
rightful position on disk. \jnf{Is this accurate?}If the manifest reports no
...
\jnf{Is this code right? Do does it need a ``do''?}
\begin{code}
let result = Stats \{ last = sites_mod ()
                   , topk = top 10\}
manifest <- generateManifest 
  ( result
  , defaultMd result "/var/log/coral/stats")
store manifest 
\end{code}
