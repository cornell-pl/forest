...
together form an instance of the \cd{Forest} type class.
%
...
\end{code}
Because \cd{Dat} is a comprehension, the \cd{rep} is a
...
\end{code}
%
\reviewer{How come the example CDN metadata includes values that look
  like strings but aren't in quotes?}
%
...
%
\reviewer{By "Forest uses lazy I/O operations" (page 7), do you mean
  "Haskell uses lazy I/O operations"?  Or does Forest also have its
  own I/O operations?}
%
...
\reviewer{- Forest doesn't solve (or purport to solve) the problem of
  "magic strings" like "/var/log/coral/stats" strewn about.  I'm
  wondering if the authors' experience with semi-structured data
  processing has led to any insight into good language-based solutions
  to this problem.}
