...
detected during the storing process as well as the sequence
...
there.  Details concerning the semantics of storing, 
...
useful functions on pairs of Forest representation and metadata
structures. These functions include the load and manifest
functions as well as other functions for constructing defaults
(\cd{f_default}) and setting owners, permissions, and file locations
(\cd{setLoc}).  
...
As a simple example, consider the \cd{Dat} description in
...
\end{code}
Because \cd{Dat} is a comprehension, both \cd{rep} and \cd{md} are
...
Given these analyses, a programmer can store  results
in the \cd{stats} directory using the following code.

\begin{code}
manifest <- stats_manifest 
  ( Stats \{last = sites_mod (), topk = top 10\}
  , stats_setLoc f_default "/var/log/coral/stats")
store manifest 
\end{code}
