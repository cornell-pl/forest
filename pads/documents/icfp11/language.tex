...
data stored in ad hoc data files or in in-memory data structures.
Consequently, we
adopted a type-based metaphor for the \forest{} language: base
types denote various kinds of files while type constructors describe 
various kinds of file structures.  This design mirrors traditional
types, so we based our design on an underlying type theory, 
described in \secref{sec:theory}.   

We want to allow programers to move seamlessly between on-disk and
in-memory representations of \filestore{} data, so we implemented
\forest{} as an embedded domain-specific language.  This choice allows
programmers to manipulate \filestore{} data almost as if it were
simply stored in data structures in the host language.  We chose
Haskell as the host language because its support for type-directed
programming is useful for building tools that work over any \forest{}
description.  As a bonus, Haskell's quasiquoting mechanism proved a
useful way to implement \forest{}.  It enabled a smooth integration of
the two languages while allowing us to control the syntax of \forest{}.  
To foster this integration, we strove to have \forest{}
declarations look like extensions of the corresponding Haskell
declarations so that programmers would have an intuitive understanding
of the generated Haskell structures.  Within this framework, we
designed constructs that were sufficiently expressive to capture all
the \filestore{}s we had seen in practice.

Since \forest{} is a domain-specific language embedded within Haskell
using the quasiquote mechanism~\cite{Mainland:quasi}, \forest{}
declarations can be embedded in simple Haskell files in any context
expecting a declaration. To introduce new \forest{} declarations,
the programmer simply opens the \forest{} sublanguage using
Haskell's notation for quasiquotes:
\begin{code}
...
\begin{enumerate}
\item It describes a fragment of the file system.
\item It specifies the structure of the in-memory {\em representation}
    that will be constructed when the fragment is (lazily) loaded into a Haskell program. 
\item It specifies the structure of the in-memory {\em metadata}
    that will be generated when the fragment loaded.
\end{enumerate}
Such metadata includes
error information (missing file, insufficient
permissions, \etc{})  as well as file system attributes (owner, size,
\etc{}).
...
In the remainder of this section, we discuss the types and type
constructors that constitute the \forest{} language.  We use our
running examples to illustrate the various constructs.  
A note on terminology: we use the term
...
individual files: \cd{Text} for ASCII files, \cd{Binary} for binary files, and
...
\figref{fig:student-pic}.  We borrow Haskell's notation for set
