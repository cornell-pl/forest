...
\forest{} as were developing it, and to prove key theorems like the
...
file system $F$ is finite map from paths to pairs $(\att,T)$ of
...
parameterized over a collection of constants $\Sk$. In examples, we
...
function $\eval{e}{\Env}{\tau}$ that evaluates $e$ in $\Env$, yielding
...
fragment of the file system as $s$ but checks whether current path is
...
\SPair{c\,}{\,(\SPath{\textrm{"}\mathrm{c.txt}\textrm{"}}{\mathrm{C}})\;}
           {\SPair{d\,}{(\SPath{\textrm{"}\mathrm{d.txt}\textrm{"}}{\mathrm{D}\; c})\;}
                       {(\SPath{\textrm{"}\mathrm{e.csv}\textrm{"}}{\mathrm{E}\; d})}} 
\]
%
...
Figure~\ref{fig:csemantics}. The types for constants $\Sk$, are simply
read off from the $\tau_1$ annotation. For other specifications, they
...
structured datatypes containing a boolean value each level of
...
a modified file system $F'$ and a validator $\phi'$, which is a
predicate on file systems that tests for inconsistencies during
storing. As with load functions, this judgment may be read as a total
...
suppose that invoke $s$'s load function with $\Env$, $F$, $r$ where
...
\]
%
Finally, it conjoins the two validators $\phi_1'$ and $\phi_2$'. The
...
first store satisfies the validator. This theorem ensures that the
