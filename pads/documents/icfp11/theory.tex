...
\newcommand{\storek}[4]{\fn{\storekname{#1}}{#2,#3,#4}}
...
\begin{array}{rcll}
v,d  &=& \loadk{\Env}{F}{r}\\
F',\phi' &=& \storek{\Env}{F}{r}{v,d}\\
\end{array}
\]
By assumptions about the behavior of $\loadkname{\Sk}$ and
$\storekname{\Sk}$, we have $F' = F$ and $\phi'(F')$, which finishes
the case.
...
\begin{array}{rcll}
F',\phi &=& \storek{\Env}{F}{r}{v,d}\\
v',d'  &=& \loadk{\Env}{(G_1 \FAppend F' \FAppend G_2)}{rs}\\
\end{array}
\]
By assumptions about the behavior of $\loadkname{\Sk}$ and
$\storekname{\Sk}$, we have $v' = v$ and $\valid{d} \iff \valid{d'}$,
