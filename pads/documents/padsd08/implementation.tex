...
and 
(2) \cd{fmlc}, the compiler for feed declarations (.fml files). 
Both compilers convert their sources into \ocaml{}, which is compiled
...
although the meta-data in the implementation is richer, 
adding data arrival times and more detailed error codes for 
HTTP errors, late item arrivals, parse errors, \etc{}
...
%(known as the {\em rep})
using the \padsml{} parser, or synthesizes its data by calling a
generator function.  Using error conditions, location, scheduled
and arrival times, the worker generates the appropriate metadata,
...
\padsd{} off-the-shelf tool suite using our generic tool framework.  Many of
the tools rely upon format-specific libraries generated 
from format specifications.  For instance, the feed
selector relies upon a data selector generated from a \padsml{}
description. 
%For instance, the
%feed selector relies upon a data selector generated by type-directed
%compilation of the \padsml{} description.
Other tools also depend upon auxiliary libraries.  For instance, the
...
the average network latency to fetch a data item, 
the average system latency to return a data item after receiving it
from the network
...
\figref{fig:throughput} shows the average throughtput
...
An anomaly occurred at 400 nodes, as a number nodes were unreachable
...
small, showing that the \padsd{} runtime system adds
very little overhead to the inevitable network fetching
cost. It is also worth noting that the network latency is
almost linear in the number of nodes. The experiments show that
the system can fetch from 800~nodes and archive the
resulting data in under 40~seconds, well under the
5 minute turnaround time currently supported by CoMon. 
Taken together, these results suggest that \padsd{} is  
capable of supporting PlanetLab-scale monitoring applications. 
...
preprocessor.   
Hence, a natural question is whether the system would be better
...
in modules for name-space management and functors usage, 
(c) automatically filling in defaults for values omitted from config 
files, and (d) generating complete, stand-alone executables from
declarative descriptions and config files. 
...
\textbf{\textit{Generic programming.}} \ocaml{} has no direct support for
...
others to embed our abstractions directly in language
