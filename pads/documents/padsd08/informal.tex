...
\cd{Data}. The description also defines a helper function \cd{isNode},
...
the list of hosts to remove comments using the \cd{isNode} function
...
the earlier CoMon example, the \cd{stats} feed in \figref{fig:pulse}
monitors a collection of machines described in a configuration file.
Before we can explain the \cd{stats} feed itself, we need to first
explain some auxiliary feeds.  

The \cd{rawHostList} description has the same form as the \cd{nodes}
feed we saw earlier, except it draws the data from a local file just
once a day.  We then use a \textit{feed comprehension} to select only
lists of host names not containing any errors, forming the clean feed
\cd{hostLists}.  
...
first uses the Unix command \cd{ping} to collect network statistics
about the route to \cd{h} while the second performs a remote shell
invocation using \cd{ssh} to find out statistics about how long the
...
\cd{hostLists} feed, we filter the list using the \cd{isNode} function
to remove ...
...
\kw{feed} stats =
  \kw{foreach} hl \kw{in} hostLists \kw{update}
  [ gen_stats (get_host h) | 
          h <- List.filter Hosts.isNode hl ]
\end{code}
\vskip -2ex
\caption{Simplified version of \vizGems{} feed.}
\label{fig:pulse2}
