...
specification processes the \cd{nodelist} to handle errors and strip out
comment fields.  The \cd{current} function checks if the \cd{nodelist}
is \cd{None}, signalling a fetching error, in which case it uses
the most recently cached list of nodes instead.  The \kw{source}
...
\cd{makeURL}.  The \kw{schedule} for this CoMon feed is \cd{once}
...
of CoMon statistics to the overall \cd{comon} feed. As before, the
format of data fetched from each node matches the description
\cd{Comon\_format.Source}.
...
\kw{feed} hostList =  
  \kw{base} \{|
   \kw{sources}  = \kw{all} [("file:///" ^ Sys.getcwd ()  ^ "/machine_list")];
   \kw{schedule} = every 1 day;
   \kw{format}   = Hosts.Source;
 |\}

\kw{let} mk_host (Hosts.Data h) = h

\kw{feed} hosts = {| mk_host n | n <- (flatten hostList), Hosts.isNode n |}

\kw{feed} stats =
  \kw{foreach} h in hosts \kw{create}
  (
   \kw{base} \{| 
      \kw{sources} = proc ("ping -c 1 " ^ h);   
      \kw{format} = Ping.Source;  
      \kw{schedule} = every 5 min, timeout 1 min; |\},
   \kw{base} \{| 
      \kw{sources}  = proc ("ssh " ^ h ^ " uptime");  
      \kw{format}   = Unix_formats.Line;  
      \kw{schedule} = every 5 min, timeout 1 min; |\}
  ) 
\end{code}
\vskip -2ex
