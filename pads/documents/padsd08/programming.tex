...
\kw{let} load_tbl = \textit{\textbf{Feed.fold_p}} update_l empty_tbl loads
\kw{in} print_top_with_loc 10 load_tbl
\end{codebox}
  \caption{Revised code fragment: with provenance metadata. }
...
\figref{fig:sample-loads} can only return the lowest
average loads, not the names of the associated machines.
In such situations, provenance data is essential.  In
\figref{fig:sample-loads-prov}, we replace the last two lines of
...
function. We also sketch an \cd{update_l} function which uses
metadata to add the location and the load to the table. 
Function \cd{print\_top\_with\_loc} \cd{10} prints the ten lowest average loads
with their locations.
Notice the use of the lower level \cd{IData} and
\cd{Meta} interfaces to facilitate management of both data and 
...
%\begin{code}
\cd{feed\_create : 'a prefeed -> 'a feed}
%\end{code} 
...
proposed~\cite{padsml-padl,hinz:icfp04,weirich:encodingtypecase,yang:icfp98}. We have 
chosen to represent our AST using a variant of the Mogensen-Scott
encoding  which exploits higher-order abstract syntax 
