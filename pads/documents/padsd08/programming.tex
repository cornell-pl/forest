...
\end{codebox}
  \caption{Code fragment for finding least loaded \planetlab nodes.}
...
\kw{let} update tbl adata =
  ...
  \kw{match} meta, data \kw{with} 
    (h, Some basemeta), Some load ->
      ...
      (* ... Update tbl with (location, data) pair *)
  | _ -> tbl \textit{ (* no change to tbl *)} \kw{in}
\kw{let} load_tbl = \textit{\textbf{Feed.fold_p}} update empty_tbl loads
...
\figref{fig:sample-loads-prov}, to exploit metadata. First, we sketch an \cd{update} function that uses
metadata to associate a location with every load in the table. It relies on the \cd{Meta} module, which we provide to facilitate management of metadata from the feed. Next, we show a call to the lower-level fold,
\cd{fold_wm} (fold with meta), which passes the data with its metadata to the folding
function.  Last, the call \cd{print_top_with_loc 10} prints the ten lowest average loads
...
proposed~\cite{padsml-padl,xi:icfp03,weirich:encodingtypecase,yang:icfp98}. We have 
