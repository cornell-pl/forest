...
Our current tool suite includes the following components:

\begin{itemize}
\item {\bf A fetching engine} that collects distributed data on the specified 
...
\item {\bf An RRD database loader} that takes the data and extracts specified 
...
by using Princeton's CoMon~\cite{comon} system as an example.
...
machines or user programs.  Readers interested in further examples
and a semantics for the language should refer to our technical 
report~\cite{zhu+:padsdtr}.
