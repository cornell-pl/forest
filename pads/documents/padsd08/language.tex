...
of our language design process.  
The semantics helps communicate our ideas precisely and explore 
the nuances of design decisions. 
Moreover, the semantics provides users with a tool to
...
Having covered these preliminaries, 
...
(\kw{foreach} $x$ in $\feed_1$ \kw{create} $\feed_2$) 
...
\figref{fig:semantics}.  The principal semantic functions are
...
\paragraph*{Dependency Correctness.}  
%The {\em Dependency Correctness}
%theorem states that feed elements depend exclusively on the times and
%locations mentioned in their dependency set.  
