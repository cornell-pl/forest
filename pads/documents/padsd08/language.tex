...
$\Gamma \turn \feed : \tau$, where $\Gamma$ is a context that maps
variables to types.  This judgment depends upon an auxiliary judgment
defining the well-typed host language expressions, which is not shown.

Many of the elements of the calculus should look familiar from
discussions in previous sections.  We touch on some highlights here.
The empty feed ($\emptyset$) contains no elements and serves as
...
$e\; v$ is $\boolf$.  Together computed feeds and filter feeds
model the feed comprehensions shown in
Section~\ref{sec:informal}. Let feeds $\letfeed{x}{e_1}{\feed_2}$
...
but combining window and schedule together is slightly more
pleasant for the programmer.  These minor syntactic differences
have no semantic importance.} Notice that the preprocessor
...
errors.  We do not model the variety of different error codes that
...
that are {\em scheduled} at the same time (regardless of when those elements
...
The final elements of the calculus are the $\mathtt{foreach}$ feeds.
...
the unreliability present in the networking environment.  The
...
schedule time and location to a rival time and string option.
Intuitively, the universe models network unreliability.
...
$\semantics{\feed}{\environment}{\universe}$
is a set of metadata/data pairs.  The metadata 
...
time can be used to take the set of metadata/data pairs that
defines the semantics of a feed and serialize it as a stream.
...
The semantics also depends upon an auxiliary semantics function
for the host language:  When $e$ is a host language expression
with free variables in the domain of $\environment$ then
$\esemantics{e}{\environment}$ is the denotation of the value.

Figure~\ref{fig:semantics} presents the semantic definitions themselves.
...
The semantics of {\tt all} and {\tt any} feeds are somewhat complex
...
scheduled time plus window ($\esemantics{e_5}{\environment}$).  If
the arrival time exceeds its window, {\tt None} is returned. Otherwise,
its data argument ($x_s$, which may, due to other networking errors be 
{\tt None}) is returned.  The {\tt timeout} is used in definition of an 
...
with defined by $\esemantics{e_1}{\environment} \; (\universe'(\loc,\atime))$
where $e_1$ is the \pads{}-generated parser.
The semantics of an {\tt any} feed are defined by selecting one of the
good (non-error) elements of the corresponding {\tt all} feed at the same time,
if any exist.  If there are no good elements in 
 the corresponding {\tt all} feed, the {\tt any} feed will contain a
...
Defining pairs requires care is taken with subtle aspects of timing.
...
not share common schedule time.  We adopt the later time ({\em i.e.,}
...
scheduled time for the item under consideration in $feed_2$
should fall between the scheduled times for $v_1$ and $v_1$
...
As a sanity check on our semantic definitions, one can easily prove
that the values contained in each feed may be given the simple types
required by our feed typing rules.

\begin {theorem} theorem about feeds goes here...
\end{theorem}
