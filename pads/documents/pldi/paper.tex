...
We attempted to write the equivalent \perl{} programs in as efficient a manner possible, given the specified tasks.  The \perl{} vetter uses the built-in \cd{split} operator to produce an in-memory list of the pipe-separated fields of the data source.  The \perl{} selection program uses \perl{}'s regular expression pattern matcher to find lines with the desired state in any 
position after the 13th field.  \figref{figure:perl-re} shows the relevent regular expression, looking for orders going through state ``10.''
...
The \pads{} vetter was more than twice as fast as the \perl{} vetter, while the \perl{} selection program outperformed the \pads{} selection program after the initial run, which presumably loaded some \perl{} libraries that remained resident for the successive runs.
