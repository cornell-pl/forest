...
analyses.  Today, people often hand craft parsers in \C{} or
...
provisioning data format.  In \pads{} descriptions, types are declared
...
Analysts working with ad hoc data also like to query their data.  The
\dibbler{} analyst wanted to answer questions such as ``Select all
...
time required to go from a particular state to another particular
state''.

\scream{Mary: left off here}

The \pads{} libraries and tools greatly simplify many common tasks
such as parsing, detecting errors, and summarizing data.  \pads{}
however 

We wanted to support declarative querying over ad hoc sources, but we
didn't want to invent an entirely new query language, which led us to
examine existing languages.  Because XQuery is designed to manipulate
semi-structured data, its expressiveness matched our data sources
well.  We were able to code all the \dibbler{}-related queries in
XQuery.  For example, the XQuery
\begin{small}
\begin{code}
{ $pads/pads:Psource/orders/pads:elt[events/pads:elt[1]
    [tstamp >= xs:dateTime("2004-10-01:00:00:00") and 
     tstamp < xs:date("2004-11-01:00:00:00") ]] }
\end{code}
\end{small}
asks for all orders that started in October, 2004. 
