...
\label{section:intro}
Although there is a lot of data in ``well-behaved'' formats such
...
\end{center}
\end{figure}
The lack of standard tools for processing such data forces
...
questions as quickly as possible?  As is typical, the accompanying
documentation is incomplete and out of date, so developing a parser is
a non-trivial task, requiring a lot of preliminary data exploration
with additional hand-written programs. Eventually, she hand-codes a
parser that reflects her hard-won understanding of the data, usually
in \C{} or \perl{}.  Pressed for time, she interleaves code to compute
the answers to the supplied questions with the parser.  As soon as the
answers are computed, she gets a new data source or a new set of
questions to answer.

Through her heroic efforts, the data analyst was able to answer
the necessary questions, but the approach is deficient in many
respects.  The analyst's hard-won understanding of the data ended up
embedded in a hand-written parser, where it is unlikely anyone else
will be able to benefit from her understanding.  The parser is likely
brittle with respect to changes in the input sources.  Figuring out
which \$4's should be \$5's, for example, in a \perl{} program when a
new column is added to the data can be quite tricky.  Ad hoc data
sources tend to have errors in them, and hand-written parser likely to
either ignore errors entirely (which can lead to errors corrupting
down-stream data) or be swamped with error checking code.  In
addition, during both data exploration and to answer the specified
questions, the analyst had to code how to compute the queries rather
than being able to express the queries in a declarative fashion.

Of course, many of these pitfalls can be avoided with careful design
and extra time, but this takes time, which the analyst does not have.  
