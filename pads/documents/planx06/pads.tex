...
In \figref{figure:dibbler}, the type declaration for \cd{order\_t}
illustrates a simple \kw{Pstruct}.  It starts with an order header
...
the \cd{dib\_ramp\_t} type indicates
...
The \cd{order_header_t} type contains several anonymous uses of the
...
data all with the same type.  The \cd{event\_seq\_t} declaration uses a
...
a time in a loop.  The parsing code generated for \kw{Parray}s allows
