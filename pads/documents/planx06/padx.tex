...
\padx{} \condm{} to decide when and how to load, \ie{} read and parse,
...
which is required by the \Galax{} \absdm{}.
...
simplest strategy to implement genuine random access, but because of
the overhead of \pads{} meta-data, it incurs a high memory cost and is
only feasible for smaller data sources.  

The \emph{on-demand, random-access} strategy loads \pads{} data when
\Galax{} attempts to access it.  This strategy allocates a fixed-size
buffer for parsed data.  Currently, this strategy loads and expels
values whose type is annotated with \kw{Precord}.  This heuristic
works well in practice, because many \pads{} sources have a header, a
large \kw{Precord} \kw{Parray}, and a trailer.  \padx{} bulk loads all
data before the array and on-demand loads each item in the array.  A
small amount of meta-data is preserved for each record that is
expelled from the buffer, so that if the virtual node containing that
data can be reconstructed on subsequent accesses.

The \emph{on-demand, sequential} strategy is an optimization of the
on-demand, random-access strategy.
