...
module Language.Forest.IO.DeltaLoading where
...
doLoadDeltaFile :: (ForestInput fs FSThunk Inside,MData NoCtx (ForestI fs) pads,MData NoCtx (ForestI fs) md,Eq pads,Eq md,MData NoCtx (ForestO fs) pads,FSRep fs,Pads pads md,MData NoCtx (ForestO fs) md) =>
...
doLoadDeltaFile mpath path' oldtree df tree' ((rep_thunk::ForestFSThunkI fs pads,md_thunk),getMD) = do
	...
	case (path == path',df) of
		...
		(True,Just (FSTreeChg _ _)) -> debug "constant0 attrs" $ do
			fsmodify [tree'] md_thunk $ \(_,md) -> getMD path' tree' >>= \fmd' -> return (fmd',md)
			return (Id,Delta)
		otherwise -> debug "constant0 changed" $ do
			...
			fsoverwrite [tree'] md_thunk $ fsforce md_thunk'
			return (Delta,Delta)

doLoadDeltaFile1 :: (ForestInput fs FSThunk Inside,MData NoCtx (ForestI fs) pads,MData NoCtx (ForestI fs) md,ForestMD fs md,Eq arg,Typeable arg,Eq pads,Eq md,MData NoCtx (ForestO fs) pads,MData NoCtx (ForestO fs) md,FSRep fs,Pads1 arg pads md)
...
doLoadDeltaArchive ext mpath path' oldtree df tree' ((rep_thunk,md_thunk),getMD) load loadD = do
	...
	case (path == path',df) of
		...
		(True,Just (FSTreeChg _ _)) -> do
			fsmodify [tree'] md_thunk $ \(_,imd') -> do
				fmd' <- getMD path' tree'
...
doLoadDeltaSymLink :: (ForestInput fs FSThunk Inside,FSRep fs)
			...
			if isUnevaluated -- just an optimization to preserve the evaluation status of the thunk
				then do
					modify_errors_under md $ \err ->
						force cond_thunk >>= \cond -> if cond then return err else return $ updateForestErr err [constraintViolationForestErr]
					...
					forceOutside cond_thunk >>= \cond -> if cond
						then debug ("doLoadDeltaConstraintReturn2: ") $ return (direp,mapSValueDelta dimd)
							...
							modify_errors md $ \err -> return $ updateForestErr err [constraintViolationForestErr]
...
doLoadDeltaArgs :: (ForestArgs fs args,Eq rep,Eq md,FSRep fs) =>
...
doLoadDeltaDirectory :: (MData NoCtx (ForestI fs) md,MData NoCtx (ForestI fs) rep,Eq rep,Eq md,FSRep fs,MData NoCtx (ForestO fs) rep,MData NoCtx (ForestO fs) md) =>
...
doLoadDeltaDirectory mpath ((rep_thunk,md_thunk),getMD) path' oldtree df tree' collectMDErrors load loadD = do
	...
	exists' <- doesDirectoryExistInTree path' tree'
	debug ("doLoadDeltaDirectory: " ++ show (path,exists,path',exists')) $ case (exists,exists') of
		(False,False) -> case (path == path',isEmptyTopFSTreeDeltaNodeMay df) of
			...
			otherwise -> do
				fsmodify [tree'] md_thunk $ \(_,md1) -> do
					fmd' <- defaultForest_mdWithErrors $ missingDirForestErr path'
					return (fmd',md1)
				return (Id,Delta)
		(True,False) -> do
			fsoverwrite [tree'] rep_thunk $ mdefault
			fsmodify [tree'] md_thunk $ \(_,md1) -> do
				fmd' <- defaultForest_mdWithErrors $ missingDirForestErr path'
				return (fmd',md1)
			return (Delta,Delta)
		(True,True) -> do -- this is the ONLY place where non-stable @SValueDelta@s appear!
			irep <- inside $ fsforce rep_thunk
			md@(fmd,imd) <- inside $ fsforce md_thunk
			(direp,dimd) <- loadD ((irep,imd),getMD) -- load recursively
			drep <- case direp of
				StableVD d -> return $ liftSValueDelta d
				Modify f -> do
					let irep' = applyNSValueDelta direp irep
					fsset rep_thunk (f irep') >> return Delta -- modify the directory thunk, since underlying non-stable changes did not occur inside a modifiable
		...
		(False,True) -> do -- load from scratch
			...
			fsoverwrite [tree'] rep_thunk loadRep
			fsoverwrite [tree'] md_thunk loadMd
			return (Delta,Delta)
	...
	case (exists,exists') of
		(False,False) -> do
			case (path == path',isEmptyTopFSTreeDeltaNodeMay df) of
				(True,True) -> return (Id,Id)
				otherwise -> do
					fsmodify [tree'] md_thunk $ \(_,md1) -> cleanForestMDwithFile path' >>= \fmd' -> return (fmd',md1)
					return (Id,Delta)
		(True,False) -> do
			fsoverwrite [tree'] rep_thunk mdefault
			fsoverwrite [tree'] md_thunk $ cleanForestMDwithFile path' >>= \fmd' -> return (fmd',Nothing)
			return (Delta,Delta)
		(True,True) -> do
			rep <- inside $ fsforce rep_thunk
			md@(fmd,imd) <- inside $ fsforce md_thunk
			repimd <- mergeJustDefault (rep,imd)
			(direp,dimd) <- loadD (repimd,getMD) -- load recursively
			fsoverwrite [tree'] md_thunk $ do
				fmd' <- maybe mdefault get_fmd_header imd
...
pickFileDelta :: FSRep fs => ForestI fs FilePath -> FilePath -> [FileName] -> FSTree fs -> FSTreeDeltaNodeMay -> FSTree fs -> ForestI fs FileName
...
-- changes the delta
doLoadDeltaNewPath :: (FSRep fs) => ForestI fs FilePath -> FilePath -> FilePath -> FSTree fs -> FSTreeDeltaNodeMay -> FSTree fs
	-> (ForestI fs FilePath -> FilePath -> FSTreeDeltaNodeMay -> ForestO fs (SValueDelta rep,SValueDelta md))
	-> ForestO fs (SValueDelta rep,SValueDelta md)
doLoadDeltaNewPath moldpath path' file' oldtree df tree' loadD = debug ("doLoadDeltaNewPath: "++show (path',file',df)) $ do
	newpath' <- stepPathInTree tree' path' file'
...
doLoadDeltaSimple :: (Eq rep',Eq md',Eq irep,ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,MData NoCtx (ForestO fs) rep',md' ~ ForestFSThunkI fs imd',Eq imd',ForestMD fs imd') =>
...
doLoadDeltaSimple lensRep lensMd mpath path' matchingM oldtree df tree' (repmd@(rep,md),getMD) load loadD = debug ("doLoadDeltaSimple: "++show (path')) $ do
	...
	let irepmd@(irep,imd) = (get lensRep >< get lensMd) repmd
...
doLoadDeltaSimpleWithConstraint :: (ForestOutput fs ICThunk Inside,Eq rep',Eq md',Eq irep,ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,MData NoCtx (ForestO fs) rep',md' ~ ForestFSThunkI fs imd',Eq imd',ForestMD fs imd') =>
...
doLoadDeltaSimpleWithConstraint lensRep lensMd mpath path' matchingM oldtree df tree' ((rep,md),getMD) pred load loadD = debug ("doLoadDeltaSimpleWithConstraint: "++show (path')) $ do
	matching <- inside $ matchingM
	let irepmd@(irep,imd) = (get lensRep >< get lensMd) (rep,md)
...
doLoadDeltaCompound :: 	(Eq rep',Eq md',Eq irep,ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,FSRep fs,ForestMD fs md',MData NoCtx (ForestO fs) list_rep',MData NoCtx (ForestO fs) list_imd
...
doLoadDeltaCompound lensRep lensMd isoRep isoMd mpath path' matchingM oldtree df tree' ((rep,md),getMD) load loadD = debug ("doLoadDeltaCompound: "++show (path')) $ do
	...
	let crep = get lensRep rep
	let cmd = get lensMd md
	...
	newfiles <- getMatchingFilesInTree path matching tree'
	(newrepmds,noDeletions) <- forestIO $ alignWithLookup newfiles oldfiles oldrepmds -- alignment by filename; newfiles first for the association list to be sorted by newfile positions
...
doLoadDeltaCompoundFile mpath path' file' Nothing oldtree df tree' load loadD = debug ("doLoadDeltaCompoundFileNothing: "++show (path',file')) $ do
	...
	fileInfo_thunk <- inside $ fsref info
...
doLoadDeltaCompoundFile mpath path' file' (Just (rep,(md,fileInfo_thunk))) oldtree df tree' load loadD = debug ("doLoadDeltaCompoundFile: "++show (path',file')) $ do
	...
	(newGetMD,dFileInfo) <- do
		...
		case (isUnevaluated,path == path',isEmptyTopFSTreeDeltaNodeMay $ focusFSTreeDeltaNodeMayByRelativePath df file') of
			...
			otherwise -> getRelForestMDInTree path' tree' file' >>= \fmd -> fsset fileInfo_thunk (fileInfo fmd) >> return (const2 $ return fmd,Delta)
...
doLoadDeltaCompoundWithConstraint :: (ForestOutput fs ICThunk Inside,Eq rep',Eq md',Eq irep,ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,FSRep fs,ForestMD fs md',MData NoCtx (ForestO fs) list_rep', imd ~ (md',(ForestFSThunkI fs FileInfo,ForestICThunkI fs Bool))
...
stopIf :: (Eq rep,Eq md,Eq irep,ForestMD fs md,FSRep fs,rep ~ ForestFSThunkI fs irep,StableMD fs md) =>
...
skipUpdateIf3 :: FSRep fs => String -> Bool -> Bool -> Bool -> ForestO fs (SValueDelta rep,SValueDelta md) -> ForestO fs (SValueDelta rep,SValueDelta md)
skipUpdateIf3 str b1 b2 b3 io = if b1 && b2 && b3
	then debug ("skippedUpdate "++str) $ return (Id,Id)
	else io
	
skipUnevaluated :: (Eq rep,Eq md,FSRep fs,Eq irep,ForestRep rep (ForestFSThunkI fs irep),ForestMD fs md,StableMD fs md) =>
...
skipUnevaluated str tree' olddata@((rep,md),getMD) load loadD = do
	...
	if cond1 && cond2
		then do -- if the thunks have not been forced, then we can return a NoOp update because we know that their values have not been necessary, so no other part of the specification needs to incrementally repaired
			...
			fsoverwrite [tree'] rep_thunk $ fsforce =<< liftM (to isoRep) (getRep $ read loadThunk)
			overwriteMD [tree'] md $ getMd $ read loadThunk
