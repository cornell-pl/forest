...
-- for each variable name, we store (a boolean that indicates whether its value has NOT changed and optionally a thunk (the name of a thunk that holds its value,a pattern to match against the thunk's value)
type ZDeltaEnv = Map Name (TH.Exp,Maybe (Name,Pat))

type ZDeltaQ a = ReaderT (Name,ZDeltaEnv) Q a

--environments store maps from variables to optional thunks (the name of the thunk that holds its value,a pattern to match against the thunk's value)
