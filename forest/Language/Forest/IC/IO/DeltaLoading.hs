...
doLoadDeltaFile :: (IncK (IncForest fs) Forest_err,IncK
	...
	ForestI fs FilePath -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs -> OldData fs (ForestFSThunkI fs pads) (ForestFSThunkI fs (Forest_md fs,ForestFSThunkI fs md))
...
doLoadDeltaFile mpath path' oldtree df tree' ((rep_thunk::ForestFSThunkI fs pads,md_thunk),getMD) = do
	...
	case (path == path',df) of
		(True,isEmptyFSTreeD fs -> True) -> debug "constant0 unchanged" $ return (Id,Id)
		(True,isChgFSTreeD fs -> True) -> debug "constant0 attrs" $ do
			modify md_thunk $ \(_,md) -> getMD path' tree' >>= \fmd' -> return (fmd',md)
			return (Id,Delta)
		otherwise -> debug "constant0 changed" $ do
			(rep_thunk',md_thunk') <- inside $ doLoadFile (Proxy::Proxy pads) (fsTreeDPathFilter fs df path') path' tree' getMD
			overwrite rep_thunk $ Inc.get rep_thunk'
			overwrite md_thunk $ Inc.get md_thunk'
			return (Delta,Delta)

doLoadDeltaFile1 :: (				IncK
			                        (IncForest fs) (Forest_md fs, ForestFSThunkI fs md),		IncK (IncForest fs) (pads, md),
			                      IncK (IncForest fs) pads,
			                      IncK (IncForest fs) md,MData NoCtx (ForestI fs) arg,ICMemo fs,ForestIs fs arg ~ ForestI fs arg,ForestInput fs FSThunk Inside,ForestDefault fs Inside pads,ForestDefault fs Inside (Forest_md fs,ForestFSThunkI fs md),ForestMD fs md,Eq arg,Typeable arg,ICRep fs,Pads1 arg pads md)
	=> Bool -> arg -> ForestI fs FilePath -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs -> OldData fs (ForestFSThunkI fs pads) (ForestFSThunkI fs (Forest_md fs,ForestFSThunkI fs md))
	-> ForestO fs (SValueDelta (ForestFSThunkI fs pads),SValueDelta (ForestFSThunkI fs (Forest_md fs,ForestFSThunkI fs md)))
doLoadDeltaFile1 isEmptyDArg arg' mpath path' oldtree df tree' ((rep_thunk::ForestFSThunkI fs pads,md_thunk),getMD) = do
	...
	case (isEmptyDArg,path == path',df) of
		(True,True,isEmptyFSTreeD fs -> True) -> debug "constant1 unchanged" $ return (Id,Id)
		(True,True,isChgFSTreeD fs -> True) -> debug "constant1 attrs" $ do
			modify md_thunk $ \(_,md) -> getMD path' tree' >>= \fmd' -> return (fmd',md)
			return (Id,Delta)
		otherwise -> debug "constant1 changed" $ do
			(rep_thunk',md_thunk') <- inside $ doLoadFile1 (Proxy::Proxy pads) arg' (fsTreeDPathFilter fs df path') path' tree' getMD
			overwrite rep_thunk $ Inc.get rep_thunk'
			overwrite md_thunk $ Inc.get md_thunk'
			return (Delta,Delta)

doLoadDeltaArchive :: (IncK (IncForest fs) (Forest_md fs, md),			IncK (IncForest fs) (rep, (Forest_md fs, md)),
			                      IncK (IncForest fs) rep,
			                      IncK (IncForest fs) md,Typeable rep,Typeable md,ForestDefault fs Inside (rep,(Forest_md fs,md)),ICMemo fs
		,ForestInput fs FSThunk Inside,ForestMD fs md,ICRep fs) =>
	Bool -> [ArchiveType] -> ForestI fs FilePath -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs -> OldData fs (ForestFSThunkI fs rep) (ForestFSThunkI fs (Forest_md fs,md))
...
doLoadDeltaArchive isClosed exts mpath path' oldtree df (tree' :: FSTree fs) ((rep_thunk::(ForestFSThunkI fs rep),md_thunk),getMD) load loadD = do
	let fs = Proxy :: Proxy fs
	path <- inside mpath
	case (path == path',df) of
		(True,isEmptyFSTreeD fs -> True) -> return (Id,Id)
		(True,isChgFSTreeD fs -> True) -> do
		...
		(_,isMoveFSTreeD fs -> Just from) -> do
			(rep',md') <- inside $ doLoadArchive isClosed (Proxy::Proxy rep) exts (fsTreeDPathFilter fs df path') path' tree' getMD load loadD
...
doLoadDeltaSymLink mpath path' oldtree df (tree' :: FSTree fs) ((rep_thunk,md_thunk),getMD) = do
	let fs = Proxy :: Proxy fs
	path <- inside mpath
	case (path == path',df) of
		(True,isEmptyFSTreeD fs -> True) -> debug "symlink unchanged" $ return (Id,Id)
		(True,isChgFSTreeD fs -> True) -> do
			modify md_thunk $ \(_,md) -> getMD path' tree' >>= \fmd' -> return (fmd',md)
			return (Id,Delta)
...
doLoadDeltaConstraint :: (IncK (IncForest fs) Bool,IncK (IncForest fs) imd',ForestOutput fs ICThunk Inside,ForestInput fs FSThunk Outside,md ~ ForestFSThunkI fs imd',ForestMD fs imd', imd ~ MDArgs mode md (ForestICThunkI fs Bool)) =>
...
doLoadDeltaArgs :: (ForestArgs fs args,ICRep fs, imd ~ MDArgs mode md (ForestICThunksI fs args)) =>
...
--					set rep_thunk (f irep') >> return Delta -- modify the directory thunk, since underlying non-stable changes did not occur inside a modifiable
			dmd <- case (path == path',isEmptyTopFSTreeD fs df) of 
...
doLoadDeltaMaybe :: (IncK (IncForest fs) (Forest_md fs, Maybe md),IncK (IncForest fs) (Maybe rep),IncK (IncForest fs) (rep, md),ForestDefault fs Outside rep,ForestDefault fs Outside md,ForestMD fs md,ForestDefault fs Inside (Maybe rep),ForestDefault fs Inside (Forest_md fs,Maybe md),ForestDefault fs Inside (Forest_md fs)) =>
	ForestI fs FilePath -> OldData fs (ForestFSThunkI fs (Maybe rep)) (ForestFSThunkI fs (Forest_md fs,Maybe md)) -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs
	-> (GetForestMD fs -> ForestI fs (rep,md))
	-> (OldData fs rep md -> ForestO fs (SValueDelta rep,SValueDelta md))
...
doLoadDeltaMaybe mpath ((rep_thunk,md_thunk),getMD) path' oldtree df (tree' :: FSTree fs) load loadD = do
	...
	case (exists,exists') of
		(False,False) -> do
			case (path == path',isEmptyTopFSTreeD fs df) of
				(True,True) -> return (Id,Id)
				otherwise -> do
		...
		(True,True) -> do
			...
			return (mapSValueDelta direp,Delta)
		(False,True) -> do
			(loadRep,loadMd) <- inside $ doLoadMaybe' (fsTreeDPathFilter fs df path') path' tree' $ load getMD
			overwrite rep_thunk loadRep
			overwrite md_thunk loadMd
			return (Delta,Delta)
...
doLoadDeltaFocus :: (ForestRep rep (ForestFSThunkI fs irep),Matching fs a,ForestMD fs md) =>
	ForestI fs FilePath -> FilePath -> OldData fs rep md -> a -> FSTree fs -> FSTreeD fs -> FSTree fs
	-> (FilePath -> GetForestMD fs -> ForestI fs (rep,md))
	-> (ForestI fs FilePath -> FilePath -> FSTreeD fs -> ForestO fs (SValueDelta rep,SValueDelta md))
	-> ForestO fs (SValueDelta rep,SValueDelta md)
doLoadDeltaFocus mpath path' olddata matching oldtree df (tree' :: FSTree fs) load loadD = do
	let fs = Proxy :: Proxy fs
	let staticLoad getMD = doLoadFocus (fsTreeDPathFilter fs df path') path' matching tree' getMD load
	let deltaLoad (repmd@(rep,md),getMD) = do
		...
		case files' of
			...
			files' -> do
				...
				doLoadDeltaNewPath mcnewpath path' file' oldtree df tree' $ \mnewpath newdpath newdf -> do
					(drep,dmd) <- loadD mnewpath newdpath newdf -- changes the delta
					dmd' <- case dmd of
						Id -> return Id
...
pickFileDelta :: ICRep fs => ForestI fs FilePath -> FilePath -> [FileName] -> FSTree fs -> FSTreeD fs -> FSTree fs -> ForestI fs FileName
pickFileDelta moldpath path' files' oldtree df (tree' :: FSTree fs) = do
--	newpaths <- mapM (\f -> liftM (,f) $ canonalizePathInTree (path' </> f) tree') files'
--	case List.lookup oldpath newpaths of
--		Just file -> return file
--		Nothing ->
	let fs = Proxy :: Proxy fs
	inside $ pickFileNoDelta (fsTreeDPathFilter fs df path') path' files' tree'

-- changes the delta
doLoadDeltaNewPath :: (ICRep fs) => ForestI fs FilePath -> FilePath -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs
	-> (ForestI fs FilePath -> FilePath -> FSTreeD fs -> ForestO fs (SValueDelta rep,SValueDelta md))
	-> ForestO fs (SValueDelta rep,SValueDelta md)
doLoadDeltaNewPath moldpath path' file' oldtree df (tree' :: FSTree fs) loadD = debug ("doLoadDeltaNewPath: "++show (path',file')) $ do
	let fs = Proxy :: Proxy fs
	...
	debug ("changed FSDelta: ") $ loadD moldpath newpath' newdf -- load recursively

doLoadDeltaSimple :: (IncK (IncForest fs) imd',ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,MData NoCtx (ForestO fs) rep',md' ~ ForestFSThunkI fs imd',Eq imd',ForestMD fs imd') =>
	ForestI fs FilePath -> FilePath -> ForestI fs a -> FSTree fs -> FSTreeD fs -> FSTree fs -> OldData fs rep' md'
	-> (FilePath -> GetForestMD fs -> ForestI fs (rep',md'))
	-> (OldData fs rep' md' -> ForestI fs FilePath -> FilePath -> FSTreeD fs -> ForestO fs (SValueDelta rep',SValueDelta md'))
	-> ForestO fs (SValueDelta rep',SValueDelta md')
...
doLoadDeltaSimpleWithConstraint :: (IncK (IncForest fs) Bool,IncK (IncForest fs) imd1,ForestOutput fs ICThunk Inside,ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,MData NoCtx (ForestO fs) rep',md' ~ ForestFSThunkI fs imd1,ForestMD fs imd1, imd' ~ MDArgs mode md' (ForestICThunkI fs Bool)) =>
	LiftedICMode mode -> Bool -> ForestI fs FilePath -> FilePath -> ForestI fs a -> FSTree fs -> FSTreeD fs -> FSTree fs -> OldData fs rep' imd' -> ((rep',md') -> ForestI fs Bool)
	-> (FilePath -> GetForestMD fs -> ForestI fs (rep',md'))
	-> (OldData fs rep' md' -> ForestI fs FilePath -> FilePath -> FSTreeD fs -> ForestO fs (SValueDelta rep',SValueDelta md'))
	-> ForestO fs (SValueDelta rep',SValueDelta imd')
...
doLoadDeltaCompound :: 	(IncK (IncForest fs) container_rep',IncK (IncForest fs) container_md',IncK (IncForest fs) FileInfo,rep ~ ForestFSThunkI fs container_rep',md ~ ForestFSThunkI fs container_md',ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,ICRep fs,ForestMD fs md',MData NoCtx (ForestO fs) list_rep',MData NoCtx (ForestO fs) list_imd
	  ,list_rep' ~ [(FileName,rep')],list_imd ~ [(FileName,imd)], imd ~ MDArgs mode md' (ForestFSThunkI fs FileInfo)) =>
		...
		-> (FileName -> FileName -> ForestFSThunkI fs FileInfo -> SValueDelta (ForestICThunkI fs FileInfo) -> OldData fs rep' md' -> ForestI fs FilePath -> FilePath -> FSTreeD fs -> ForestO fs (SValueDelta rep',SValueDelta md'))
		-> ForestO fs (SValueDelta rep,SValueDelta md)
...
doLoadDeltaCompoundFile :: (IncK (IncForest fs) FileInfo,ForestRep rep (ForestFSThunkI fs irep),ForestMD fs md, imd ~ MDArgs mode md (ForestFSThunkI fs FileInfo)) => 
	LiftedICMode mode -> ForestI fs FilePath -> FilePath -> FileName -> Maybe (rep,imd) -> FSTree fs -> FSTreeD fs -> FSTree fs
	-> (FileName -> ForestFSThunkI fs FileInfo -> FilePath -> GetForestMD fs -> ForestI fs (rep,md))
	-> (FileName -> FileName -> ForestFSThunkI fs FileInfo -> SValueDelta (ForestICThunkI fs FileInfo) -> OldData fs rep md -> ForestI fs FilePath -> FilePath -> FSTreeD fs -> ForestO fs (SValueDelta rep,SValueDelta md))
	-> ForestO fs ((Maybe (FileName,rep),ValueDeltaKind),(Maybe (FileName,imd),ValueDeltaKind))
doLoadDeltaCompoundFile mode mpath path' file' Nothing (oldtree :: FSTree fs) df tree' load loadD = debug ("doLoadDeltaCompoundFileNothing: "++show (path',file')) $ do
	let fs = Proxy :: Proxy fs
...
doLoadDeltaCompoundWithConstraint :: (IncK (IncForest fs) container_rep',IncK (IncForest fs) container_md',IncK (IncForest fs) FileInfo,
	                      IncK (IncForest fs) Bool,rep ~ ForestFSThunkI fs container_rep',md ~ ForestFSThunkI fs container_md',ForestOutput fs ICThunk Inside,ForestRep rep' (ForestFSThunkI fs irep),Matching fs a,ICRep fs,ForestMD fs md',MData NoCtx (ForestO fs) list_rep', imd ~ MDArgs mode md' (ForestFSThunkI fs FileInfo,ForestICThunkI fs Bool)
...
-- returns the new values and a boolean indicating whether it has changed
doLoadDeltaCompoundFileWithConstraint :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Bool,ForestOutput fs ICThunk Inside,ForestRep rep (ForestFSThunkI fs irep),ForestMD fs md, imd ~ MDArgs mode md (ForestFSThunkI fs FileInfo,ForestICThunkI fs Bool)) => 
	LiftedICMode mode -> ForestI fs FilePath -> FilePath -> FileName -> Maybe (rep,imd) -> FSTree fs -> FSTreeD fs -> FSTree fs
	-> (FileName -> ForestFSThunkI fs FileInfo -> ForestI fs Bool)
	-> (FileName -> ForestFSThunkI fs FileInfo -> FilePath -> GetForestMD fs -> ForestI fs (rep,md))
	-> (FileName -> FileName -> ForestFSThunkI fs FileInfo -> SValueDelta (ForestICThunkI fs FileInfo) -> OldData fs rep md -> ForestI fs FilePath -> FilePath -> FSTreeD fs -> ForestO fs (SValueDelta rep,SValueDelta md))
	-> ForestO fs ((Maybe (FileName,rep),ValueDeltaKind),(Maybe (FileName,imd),ValueDeltaKind))
doLoadDeltaCompoundFileWithConstraint mode mpath path' file' Nothing (oldtree :: FSTree fs) df tree' pred load loadD = debug ("doLoadDeltaCompoundFileNothing: "++show (path',file')) $ do
	path <- inside mpath
	let fs = Proxy :: Proxy fs
	-- try to reuse the original metadata (in this case it is not reused)
	(newGetMD,info) <- inside $ getRelForestMDInTree path' tree' file' >>= \fmd -> return (const2 $ return fmd,fileInfo fmd)
...
doLoadDeltaCompoundFileWithConstraint mode mpath path' file' (Just (rep,imd)) oldtree df (tree' :: FSTree fs) pred load loadD = debug ("doLoadDeltaCompoundFile: "++show (path',file')) $ do
	let fs = Proxy :: Proxy fs
	path <- inside mpath
...
stopIf :: (IncK (IncForest fs) (ForestFSThunkI fs irep, md),
                      IncK (IncForest fs) irep,ForestMD fs md,ICRep fs,rep ~ ForestFSThunkI fs irep,StableMD fs md) =>
	String -> Bool -> ForestI fs FilePath -> FilePath -> FSTreeD fs -> FSTree fs -> OldData fs rep md
	...
	-> ForestO fs (SValueDelta rep,SValueDelta md)
stopIf str isEmptyEnv mpath path' df (tree' :: FSTree fs) repmd load loadD = skipUnevaluated str tree' repmd load $ \newrepmd -> do
	let fs = Proxy :: Proxy fs
	path <- inside mpath
	skipUpdateIf3 str isEmptyEnv (path == path') (isEmptyFSTreeD fs df) $ loadD newrepmd
...
skipUnevaluated :: (IncK (IncForest fs) (rep, md),IncK (IncForest fs) irep,ICRep fs,ForestRep rep (ForestFSThunkI fs irep),ForestMD fs md,StableMD fs md) =>
