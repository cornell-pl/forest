...
module Language.Forest.Pure.CodeGen.Storing where

import Language.Forest.Pure.CodeGen.Utils
import Control.Monad.Trans
import Language.Forest.IO.Utils
import Language.Forest.Pure.IO.Storing
...
genManifestM :: Name -> Name -> ForestTy -> [(TH.Pat,TH.Type)] -> Q TH.Exp
genManifestM rep_name md_name forestTy pat_infos = do
	...
	case pat_infos of
		...
		otherwise -> do
			(argsP) <- genManifestArgsE (zip [1..] pat_infos) forestTy
			let argsT = forestTupleTy $ map (AppT (ConT ''Arg) . snd) pat_infos
			
			core_bodyE <- genManifestBody treeE dtaE manE rep_name forestTy
			return $ LamE [argsP,treeP,dtaP,manP] core_bodyE

genManifestBody :: TH.Exp -> TH.Exp -> TH.Exp -> Name -> ForestTy -> Q TH.Exp
...
genManifestArgsE :: [(Int,(TH.Pat,TH.Type))] -> ForestTy -> Q Pat
genManifestArgsE [pat_info] forestTy = genManifestArgE pat_info forestTy
genManifestArgsE (pat_info:pat_infos) forestTy = do
	pat <- genManifestArgE pat_info forestTy
	pats <- genManifestArgsE pat_infos forestTy
	return (ConP '(:*:) [pat,pats])

genManifestArgE :: (Int,(TH.Pat,TH.Type)) -> ForestTy -> Q Pat
genManifestArgE (i,(pat,pat_ty)) forestTy = return pat

manifestE :: ForestTy -> Exp -> Exp -> Exp -> Q Exp
...
manifestWithArgsE :: [TH.Exp] -> TH.Exp -> TH.Exp -> TH.Exp -> Q TH.Exp
manifestWithArgsE [] treeE dtaE manE = return $ appE4 (VarE 'updateManifestWithTree) (TupE []) treeE dtaE manE
manifestWithArgsE argsE treeE dtaE manE = do
	let tupArgsE = foldl1' (appE2 (ConE '(:*:))) argsE
	return $ appE4 (VarE 'updateManifestWithTree) tupArgsE treeE dtaE manE

manifestArchive :: [ArchiveType] -> ForestTy -> TH.Exp -> TH.Exp -> TH.Exp -> Q TH.Exp
manifestArchive archtype ty treeE dtaE manE = do
	newTreeName <- newName "new_tree"
	let (newTreeE, newTreeP) = genPE newTreeName
	newmanName <- newName "man"
	let (newmanE,newmanP) = genPE newmanName
	newdtaName <- newName "dta"
...
manifestSymLink :: TH.Exp -> TH.Exp -> TH.Exp -> Q TH.Exp
manifestSymLink treeE dtaE manE = do
  return $ appE3 (VarE 'doManifestSymLink) treeE dtaE manE

manifestConstraint :: TH.Exp -> TH.Pat -> TH.Exp -> TH.Exp -> TH.Exp -> (Exp -> Exp -> Q TH.Exp) -> Q TH.Exp
manifestConstraint treeE pat predE dtaE manE manifest = do
	newmanName <- newName "man"
	let (newmanE,newmanP) = genPE newmanName
	newdtaName <- newName "dta"
	let (newdtaE,newdtaP) = genPE newdtaName
...
manifestFile :: String -> Maybe TH.Exp -> TH.Exp -> TH.Exp -> TH.Exp -> Q TH.Exp
...
manifestMaybe :: ForestTy -> TH.Exp -> TH.Exp -> TH.Exp -> Q TH.Exp
manifestMaybe ty treeE dtaE manE = do 
	newdtaName <- newName "newdta"
	newrepName <- newName "newrep"
	newpathName <- newName "newpath"
	newmanName <- newName "newman"
	let (newdtaE,newdtaP) = genPE newdtaName
	let (newmanE,newmanP) = genPE newmanName
	...
	defaultContentsE <- liftM (LamE [newrepP,newpathP]) $ defaultE ty newrepE newpathE
	return $ appE5 (VarE 'doManifestMaybe) treeE dtaE doContentsE defaultContentsE manE

manifestDirectory :: DirectoryTy -> TH.Exp -> TH.Exp -> TH.Exp -> Q TH.Exp
...
manifestDirectoryContents :: DirectoryTy -> TH.Exp -> TH.Exp -> TH.Exp -> TH.Exp -> Q Exp
...
manifestFields :: [Field] -> TH.Exp -> TH.Exp -> TH.Exp -> TH.Exp -> Q [Stmt]
...
manifestFields (field:fields) treeE parentPathE dtaE man0E = do
	man1Name <- newName "man"
	let (man1E,man1P) = genPE man1Name
	(rep_field,md_field,stmts_field) <- manifestField field treeE parentPathE dtaE man0E man1P
	stmts_fields <- manifestFields fields treeE parentPathE dtaE man1E
	return $ stmts_field++stmts_fields

manifestField :: Field -> TH.Exp -> TH.Exp -> TH.Exp -> TH.Exp -> Pat -> Q (Name,Name,[Stmt])
...
manifestComp :: CompField -> TH.Exp -> TH.Exp -> TH.Exp -> Q TH.Exp
...
manifestCompound :: Bool -> CompField -> TH.Exp -> TH.Exp -> TH.Exp -> TH.Exp -> TH.Pat -> Q (Name,Name,[Stmt])
manifestCompound isNested (CompField internal tyConNameOpt explicitName externalE descTy generatorP _ generatorG predM) treeE parentPathE dtaE man0E man1P = do
	-- variable declarations
	let repName = mkName internal
	let mdName  = mkName (internal++"_md")
	let (repE,repP) = genPE repName
...
--	let letMdS = LetS [ValD mdP (NormalB innerMdE) []]
	...
	case predM of
		Nothing -> do
			manifestSingleE <- liftM (LamE [VarP fileName,VarP fileNameAtt,newdtaP,newmanP]) $ manifestE descTy treeE newdtaE newmanE
