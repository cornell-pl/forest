...
We briefly touch on how programmers can verify, at any time, if a filestore conforms to its specification~(\ref{subsec:validation}), and finish by introducing analogues of standard file system operations over filestores~(\ref{subsec:fsops}).
...
In TxForest, one runs a transaction by calling the |atomic| function with type:\footnote{For the original \texttt{STM} interface, substitute |FTM| by |STM|~\cite{HaskellSTM}.}
\begin{spec}
	atomic :: FTM a -> IO a
\end{spec}
...
\begin{spec}
	[forest|
		type NamedAccounts (acc :: String) = [ a :: Account | a <- matches (GL (acc ++ "*")) ]
	|]
\end{spec}
