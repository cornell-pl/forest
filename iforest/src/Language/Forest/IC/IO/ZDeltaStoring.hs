...
doZDeltaManifestNamed :: (FTK fs rep,ZippedICForest fs rep) =>
	Proxy rep -> LoadDeltaArgs fs (ForestArgs rep) -> FilePath -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs -> (ForestFSThunkI fs rep) -> ValueDelta fs (ForestFSThunkI fs rep) -> Manifest fs -> MManifestForestO fs
doZDeltaManifestNamed proxy (margs,dargs) path path' tree df tree' rep dv man = do
	irep <- lift $ outside $ fsForce rep
	idv <- lift $ diffValueBelow dv diffValue tree irep
	zupdateManifestDeltaGeneric (margs,dargs) path path' tree df tree' irep idv man

-- updates the thunks that keep track of the arguments of a top-level declaration
doZDeltaManifestArgs :: (ForestArgsClass fs (ForestArgs rep),ICRep fs) =>
	LoadDeltaArgs fs (ForestArgs rep) -> rep -> ValueDelta fs rep
	-> (ForestICThunksI fs (ForestArgs rep) -> rep -> ValueDelta fs rep -> Manifest fs -> MManifestForestO fs)
	->  Manifest fs -> MManifestForestO fs
doZDeltaManifestArgs (margs,_) (rep::rep) dv manifestD (man :: Manifest fs) = debug ("doStoreDeltaArgs") $ do
...
doZDeltaManifestArchive isClosed archTy path path' (tree :: FSTree fs) df tree' arch_rep arch_dv manifest manifestD diffValue man = do
	...
	if (not $ isIdValueDelta topdv) then mani_arch else do
		...
		(fmd,rep) <- lift $ outside $ fsForce arch_rep
		...
		case (exists,exists_dir,exists') of
			(False,False,False) -> case (path == path',isIdValueDelta arch_dv,isEmptyTopFSTreeD fs df) of
				...
				otherwise -> do
					...
					Writer.tell $ \latest -> replaceForestMDErrorsWith latest fmd $ liftM (:[]) $ liftM (missingDirForestErr path `mergeForestErrs`) $ get_errors rep
			...
			(True,True,True) -> do
				...
				let testm = case (path==path',isIdValueDelta top && isIdValueDelta toperror) of
					...
					otherwise -> do
						...
						status2 <- liftM (boolStatus $ ConflictingPath path path_fmd) $ latestTree >>= sameCanonicalFullPathInTree path_fmd path
						return $ status1 `mappend` status2
				Writer.tell $ \latest -> replaceForestMDErrorsWith latest fmd $ liftM (:[]) $ get_errors rep
...
doZDeltaManifestConstraint :: (Typeable rep,ForestContent fs rep,IncK (IncForest fs) (ForestFSThunkI fs Forest_err, rep),ForestMD fs rep,ICRep fs, toprep ~ ForestFSThunkI fs (ForestFSThunkI fs Forest_err,rep)) =>
	Bool -> (ForestContentTy fs rep -> ForestI fs Bool) -> FSTree fs -> FSTree fs -> toprep -> ValueDelta fs toprep
	-> (rep -> Manifest fs -> MManifestForestO fs)
	-> (rep -> ValueDelta fs rep -> Manifest fs -> MManifestForestO fs)
	-> (FSTree fs -> rep -> ForestO fs (ValueDelta fs rep))
	-> Manifest fs -> MManifestForestO fs
doZDeltaManifestConstraint emptyDArgs pred tree tree' rep_t dv manifest manifestD diffValue man = do
	
	let mani_k = doZManifestConstraint pred tree' rep_t manifest man
	
	topdv <- lift $ diffTopValueThunk tree rep_t
	if (not $ isIdValueDelta topdv) then mani_k else do
		(err_t,rep) <- lift $ outside $ fsForce rep_t
		idv <- lift $ diffValueBelow dv diffValue tree rep
		doZDeltaManifestConstraintInner emptyDArgs pred tree tree' (err_t,rep) (mapValueDelta Proxy idv) manifestD diffValue man

doZDeltaManifestConstraintInner :: (ForestContent fs rep,ForestMD fs rep,ICRep fs, toprep ~ (ForestFSThunkI fs Forest_err,rep)) =>
	Bool -> (ForestContentTy fs rep -> ForestI fs Bool) -> FSTree fs -> FSTree fs -> toprep -> ValueDelta fs toprep
	-> (rep -> ValueDelta fs rep -> Manifest fs -> MManifestForestO fs)
	-> (FSTree fs -> rep -> ForestO fs (ValueDelta fs rep))
	-> Manifest fs -> MManifestForestO fs
doZDeltaManifestConstraintInner emptyDArgs pred tree tree' (err_t,rep) dv manifestD diffValue man = do
	isRepairMd <- Reader.ask
	idv <- lift $ diffValueBelow dv diffValue tree rep
	man1 <- manifestD rep idv man
	
	Writer.tell $ \latest -> do
		fsOverwrite latest err_t $ do
...
doZDeltaManifestDirectory path path' tree df (tree' :: FSTree fs) dirrep_t dv collectMDErrors manifest manifestD diffValue man = do
	...
	if (not $ isIdValueDelta topdv) then mani_dir else do
		...
		(fmd,rep) <- lift $ outside $ fsForce dirrep_t
		err_t <- lift $ get_errors_thunk fmd
		path_fmd <- lift $ get_fullpath fmd
		...
		case (exists,exists_dir,exists') of
			(False,False,False) -> case (path == path',isIdValueDelta dv,isEmptyTopFSTreeD fs df) of
				...
				otherwise -> do
					let testm = do
						...
						status2 <- liftM (boolStatus $ ConflictingPath path path_fmd) $ latestTree >>= sameCanonicalFullPathInTree path_fmd path
						return $ status1 `mappend` status2
					Writer.tell $ \latest -> replaceForestMDErrorsWith latest fmd $ liftM (:[]) $ liftM (missingDirForestErr path `mergeForestErrs`) $ collectMDErrors rep
					return $ addTestToManifest testm man -- errors in the metadata must be consistent
			...
			(True,True,True) -> do
				...
				idv <- lift $ diffValueBelow dv diffValue tree rep
				Writer.tell $ \latest -> replaceForestMDErrorsWith latest fmd $ liftM (:[]) $ collectMDErrors rep
...
doZDeltaManifestMaybe path path' tree df (tree' :: FSTree fs) mbrep_t dv manifest manifestD diffValue man = do
	...
	if (not $ isIdValueDelta topdv) then mani_maybe else do
		...
		(fmd,mb_rep) <- lift $ outside $ fsForce mbrep_t
		err_t <- lift $ get_errors_thunk fmd
		path_fmd <- lift $ inside $ get_fullpath fmd
		
		-- guarantee that the error thunk is being updated on inner changes 
		Writer.tell $ \latest -> get_errors_thunk fmd >>= flip (fsOverwrite latest) (maybe (return cleanForestErr) get_errors mb_rep)
				...
				case (path == path',isIdValueDelta dv,isEmptyTopFSTreeD fs df) of
					...
					otherwise -> do
						let testm = do
							...
							status2 <- if isRepairMd then return Valid else liftM (boolStatus ConflictingRepMd) $ forestO $ inside $ liftM (==fmd) $ cleanForestMDwithFile tree' path'
							return $ status1 `mappend` status2
						return $ addTestToManifest testm man
...
doZDeltaManifestSimpleWithConstraint :: (ForestContent fs rep,ForestMD fs rep,ICRep fs,Matching fs a,err_rep ~ (ForestFSThunkI fs Forest_err, rep)) => 
	Lens dir_rep err_rep -> Bool -> (ForestContentTy fs rep -> ForestI fs Bool)
	-> FilePath -> FilePath -> ForestI fs a -> FSTree fs -> FSTreeD fs -> FSTree fs
	-> dir_rep -> ValueDelta fs dir_rep
	-> (rep -> ValueDelta fs rep -> FilePath -> FilePath -> FSTreeD fs -> Manifest fs -> MManifestForestO fs)
	-> (FSTree fs -> rep -> ForestO fs (ValueDelta fs rep))
...
doZDeltaManifestSimpleWithConstraint lens emptyDArgs pred path path' matchingM tree df tree' dir_rep dir_dv manifestD diffValue man = do
	...
	doZDeltaManifestConstraintInner emptyDArgs pred tree tree' rep (mapValueDelta Proxy dv) (\rep dv -> doZDeltaManifestFocus matching path path' tree df tree' $ manifestD rep dv) diffValue man
...
doZDeltaManifestCompound lens isoRep mkeyarg path path' matchingM tree df tree' dir_rep dir_dv manifestD diffValue man = do
	...
	repinfos <- lift $ inside $ mapM (\rep -> fsThunk tree' (get_info rep) >>= \fileInfo_t -> return (rep,fileInfo_t)) newreps
...
zskipManifestIf :: (Typeable rep,Typeable (ForestRepTy fs rep),IncK (IncForest fs) (ForestRepTy fs rep),ForestMD fs rep,ICRep fs,ForestRep fs rep,DeltaClass d) =>
			...
			Writer.tell $ \latest_tree -> do
				...
				case mb_latest_df of
					Just latest_df -> do
						...
						return ()
					Nothing -> fsOverwrite tree' (to (iso_rep_thunk fs) rep) $ fsForce =<< liftM (to (iso_rep_thunk fs)) (load latest_tree)
			return man
