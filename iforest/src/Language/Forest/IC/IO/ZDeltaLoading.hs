...
doZLoadDeltaNamed :: (FTK fs rep,ZippedICForest fs rep) =>
	Proxy rep -> LoadDeltaArgs fs (ForestArgs rep) -> ForestI fs FilePath -> FSTree fs -> (ForestFSThunkI fs rep,GetForestMD fs) -> FilePath -> FSTreeD fs -> FSTree fs -> ValueDelta fs (ForestFSThunkI fs rep) -> ForestO fs (SValueDelta (ForestFSThunkI fs rep))
doZLoadDeltaNamed proxy (args,dargs) path tree (rep,getMD) path' df tree' dv = do
	irep <- outside $ fsForce rep
	idv <- diffValueBelow dv diffValue tree irep
	direp <- zloadDeltaGeneric (args,dargs) path tree (irep,getMD) path' df tree' idv
	fsSet tree' rep $ applyNSValueDelta direp irep
	return Delta
...
doZLoadDeltaFile1 isEmptyDArg (marg' :: ForestI fs arg) mpath path' oldtree df tree' dv (rep_thunk,getMD) = do
	...
	let load_file = debug "constant1 changed" $ do
		...
		fsOverwrite tree' rep_thunk $ fsForce rep_thunk'
		inside $ addZippedMemo path' marg' rep_thunk (Just tree')
		return Delta
	...
	case (isEmptyDArg,path == path',isIdValueDelta dv,df) of
		...
		(True,True,True,isChgFSTreeD fs -> True) -> debug "constant1 attrs" $ do
			fsModify tree' rep_thunk $ \((_,bmd),rep) -> getMD path' tree' >>= \fmd' -> return ((fmd',bmd),rep)
			inside $ addZippedMemo path' marg' rep_thunk (Just tree')
			return Delta
		otherwise -> load_file

doZLoadDeltaFileInner1 :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,IncK (IncForest fs) ((Forest_md fs, md), pads),rept ~ ((Forest_md fs,md),pads),ZippedICMemo fs,MData NoCtx (ForestI fs) arg,ForestInput fs FSThunk Inside,Eq arg,Typeable arg,ICRep fs,Pads1 arg pads md)
	=> Bool -> ForestI fs arg -> ForestI fs FilePath -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs -> ValueDelta fs rept -> (rept,GetForestMD fs)
...
doZLoadDeltaArchiveInner :: (DeltaClass d,ForestRep fs rep,ZippedICMemo fs
	...
	-> (FilePath -> FSTree fs -> ForestI fs rep)
...
doZLoadDeltaArchiveInner isClosed exts mpath path' oldtree df (tree' :: FSTree fs) dv ((fmd,rep),getMD) loadGood loadBad loadD diffValue = do
	...
	case (path == path',isIdValueDelta dv,df) of
		...
		(True,True,isChgFSTreeD fs -> True) -> do
			fmd' <- inside $ getMD path' tree'
			updateForestMDErrorsWith tree' fmd' $ liftM (:[]) $ get_errors rep
		...
		otherwise -> do
			...
			case (path==path',drep) of
				(True,StableVD d) -> do
					replaceForestMDErrorsWith tree' fmd $ liftM (:[]) $ get_errors rep
					return $ StableVD Delta
				otherwise -> do
					fmd' <- inside $ getMD path tree'
					let rep' = applyDelta drep rep
					updateForestMDErrorsWith tree' fmd' $ liftM (:[]) $ get_errors rep' -- like a directory
					return $ Modify $ Prelude.const (fmd',rep')

doZLoadDeltaArchive :: (Typeable rep,IncK (IncForest fs) (Forest_md fs, rep),DeltaClass d,ForestRep fs rep,ZippedICMemo fs
		,ForestInput fs FSThunk Inside,ForestMD fs rep) =>
...
--				when isClosed $ inside $ addZippedMemo path' argsProxy () rep_thunk (Just tree')
				return Id
			(True,True,isChgFSTreeD fs -> True) -> do
				fsModify tree' rep_thunk $ \(_,irep') -> do
					fmd' <- getMD path' tree'
					fmd'' <- updateForestMDErrorsInsideWith tree' fmd' $ liftM (:[]) $ get_errors irep'
					return (fmd'',irep')
--				when isClosed $ inside $ addZippedMemo path' argsProxy () rep_thunk (Just tree')
				return Delta
			...
			-- compute a diff for the archive's content, and continue
			otherwise -> do
				rep@(fmd,irep) <- outside $ fsForce rep_thunk
				...
				case (path==path',direp) of
					(True,StableVD d) -> do
						replaceForestMDErrorsWith tree' fmd $ liftM (:[]) $ get_errors irep
					otherwise -> do
						fmd' <- inside $ getMD path tree'
						let irep' = applyNSValueDelta direp irep
						updateForestMDErrorsWith tree' fmd' $ liftM (:[]) $ get_errors irep' -- like a directory
						fsSet tree' rep_thunk (fmd',irep')
--				when isClosed $ inside $ addZippedMemo path' argsProxy () rep_thunk (Just tree')
				return Delta

doZLoadDeltaSymLink :: (IncK (IncForest fs) FileInfo,sym ~ ForestFSThunkI fs ((Forest_md fs,Base_md),FilePath),IncK (IncForest fs) Forest_err,IncK (IncForest fs) ((Forest_md fs, Base_md), FilePath),ForestInput fs FSThunk Inside,ICRep fs)
...
doZLoadDeltaSymLink mpath path' oldtree df (tree' :: FSTree fs) dv (rep_thunk,getMD) = do
	...
	case (path == path',isIdValueDelta dv,df) of
		...
		otherwise -> debug "symlink changed" $ do
			...
			fsSet tree' rep_thunk rep'
			return Delta

doZLoadDeltaSymLinkInner :: (IncK (IncForest fs) FileInfo,sym ~ ((Forest_md fs,Base_md),FilePath),IncK (IncForest fs) Forest_err,IncK (IncForest fs) ((Forest_md fs, Base_md), FilePath),ForestInput fs FSThunk Inside,ICRep fs)
	=> ForestI fs FilePath -> FilePath -> FSTree fs -> FSTreeD fs -> FSTree fs -> ValueDelta fs sym
	-> (sym,GetForestMD fs)
...
doZLoadDeltaConstraint :: (IncK (IncForest fs) FileInfo,MData NoCtx (ForestI fs) rep,Typeable rep,ForestContent fs rep,IncK (IncForest fs) (ForestFSThunkI fs Forest_err, rep),err_rep ~ ForestFSThunkI fs (ForestFSThunkI fs Forest_err,rep),DeltaClass d,ForestMD fs rep,ForestOutput fs ICThunk Inside,ForestInput fs FSThunk Outside) =>
	Bool -> FSTree fs -> FSTree fs -> ValueDelta fs err_rep -> (err_rep,GetForestMD fs) -> (ForestContentTy fs rep -> ForestI fs Bool)
	-> ForestI fs rep
...
doZLoadDeltaConstraint emptyDArgs oldtree tree' dv (t,getMD) pred load loadD diffValue = do
	...
	let load_k = do
		fsOverwrite tree' t $ doZLoadConstraintInner tree' pred load
		return Delta
	...
	drep <- liftM toNSValueDelta $ doZLoadDeltaConstraintInner emptyDArgs oldtree tree' (mapValueDelta Proxy idv) (v,getMD) pred loadD diffValue
	case drep of
		StableVD d -> return $ mapSValueDelta d
		Modify f -> do
			fsModify tree' t $ return . applyNSValueDelta drep
			return Delta

doZLoadDeltaConstraintInner :: (IncK (IncForest fs) FileInfo,ForestContent fs rep,err_rep ~ (ForestFSThunkI fs Forest_err,rep),DeltaClass d,ForestMD fs rep,ForestOutput fs ICThunk Inside,ForestInput fs FSThunk Outside) =>
	Bool -> FSTree fs -> FSTree fs -> ValueDelta fs err_rep -> (err_rep,GetForestMD fs) -> (ForestContentTy fs rep -> ForestI fs Bool)
	-> (ValueDelta fs rep -> (rep,GetForestMD fs) -> ForestO fs (d rep))
	-> (FSTree fs -> rep -> ForestO fs (ValueDelta fs rep))
	-> ForestO fs (d err_rep)
doZLoadDeltaConstraintInner emptyDArgs oldtree tree' dv ((err_t,rep),getMD) pred loadD diffValue = debug ("doLoadDeltaConstraint: ") $ do
	...
	case (emptyDArgs,isIdValueDelta dv,isEmptyDelta direp) of
		...
		otherwise -> do
			fsOverwrite tree' err_t $ do
				err_cond <- predForestErr . pred =<< runReaderT (BX.getM lens_content (return rep)) oldtree
...
doZLoadDeltaArgs :: (Typeable rep,ForestArgsClass fs (ForestArgs rep),ICRep fs) =>
	LoadDeltaArgs fs (ForestArgs rep) -> (rep,GetForestMD fs) -> FSTree fs
	-> (ForestICThunksI fs (ForestArgs rep) -> (rep,GetForestMD fs) -> ForestO fs (d rep))
	-> ForestO fs (d rep)
doZLoadDeltaArgs (margs,_) (rep::rep,getMD) (tree' :: FSTree fs) loadD = debug ("doLoadDeltaArgs") $ do
...
doZLoadDeltaDirectory :: (IncK (IncForest fs) FileInfo,Typeable rep,IncK (IncForest fs) Forest_err,IncK (IncForest fs) (Forest_md fs, rep),DeltaClass d,ICRep fs) =>
...
doZLoadDeltaDirectory mpath (rep_thunk,getMD) path' oldtree df (tree' :: FSTree fs) dv collectMDErrors loadGood loadBad loadD diffValue = do
	...
	let load_dir = do
		fsOverwrite tree' rep_thunk $ doZLoadDirectory' path' tree' collectMDErrors getMD (loadGood getMD) loadBad
		return Delta
	...
	if (not $ isIdValueDelta topdv) then load_dir else do
		...
		exists' <- if isEmptyTopFSTreeD fs df then return exists else forestM (doesDirectoryExistInTree path' tree')
		debug ("doLoadDeltaDirectory: " ++ show (path,exists,path',exists')) $ case (exists,exists') of
			(False,False) -> case (path == path',isIdValueDelta dv,isEmptyTopFSTreeD fs df) of
				...
				otherwise -> do
					fsModify tree' rep_thunk $ \(_,rep1) -> do
				...
				drep <- case (path == path',isIdValueDelta dv,isEmptyTopFSTreeD fs df) of 
					(True,True,True) -> do -- if the current path stayed the same and no attributes changed, we can reuse its @Forest_md@ but replace its errors; all the changes are stable
						case direp of
							...
							StableVD Delta -> do
								replaceForestMDErrorsWith tree' fmd $ liftM (:[]) $ collectMDErrors irep
								return Delta
							Modify f -> do
								fmd' <- inside $ getMD path' tree'
								let irep' = applyNSValueDelta direp irep
								updateForestMDErrorsWith tree' fmd' $ liftM (:[]) $ collectMDErrors irep' -- this forces the value after the update
								fsSet tree' rep_thunk (fmd',f irep') -- fsModify tree' the directory thunk, since underlying non-stable changes did not occur inside a modifiable
								return Delta
...
doZLoadDeltaDirectoryInner :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,DeltaClass d,ICRep fs) =>
...
doZLoadDeltaDirectoryInner mpath ((fmd,irep),getMD) path' oldtree df (tree' :: FSTree fs) dv collectMDErrors loadGood loadBad loadD diffValue = do
	...
	debug ("doLoadDeltaDirectory: " ++ show (path,exists,path',exists')) $ case (exists,exists') of
		(False,False) -> case (path == path',isIdValueDelta dv,isEmptyTopFSTreeD fs df) of
			...
			otherwise -> do
				fmd' <- inside $ missingDirForestMD tree' path'
...
doZLoadDeltaMaybeInner mpath (mb_rep,getMD) path' (oldtree :: FSTree fs) df tree' dv load loadD diffValue = do
	...
	exists' <- if isEmptyTopFSTreeD fs df then return exists else forestM (doesExistInTree path' tree')
...
doZLoadDeltaMaybe mpath (rep_thunk,getMD) path' oldtree df (tree' :: FSTree fs) dv load loadD diffValue = do
	...
	let load_maybe = do
		fsOverwrite tree' rep_thunk $ doZLoadMaybe' (fsTreeDPathFilter fs df path') path' tree' $ load getMD
		return Delta
	...
	if (not $ isIdValueDelta topdv) then load_maybe else do
		...
		exists <- forestM $ doesExistInTree path oldtree
		exists' <- if isEmptyTopFSTreeD fs df then return exists else forestM (doesExistInTree path' tree')
		rep@(fmd,mb_rep) <- outside $ fsForce rep_thunk
		case (exists,mb_rep,exists') of
			(False,Nothing,False) -> do
				case (path == path',isIdValueDelta dv,isEmptyTopFSTreeD fs df) of
					(True,True,True) -> return Id
					otherwise -> do
						fsModify tree' rep_thunk $ \(_,rep1) -> cleanForestMDwithFile tree' path' >>= \fmd' -> return (fmd',rep1)
						return Delta
			(_,_,False) -> do
				fsOverwrite tree' rep_thunk $ cleanForestMDwithFile tree' path' >>= \fmd' -> return (fmd',Nothing)
				return Delta
			(True,Just irep,True) -> do
				idv <- diffValueBelow dv diffValue oldtree irep
				direp <- liftM toNSValueDelta $ loadD idv (irep,getMD) -- load recursively
				case (path==path',direp) of
					...
					otherwise -> do
						...
						updateForestMDErrorsWith tree' fmd' $ liftM (:[]) $ get_errors rep
						fsSet tree' rep_thunk $ (fmd',Just irep') -- always update the metadata with the inner @Forest_md@ header
						return Delta
			(_,_,True) -> load_maybe
				...
				if length files' == 0
					then return drep
						...
						then return drep
							...
							addMultipleMatchesErrorMD tree' newpath files' rep
							return drep
...
doZLoadDeltaNewPath mpath path' mfile file' oldtree df (tree' :: FSTree fs) loadD = debug ("doLoadDeltaNewPath: "++show (path',file')) $ do
	...
	loadD mnewpath newpath' newdf -- load recursively
...
doZLoadDeltaSimpleWithConstraint :: (ForestContent fs rep',err_rep' ~ (ForestFSThunkI fs Forest_err, rep'),DeltaClass d,ForestMD fs rep',ForestOutput fs ICThunk Inside,Matching fs a,MData NoCtx (ForestO fs) rep') =>
...
doZLoadDeltaSimpleWithConstraint lens emptyDArgs mpath path' matchingM oldtree df tree' dv (dir_rep,getMD) pred loadD diffValue = debug ("doLoadDeltaSimpleWithConstraint: "++show (path')) $ do
	...
	let idata = (irep,getForestMDInTree) -- we need to discard any previously loaded forest metadata
	direp <- liftM toNSValueDelta $ doZLoadDeltaConstraintInner emptyDArgs oldtree tree' (mapValueDelta Proxy idv) idata pred (\dv idata -> doZLoadDeltaFocus mpath path' idata matching oldtree df tree' dv (loadD idata)) diffValue
	case direp of
		StableVD d -> do
...
doZLoadDeltaCompoundFile mpath path' file' key (Just rep) oldtree df (tree' :: FSTree fs) dv load loadD = debug ("doLoadDeltaCompoundFile: "++show (path',file')) $ do
	fileInfo_thunk <- getRelForestMDInTree path' tree' file' >>= \fmd -> return $ fileInfo fmd
	path <- inside mpath
	let fs = Proxy :: Proxy fs
	...
	(newGetMD,dFileInfo) <- do
		...
		case (isUnevaluated,path == path',isIdValueDelta dv,isEmptyTopFSTreeD fs $ focusFSTreeD fs df path' file' (path' </> file')) of
			...
			otherwise -> getRelForestMDInTree path' tree' file' >>= \fmd -> fsOverwrite tree' fileInfo_thunk (get_info fmd) >> return (const2 $ return fmd,Delta)
...
zstopLoadIfNS :: (Typeable (ForestRepTy fs rep),IncK (IncForest fs) (ForestRepTy fs rep),ForestMD fs rep,ICRep fs,ForestRep fs rep) =>
...
zstopLoadIfNS str isEmptyEnv mpath path' df tree' dv repmd load loadD = liftM toNSValueDelta $ zstopLoadIf str isEmptyEnv mpath path' df tree' dv repmd load loadD

-- we try @skipUnevaluated@ first to avoid forcing unecessary original data
zstopLoadIf :: (Typeable (ForestRepTy fs rep),IncK (IncForest fs) (ForestRepTy fs rep),ForestMD fs rep,ICRep fs,ForestRep fs rep) =>
	String -> Bool -> ForestI fs FilePath -> FilePath -> FSTreeD fs -> FSTree fs -> ValueDelta fs rep -> (rep,GetForestMD fs)
	-> (GetForestMD fs -> ForestI fs rep) -- static loading function
	-> (ValueDelta fs rep -> (rep,GetForestMD fs) -> ForestO fs (SValueDelta rep)) -- delta loading function
...
zstopLoadIf str isEmptyEnv mpath path' df (tree' :: FSTree fs) dv repmd load loadD = zskipLoadUnevaluated str path' tree' repmd load $ \newrepmd -> do
	let fs = Proxy :: Proxy fs
	path <- inside mpath
	let noDataChange = isIdValueDelta dv
	let samePath = path == path'
	let noFSChange = isEmptyFSTreeD fs df
	debug ("zstopLoadIf " ++ show isEmptyEnv ++ " " ++ show samePath ++ " " ++ show noFSChange ++ " " ++ show noDataChange) $
		if (isEmptyEnv && samePath && noFSChange && noDataChange)
			then debug ("skippedUpdate "++str) $ return Id
			else loadD dv newrepmd

zskipLoadUnevaluatedNS :: (Typeable (ForestRepTy fs rep),IncK (IncForest fs) (ForestRepTy fs rep),ICRep fs,ForestRep fs rep,ForestMD fs rep) =>
	String -> FilePath -> FSTree fs -> (rep,GetForestMD fs)
	-> (GetForestMD fs -> ForestI fs rep) -- static loading function
	-> ((rep,GetForestMD fs) -> ForestO fs (SValueDelta rep)) -- delta loading function
	-> ForestO fs (NSValueDelta rep)
zskipLoadUnevaluatedNS str path' tree' olddata load loadD = liftM toNSValueDelta $ zskipLoadUnevaluated str path' tree' olddata load loadD
	
zskipLoadUnevaluated :: (Typeable (ForestRepTy fs rep),IncK (IncForest fs) (ForestRepTy fs rep),ICRep fs,ForestRep fs rep,ForestMD fs rep) =>
	String -> FilePath -> FSTree fs -> (rep,GetForestMD fs)
	-> (GetForestMD fs -> ForestI fs rep) -- static loading function
	-> ((rep,GetForestMD fs) -> ForestO fs (SValueDelta rep)) -- delta loading function
	-> ForestO fs (SValueDelta rep)
zskipLoadUnevaluated str path' (tree' :: FSTree fs) olddata@(rep,getMD) load loadD = debug ("zskipLoadUnevaluated "++show str++ " "++show path') $ do 
	let fs = Proxy :: Proxy fs
	let rep_thunk = to (iso_rep_thunk fs) rep
	cond1 <- inside $ isUnevaluatedFSThunk rep_thunk
	if cond1
		then do
			fsOverwrite tree' rep_thunk $ fsForce =<< liftM (to (iso_rep_thunk fs)) (load getMD)
