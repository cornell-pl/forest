...
doZLoadNamed :: (FTK fs rep,ZippedICForest fs rep) =>
	Proxy rep -> ForestIs fs (ForestArgs rep) -> FilePathFilter fs -> FilePath -> FSTree fs -> GetForestMD fs -> ForestI fs (ForestFSThunkI fs rep)
doZLoadNamed proxy args pathfilter path tree getMD = fsThunk tree $ zloadScratchGeneric proxy args pathfilter path tree getMD
...
doZLoadFile1 (repProxy :: Proxy pads) (marg :: ForestI fs arg) oldpath_f path (tree :: FSTree fs) getMD = debug ("doLoadFile1 " ++ show path) $ do
	...
	let fileGood = do
		...
		fmd <- getMD path tree
		fmd' <- updateForestMDErrorsInsideWithPadsMD tree fmd (return md) -- adds the Pads errors
		return ((fmd',md),pads)
	
	let fileBad = doZDefaultFile1' marg path tree
	...
	let reuse_other_file from old_rep_thunk = do
		fmd' <- getMD path tree
		fmd'' <- updateForestMDErrorsInsideWithPadsMD tree fmd' (liftM (snd . fst) $ fsForce old_rep_thunk) -- adds the Pads errors
		rep_thunk <- fsForce old_rep_thunk >>= \((fmd,bmd),rep) -> fsRef tree ((fmd'',bmd),rep) -- since the old file may come from another location and/or its attributes may have changed
		return (rep_thunk)

	oldpath <- oldpath_f path
	mb <- findZippedMemo oldpath fsrepProxy -- find a move
	rep <- case mb of
	...
	addZippedMemo path marg rep (Just tree)
	return rep
...
doZLoadArchive isClosed (repProxy :: Proxy rep) exts oldpath_f path (tree :: FSTree fs) getMD loadGood loadBad loadD diffValue = do
	...
	let fsrepProxy = Proxy
	-- static loading
	let load_folder = fsThunk tree $ doZLoadArchiveInner exts oldpath_f path tree getMD loadGood loadBad
...
doZLoadArchiveInner exts oldpath_f path  tree getMD loadGood loadBad = do
	...
	let archGood = do
		...
		fmd' <- updateForestMDErrorsInsideWith tree fmd $ liftM (:[]) $ get_errors rep_arch -- like a directory
		return (fmd',rep_arch)
	
	let archBad = do
		doZDefaultArchiveInner path tree loadBad
...
doZLoadSymLink path tree getMD = fsThunk tree $ doZLoadSymLink' path tree getMD
...
doZLoadSymLink' path tree getMD = do
	...
	let linkGood = do
		...
		case symLink info of
			...
			Nothing -> do
				md' <- updateForestMDErrorsInsideWith tree md $ return [ioExceptionForestErr]
				return ((md',cleanBasePD), "")
				
	let linkBad = doZDefaultSymLink' path tree
		
	checkZPath' Nothing path tree linkGood linkBad

doZLoadConstraint :: (IncK (IncForest fs) FileInfo,ForestContent fs rep,IncK (IncForest fs) (ForestFSThunkI fs Forest_err, rep),ForestOutput fs ICThunk Inside,ForestMD fs rep,MData NoCtx (ForestI fs) rep) =>
	FSTree fs -> (ForestContentTy fs rep -> ForestI fs Bool) -> ForestI fs rep -> ForestI fs (ForestFSThunkI fs (ForestFSThunkI fs Forest_err,rep))
doZLoadConstraint tree pred load = fsThunk tree $ doZLoadConstraintInner tree pred load

-- for IC, we need to be able to dissociate the constraint error from the inner errors
doZLoadConstraintInner :: (IncK (IncForest fs) FileInfo,ForestContent fs rep,ForestOutput fs ICThunk Inside,ForestMD fs rep,MData NoCtx (ForestI fs) rep) =>
	FSTree fs -> (ForestContentTy fs rep -> ForestI fs Bool) -> ForestI fs rep -> ForestI fs (ForestFSThunkI fs Forest_err,rep)
...
doZLoadDirectory path tree collectMDErrors getMD ifGood ifBad = fsThunk tree $ doZLoadDirectory' path tree collectMDErrors getMD ifGood ifBad
...
doZLoadDirectory' path tree collectMDErrors getMD ifGood ifBad = debug ("doLoadDirectory: "++show path) $ do
	...
	let dirBad = doZDefaultDirectory' path tree collectMDErrors ifBad
...
doZLoadMaybe pathfilter path tree ifExists = fsThunk tree $ doZLoadMaybe' pathfilter path tree ifExists
...
doZLoadMaybe' pathfilter path tree ifExists = debug ("doLoadMaybe' "++ show path) $ do
	...
	if exists
		then do
			...
			fmd' <- updateForestMDErrorsInsideWith tree fmd $ liftM (:[]) $ get_errors rep
			return (fmd',Just rep)
		else do
			fmd <- cleanForestMDwithFile tree path
			return (fmd,Nothing)
...
-- since the focus changes we need to compute the (eventually) previously loaded metadata of the parent node
doZLoadSimpleWithConstraint :: (ForestContent fs rep,ForestOutput fs ICThunk Inside,ForestMD fs rep,Matching fs a,MData NoCtx (ForestI fs) rep) =>
	FilePathFilter fs -> FilePath -> ForestI fs a -> FSTree fs -> (ForestContentTy fs rep -> ForestI fs Bool)
	-> (FilePath -> GetForestMD fs -> ForestI fs rep)
...
checkZPath path tree ifExists ifNotExists = fsThunk tree $ checkZPath' (Just False) path tree ifExists ifNotExists
