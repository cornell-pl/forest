...
doZManifestFile1 (marg :: ForestI fs arg) path tree rep_t man = do
	...
	let mani_scratch = do
		rep <- lift $ outside $ fsForce rep_t
		...
		newman <- if (exists && Pads.numErrors (get_md_header bmd) == 0)
			then do
				...
				(man1,printF) <- if repairMd
					then do
						Writer.tell $ \latest -> replaceForestMDErrorsWithPadsMD latest fmd (return $ get_md_header bmd) -- adds the Pads errors
				...
				man1 <- if repairMd
					then do
						Writer.tell $ \latest -> do
							replaceForestMDErrorsWith latest fmd $ return [missingPathForestErr path]
							updateForestMDErrorsWithPadsMD latest fmd (return $ get_md_header bmd) -- adds the Pads errors
						return man
...
doZManifestArchive isClosed archTy path tree toprep manifest manifestD diffValue man = do
	...
	let repProxy = Proxy
	(fmd,rep) <- lift $ outside $ fsForce toprep
...
--				else mani_scratch
			...
			let testm = do
				...
				status2 <- liftM (boolStatus $ ConflictingPath path path_fmd) $ latestTree >>= sameCanonicalFullPathInTree path_fmd path
				return $ status1 `mappend` status2
			Writer.tell $ \latest -> replaceForestMDErrorsWith latest fmd $ liftM (:[]) $ liftM (missingPathForestErr path `mergeForestErrs`) $ get_errors rep
...
doZManifestSymLink path tree (rep_t) man = do
	((fmd,base_md), tgt) <- lift $ outside $ fsForce rep_t
	path_fmd <- lift $ inside $ get_fullpath fmd
	...
	case exists of
		Just sym -> do
			let testm = do
				...
				status2 <- liftM (boolStatus $ ConflictingPath path path_fmd) $ latestTree >>= sameCanonicalFullPathInTree path_fmd path
				return $ status1 `mappend` status2
			Writer.tell $ \latest -> replaceForestMDErrorsWith latest fmd $ return [cleanForestErr]
		...
		Nothing -> do
			let testm = do
				...
				status2 <- liftM (boolStatus $ ConflictingPath path path_fmd) $ latestTree >>= sameCanonicalFullPathInTree path_fmd path
				return $ status1 `mappend` status2
			Writer.tell $ \latest -> replaceForestMDErrorsWith latest fmd $ return [ioExceptionForestErr]
...
doZManifestConstraint :: (ForestContent fs rep,IncK (IncForest fs) (ForestFSThunkI fs Forest_err, rep),ForestMD fs rep,ICRep fs) =>
	(ForestContentTy fs rep -> ForestI fs Bool) -> FSTree fs -> ForestFSThunkI fs (ForestFSThunkI fs Forest_err,rep)
	-> (rep -> Manifest fs -> MManifestForestO fs)
	-> Manifest fs -> MManifestForestO fs
doZManifestConstraint pred tree t manifestContent man = lift (outside $ fsForce t) >>= \v -> doZManifestConstraintInner pred tree v manifestContent man

doZManifestConstraintInner :: (ForestContent fs rep,ForestMD fs rep,ICRep fs) =>
	(ForestContentTy fs rep -> ForestI fs Bool) -> FSTree fs -> (ForestFSThunkI fs Forest_err,rep)
	-> (rep -> Manifest fs -> MManifestForestO fs)
	-> Manifest fs -> MManifestForestO fs
doZManifestConstraintInner pred tree (err_t,rep) manifestContent man = do
	isRepairMd <- Reader.ask
...
doZManifestDirectory :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,IncK (IncForest fs) (Forest_md fs, rep),ICRep fs) => 
	...
	-> (rep -> Manifest fs -> MManifestForestO fs)
	-> Manifest fs -> MManifestForestO fs
doZManifestDirectory path tree collectMDErrors rep_t manifestContent man = lift (outside $ fsForce rep_t) >>= \v -> doZManifestDirectoryInner path tree collectMDErrors v manifestContent man
...
doZManifestMaybe :: (IncK (IncForest fs) (Forest_md fs, Maybe rep),ForestMD fs rep,ICRep fs) =>
	...
	-> (rep -> Manifest fs -> MManifestForestO fs)
	-> Manifest fs -> MManifestForestO fs
doZManifestMaybe path tree rep_t manifestContent man = doZManifestMaybe' path tree rep_t (\t -> fsForce t >>= \(fmd,rep) -> return (Just fmd,rep)) manifestContent man
...
doZManifestMaybe' path tree toprep getRep manifestContent man = do
	...
	case mb_fmd of
		Nothing -> return ()
		Just fmd -> Writer.tell $ \latest -> get_errors_thunk fmd >>= flip (fsOverwrite latest) (maybe (return cleanForestErr) get_errors rep_mb)
		...
		Nothing -> do
			let testm = do
				...
				status2 <- case mb_fmd of
					Just fmd -> do
						path_fmd <- forestO $ get_fullpath fmd
						status21 <- if isRepairMd then return Valid else liftM (boolStatus ConflictingRepMd) $ forestO $ inside $ liftM (==fmd) $ cleanForestMDwithFile tree path
...
doZManifestSimpleWithConstraint :: (ForestContent fs rep,ForestMD fs rep,ICRep fs,Matching fs a) =>
	...
	-> (FilePath -> rep -> Manifest fs -> MManifestForestO fs)
	-> Manifest fs -> MManifestForestO fs
doZManifestSimpleWithConstraint parentPath matching tree pred dta manifestUnder = doZManifestConstraintInner pred tree dta $ \dta' man1 ->
...
doZManifestCompound parentPath matchingM tree mkeyarg toListRep c_rep manifestUnder man = do
	...
	repinfos' <- lift $ inside $ mapM (\rep -> fsThunk tree (get_info rep) >>= \fileInfo_t -> return (rep,fileInfo_t)) reps'
