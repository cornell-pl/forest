...
nextTxNILFSTree :: ForestM TxNILFS (FSTree TxNILFS)
nextTxNILFSTree = do
	TxNILFSEnv (starttime :!: txid :!: deltas :!: Strict.Cons txlog _) <- TxNILFSForestM Reader.ask
	(tree,ds,newtree,reads,bufftbl,memotbl,cantree,tmps) <- forestIO $ readIORef txlog
...
instance ICRep TxNILFS where
	...
	compareFSThunk t1 t2 = compare (txNILFSFSThunkId t1) (txNILFSFSThunkId t2)
	compareICThunk t1 t2 = error "no comparising for ICThunks"
...
newTxNILFSThunk :: (IncK (IncForest TxNILFS) a,TxNILFSLayer l) => FSTree TxNILFS -> l (IncForest TxNILFS) a -> ForestM TxNILFS (TxNILFSThunk l a)
newTxNILFSThunk tree m = do
...
readTxNILFSThunk :: (IncK (IncForest TxNILFS) a,AddTxNILFSParent l a,TxNILFSLayer l) => TxNILFSThunk l a -> ForestL TxNILFS l (a,FSTree TxNILFS)
readTxNILFSThunk t = do
	let eval buff = case buffTxNILFSData buff of
		TxNILFSThunkComp force -> do
			...
			deltatree <- addTxNILFSParent Proxy stone uid (buffTxNILFSTree buff) a
			debug ("evaluated " ++ show (typeRep t) ++ " " ++ show uid) $ return (buff { buffTxNILFSData = dta', buffTxNILFSDeltaTree = deltatree },(a,deltatree))
		TxNILFSThunkForce a stone -> return (buff,(a,buffTxNILFSTree buff))
...
writeTxNILFSThunk :: (IncK (IncForest TxNILFS) a,AddTxNILFSParent l a,TxNILFSLayer l) => FSTree TxNILFS -> TxNILFSThunk l a -> a -> ForestO TxNILFS ()
writeTxNILFSThunk newtree var a = do
	let set buff = do
		stone <- forestM $ forestIO $ newIORef ()
		let dta' = TxNILFSThunkForce a stone
...
overwriteTxNILFSThunk :: (IncK (IncForest TxNILFS) a,AddTxNILFSParent l a,TxNILFSLayer l) => FSTree TxNILFS -> TxNILFSThunk l a -> l (IncForest TxNILFS) a -> ForestO TxNILFS ()
overwriteTxNILFSThunk newtree var ma = do
	let set buff = do
...
instance ForestInput TxNILFS FSThunk Inside where
...
instance ForestInput TxNILFS FSThunk Outside where
	...
	fsForce var = do
		thunk <- forestM $ bufferedTxNILFSFSThunk var
		liftM Prelude.fst $ readTxNILFSThunk thunk
	fsSet tree (var :: ForestFSThunk TxNILFS Outside a) v = do
...
deleteTxNILFS proxy (rep :: rep) = do
	...
	case mb of
		...
		Just (args,path) -> do
			tree <- forestM latestTree
			def_rep :: rep <- inside $ zdefaultScratchMemo proxyRep proxyTxNILFS args path tree
			let t = to (iso_rep_thunk proxyTxNILFS) def_rep
...
copyOrElseTxNILFS proxy src tgt b f = do
	...
	srctree <- inside $ fsTree $ to (iso_rep_thunk proxyTxNILFS) src
	case (mb_src,mb_tgt) of
		(Just (args_src,path_src),Just (args_tgt,path_tgt)) -> do
			...
			tgt' <- copyFSThunks proxyTxNILFS proxyOutside tree chgPath src
			let tgtt' = to (iso_rep_thunk proxyTxNILFS) tgt'
...
loadTxNILFS proxy rep = do
	...
	inside $ unsafeWorld $ debug ("loadTxNILFS " ++ show path ++" "++ show uid) $ do
		...
		-- we are updating modifiables at the inner layer; should be fine, since we don't write to the FS, nor update the latest tree
...
writeOrElseTxNILFS (rep::rep) content b f = do
	...
	case mb of
		...
		Just (txargs,path) -> do
			...
			let tryWrite = do
				nexttree <- forestM nextTxNILFSTree
