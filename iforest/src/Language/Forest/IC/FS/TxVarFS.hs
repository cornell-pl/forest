...
instance (ForestLayer TxVarFS l) => ForestInput TxVarFS FSThunk l where
	fsRef tree c = fsThunk tree (return c)
	fsThunk tree m = do
		rdyn <- forestM $ forestIO $ newIORef (toDyn (),"--NOFILE--")
		tbl <- forestM $ forestIO $ WeakMap.new
		return $ TxVarFSThunk (rdyn,m,tbl)
	fsForce (TxVarFSThunk (rdyn,m,tbl)) = do -- always reads from the latest file system version
		fsversion <- forestM getFSVersionTxVarFS
		mb <- forestM $ forestIO $ WeakMap.lookup tbl fsversion
		case mb of
			Nothing -> do
...
deleteTxVarFS (rep :: rep) = do
	...
	case mb of
		...
		Just (args,path) -> do
			def_rep :: rep <- inside $ zdefaultScratchMemo proxyRep proxyTxVarFS args path TxVarFSTree
			var <- outside $ fsForce $ to (iso_rep_thunk proxyTxVarFS) def_rep
--			str <- showInc def_rep
...
copyOrElseTxVarFS proxy src tgt b f = do
	...
	srctree <- inside $ fsTree $ to (iso_rep_thunk proxyTxVarFS) src
	case (mb_src,mb_tgt) of
		(Just (args_src,path_src),Just (args_tgt,path_tgt)) -> do
			...
			tgt' <- copyFSThunks proxyTxVarFS proxyOutside tree chgPath src
			let tgtt' = to (iso_rep_thunk proxyTxVarFS) tgt'
...
readTxVarFS proxy (rep :: rep) = do
	...
	case mb of
		Nothing -> do
			...
			v' <- inside $ fsForce (to (iso_rep_thunk proxyTxVarFS) rep')
			forestM $ forestIO $ WeakMap.insertWithMkWeak tbl (MkWeak $ Weak.mkWeakRefKey rdyn) fsversion v'
			inside $ runReaderT (BX.getM lens_content (return v')) TxVarFSTree
		Just v -> inside $ runReaderT (BX.getM lens_content (return v)) TxVarFSTree
...
writeOrElseTxVarFS rep content b f = do
	t <- inside $ runReaderT (BX.putM lens_content (lift $ fsForce $ to (iso_rep_thunk proxyTxVarFS) rep) (return content)) TxVarFSTree
