...
type TxICFSLogs = Strict.List TxICFSLog
...
bufferTxICFSThunk var thunk = do
	TxICFSEnv (starttime :!: txid :!: deltas :!: Strict.Cons txlog _) <- TxICFSForestM Reader.ask
	(tree,ds,newtree,reads,bufftbl,memotbl,cantree,tmps) <- forestIO $ readRef txlog
...
nextTxICFSTree :: ForestM TxICFS (FSTree TxICFS)
nextTxICFSTree = do
	TxICFSEnv (starttime :!: txid :!: deltas :!: Strict.Cons txlog _) <- TxICFSForestM Reader.ask
...
instance FSRep TxICFS where
	...
	unsafeInterleaveForestM (TxICFSForestM io) = TxICFSForestM $ Reader.mapReaderT unsafeInterleaveIO io
	
	runForest (TxICFSForestCfg root) (TxICFSForestM m) = atomicallyTxICFS root (TxICFSForestO m)
...
instance Incremental (IncForest TxICFS) where
	...
	newtype IncParams (IncForest TxICFS) = TxICFSIncParams (ForestCfg TxICFS)

	runIncrementalWithParams (TxICFSIncParams cfg) (TxICFSForestO m) = runForest cfg (TxICFSForestM m)
	defaultIncParams = TxICFSIncParams $ TxICFSForestCfg "/"
...
instance ICRep TxICFS where
	...
	compareFSThunk t1 t2 = compare (txICFSThunkId t1) (txICFSThunkId t2)
	compareICThunk t1 t2 = error "no ICThunk comparison"

-- tests for equality
diffValueThunkTxICFS :: (Typeable rep,IncK (IncForest TxICFS) (ForestRepTy TxICFS rep),Typeable (ForestRepTy TxICFS rep),ForestRep TxICFS rep) => Bool -> FSTree TxICFS -> rep -> ForestO TxICFS (ValueDelta TxICFS rep)
diffValueThunkTxICFS isTop oldtree rep = do
		thunk <- forestM $ bufferedTxICFSThunk $ to (iso_rep_thunk proxyTxICFS) rep
...
readTxICThunk :: (IncK (IncForest TxICFS) a,AddTxICParent l a,TxICLayer l) => TxICThunk l a -> ForestL TxICFS l (a,FSTree TxICFS)
readTxICThunk t = do
	let eval buff = case buffTxICData buff of
		TxICThunkComp force -> do
			...
			deltatree <- addTxICParent Proxy stone uid (buffTxICTree buff) a
			return (buff { buffTxICData = dta', buffTxICDeltaTree = deltatree },(a,deltatree))
		TxICThunkForce a stone -> return (buff,(a,buffTxICTree buff))
...
writeTxICThunk :: (IncK (IncForest TxICFS) a,AddTxICParent l a,TxICLayer l) => FSTree TxICFS -> TxICThunk l a -> a -> ForestO TxICFS ()
writeTxICThunk newtree var a = do
	let set buff = do
		stone <- forestM $ forestIO $ newIORef ()
		let dta' = TxICThunkForce a stone
...
overwriteTxICThunk :: (IncK (IncForest TxICFS) a,AddTxICParent l a,TxICLayer l) => FSTree TxICFS -> TxICThunk l a -> l (IncForest TxICFS) a -> ForestO TxICFS ()
overwriteTxICThunk newtree var ma = do
	let set buff = do
...
instance ZippedICMemo TxICFS where
	...
	addZippedMemo path args (rep :: rep) mb_tree = debug ("added memo "++show path ++ show mb_tree) $ do
		let proxy = Proxy :: Proxy (ForestArgs rep)
		let proxyRep = proxyOf rep
		let var = to (iso_rep_thunk proxyTxICFS) rep
		TxICFSEnv (starttime :!: txid :!: deltas :!: Strict.Cons txlog _) <- txICFSLayer Proxy Reader.ask
		...
		let load = do
			...
			(t :: rep) <- zloadScratch proxyRep args return path latest getForestMDInTree
			fsForce (to (iso_rep_thunk proxyTxICFS) t)
	...
	findZippedMemo path (proxyRep :: Proxy rep) = do
		let proxy = Proxy :: Proxy (ForestArgs rep)
		TxICFSEnv (starttime :!: txid :!: deltas :!: Strict.Cons txlog _) <- TxICFSForestI Reader.ask
		(tree,ds,newtree,reads,bufftbl,memotbl,cantree,tmps) <- forestM $ forestIO $ readRef txlog
		mb <- forestM $ findTxICMemo path proxyRep
		case mb of
			Nothing -> return Nothing
...
argsTxICFS :: (FTK TxICFS rep) => Proxy (ForestArgs rep) -> rep -> ForestI TxICFS (ForestVs (ForestArgs rep),FilePath)
...
getFTVArgsIC :: (FTK TxICFS rep) => Proxy (ForestArgs rep) -> rep -> ForestM TxICFS (Maybe (ForestIs TxICFS (ForestArgs rep),FilePath))
...
instance Thunk (HSThunk TxICFS) Inside (IncForest TxICFS) where
	...
	read var = do
		...
		liftM Prelude.fst $ readTxICThunk thunk
...
instance ForestInput TxICFS FSThunk Inside where
	...
	fsForce var = do
		thunk <- forestM $ bufferedTxICFSThunk var
		liftM Prelude.fst $ readTxICThunk thunk
	fsSet tree (var :: ForestFSThunk TxICFS Inside a) v = do
		(thunk :: TxICThunk Inside a) <- forestM $ bufferedTxICFSThunk var
		writeTxICThunk tree thunk v
	fsOverwrite tree (var :: ForestFSThunk TxICFS Inside a) mv = do
		(thunk :: TxICThunk Inside a) <- forestM $ bufferedTxICFSThunk var
		overwriteTxICThunk tree thunk mv
	fsTree = treeTxICFSThunk
instance ForestInput TxICFS FSThunk Outside where
	fsRef tree c = fsThunk tree (return c)
	fsThunk tree m = do
		uid <- forestM $ forestIO newUnique
		txargs <- forestM $ forestIO $ newIORef Nothing
	...
	fsForce var = do
		thunk <- forestM $ bufferedTxICFSThunk var
		liftM Prelude.fst $ readTxICThunk thunk
	fsSet tree (var :: ForestFSThunk TxICFS Outside a) v = do
		(thunk :: TxICThunk Outside a) <- forestM $ bufferedTxICFSThunk var
		writeTxICThunk tree thunk v
	fsOverwrite tree (var :: ForestFSThunk TxICFS Outside a) mv = do
		(thunk :: TxICThunk Outside a) <- forestM $ bufferedTxICFSThunk var
		overwriteTxICThunk tree thunk mv
	fsTree = treeTxICFSThunk
...
deleteTxICFS :: FTK TxICFS rep => rep -> TxICFTM ()
deleteTxICFS (rep :: rep) = do
	let proxy = Proxy :: Proxy (ForestArgs rep)
	let proxyRep = proxyOf rep
	mb <- forestM $ getFTVArgsIC proxy rep
	case mb of
		...
		Just (args,path) -> do
			tree <- forestM latestTree
			def_rep :: rep <- inside $ zdefaultScratchMemo proxyRep proxyTxICFS args path tree
			let t = to (iso_rep_thunk proxyTxICFS) def_rep
			ttree <- inside $ fsTree t
			content <- inside $ runReaderT (BX.getM lens_content $ lift $ fsForce t) ttree
			writeOrElseTxICFS rep content () (error "failed to write")

copyOrElseTxICFS :: (FTK TxICFS rep) => Proxy (ForestArgs rep) -> rep -> rep -> b -> ([ManifestError] -> TxICFTM b) -> TxICFTM b
copyOrElseTxICFS proxy src tgt b f = do
	mb_src <- forestM $ getFTVArgsIC proxy src
	mb_tgt <- forestM $ getFTVArgsIC proxy tgt
	srctree <- inside $ fsTree $ to (iso_rep_thunk proxyTxICFS) src
	case (mb_src,mb_tgt) of
		(Just (args_src,path_src),Just (args_tgt,path_tgt)) -> do
			...
			tgt' <- copyFSThunks proxyTxICFS proxyOutside tree chgPath src
			let tgtt' = to (iso_rep_thunk proxyTxICFS) tgt'
			tgttree' <- inside $ fsTree tgtt'
			content' <- inside $ runReaderT (BX.getM lens_content $ lift $ fsForce tgtt') tgttree'
			writeOrElseTxICFS tgt content' b f
		otherwise -> error "tried to write to a variable that is not connected to the FS"

-- new variables always load from the latest tree: @zload@
newTxICFS :: FTK TxICFS rep => Proxy (ForestArgs rep) -> ForestVs (ForestArgs rep) -> FilePath -> TxICFTM rep
...
readTxICFS :: (ForestLayer TxICFS l,FTK TxICFS rep) => rep -> ForestL TxICFS l (ForestContentTy TxICFS (ForestRepTy TxICFS rep))
readTxICFS (rep::rep) = do
	let proxy = Proxy :: Proxy (ForestArgs rep)
	let t = to (iso_rep_thunk proxyTxICFS) rep
	...
	res <- inside $ runReaderT (BX.getM lens_content $ lift $ fsForce t) oldtree
	return res

data ManifestConflictTx = ManifestConflictTx [ManifestError] deriving (Show,Eq,Typeable)
instance Exception ManifestConflictTx

writeOrElseTxICFS :: FTK TxICFS rep => rep -> ForestContentTy TxICFS (ForestRepTy TxICFS rep) -> b -> ([ManifestError] -> TxICFTM b) -> TxICFTM b
writeOrElseTxICFS (rep :: rep) content b f = do

	let t = to (iso_rep_thunk proxyTxICFS) rep
		
	mb :: (Maybe (ForestIs TxICFS (ForestArgs rep),FilePath)) <- forestM $ getFTVArgsIC Proxy rep
	case mb of
		...
		Just (txargs,path) -> debug ("writeOrElseTxICFS " ++ show path) $ do
			...
			let tryWrite = do
				...
				nexttree <- forestM nextTxICFSTree
				-- write the new content
				fsModify nexttree t $ \s -> runReaderT (BX.putM lens_content (return s) (return content)) nexttree
				...
				(mani,_,memos) <- debug ("storing... " ++ show path) $ RWS.runRWST (zupdateManifestDeltaMemo newdeltas path path tree (emptyFSTreeD proxyTxICFS) tree rep newdv man) True ()
...
initializeTxICFS (TxICFSForestO m) = do
	...
	debug ("initializeTxICFS") $ Reader.runReaderT m $ TxICFSEnv (starttime :!: txid :!: deltas :!: Strict.Cons txlog Strict.Nil)
...
resetTxICFS m = do
	...
	debug ("resetTxICFS") $ TxICFSForestO $ Reader.local (Prelude.const $ TxICFSEnv (now :!: txid :!: deltas :!: Strict.Cons txlog Strict.Nil)) $ runTxICFSForestO m
...
startNestedTxICFS m = do
	TxICFSEnv (starttime :!: txid :!: deltas :!: txlogs@(Strict.Cons txlog_parent _)) <- TxICFSForestO Reader.ask
	...
	debug ("startNestedTxICFS ") $ TxICFSForestO $ Reader.local (Prelude.const $ TxICFSEnv (starttime :!: txid :!: deltas :!: Strict.Cons txlog_child txlogs)) $ runTxICFSForestO m
...
dropChildTxICLog m = do
	TxICFSEnv (starttime :!: txid :!: deltas :!: (Strict.Cons txlog_child txlogs@(Strict.Cons txlog_parent _))) <- TxICFSForestO Reader.ask
...
validateAndCommitTopTxICFS doWrites = atomicTxICFS "validateAndCommitTopTxICFS" $ do
	txenv@(TxICFSEnv (timeref :!: txid :!: deltas :!: txlogs@(Strict.Cons txlog Strict.Nil))) <- TxICFSForestO Reader.ask
...
validateAndCommitNestedTxICFS mbException = do
	txenv@(TxICFSEnv (timeref :!: txid :!: deltas :!: txlogs@(Strict.Cons txlog1 (Strict.Cons txlog2 _)))) <- TxICFSForestO Reader.ask
	starttime <- unsafeIOToInc $ readIORef timeref
	case mbException of
...
parentCanonicalTree Strict.Nil = return $ CanonicalTree Map.empty
...
parentSyms Strict.Nil = forestIO $ liftM (DList.empty,) $ readMVar symlinks
parentSyms (Strict.Cons txlog _) = do
	(tree,delta,newtree,reads,bufftbl,memotbl,cantree,tmps) <- forestIO $ readRef txlog
...
unbufferTxICFSWrites = do
	TxICFSEnv (starttime :!: txid :!: deltas :!: txlogs@(Strict.Cons txlog1 txlogs_parent)) <- TxICFSForestM Reader.ask
...
checkTxsICFS Strict.Nil finished = return True
checkTxsICFS env@(Strict.Cons txlog txlogs) finished = do
...
checkTxICFS txlog wrts = liftM List.and $ Prelude.mapM (checkTxICFS' txlog) wrts where
	checkTxICFS' txlog (txtime,paths) = do
		TxICFSEnv (starttime_ref :!: txid :!: _ :!: Strict.Cons txlog _) <- TxICFSForestM Reader.ask
...
atomicTxICFS msg m = do
	...
	x <- txICFSLayer proxyOutside $ withFileLocks reads writes $ unTxICFSLayer proxyOutside $ forestM (forestIO $ print $ "entered atomic " ++ msg) >> m
