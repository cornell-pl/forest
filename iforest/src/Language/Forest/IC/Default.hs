...
class (FSRep fs,ForestLayer fs l) => CopyFSThunks fs l a where
	copyFSThunks :: Proxy fs -> Proxy l -> FSTree fs -> (FilePath -> FilePath) -> a -> ForestL fs l a

copyFSThunksProxy :: Proxy fs -> Proxy l -> Proxy (CopyFSThunksDict fs l)
copyFSThunksProxy fs l = Proxy

data CopyFSThunksDict fs l a = CopyFSThunksDict { copyFSThunksDict :: Proxy fs -> Proxy l -> FSTree fs -> (FilePath -> FilePath) -> a -> ForestL fs l a }
...
instance (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestInput fs FSThunk Inside,ForestLayer fs l) => CopyFSThunks fs l (Forest_md fs) where
	copyFSThunks _ _ tree f fmd = do
		errors' <- inside $ fsForce (errors fmd) >>= fsRef tree
		fileInfo' <- inside $ fsForce (fileInfo fmd) >>= fsRef tree . (\i -> i { fullpath = f (fullpath i) })
