...
mkNewTypeForestMDEC needsEC ecName fsName ty_nameEC ty_nameE ty_nameC (unty_name,ty_name) rep = do
	...
	let inst = [
		InstanceD ctx (appT2 (ConT ''ForestMD) (VarT fsName) (appT2 (ConT ty_nameEC) (PromotedT 'E) (VarT fsName))) [
			...
			,FunD 'replace_fmd_header [Clause [VarP tree,ConP ty_name [VarP x],VarP f] (NormalB $ AppE (AppE (VarE 'liftM) $ ConE ty_name) $ appE3 (VarE 'replace_fmd_header) (VarE tree) (VarE x) (VarE f)) []]
			]
		]
	return inst
...
instance (arg ~ PadsArg pads,
	MData (CopyFSThunksDict fs Outside) (Inside (IncForest fs)) pads,
	MData (CopyFSThunksDict fs Outside) (Inside (IncForest fs)) md,
	MData NoCtx (Inside (IncForest fs)) pads,
	MData NoCtx (Inside (IncForest fs)) md,
