...
getForestMD :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs,ForestLayer fs l,ForestInput fs FSThunk Inside) => FSTree fs -> FilePath -> FilePath -> ForestL fs l (Forest_md fs)
getForestMD tree oripath diskpath = do
	
	info <- inside $ fsThunk tree $ do
...
forest_md_def :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestInput fs FSThunk Inside,ForestLayer fs l) => FSTree fs -> ForestL fs l (Forest_md fs)
forest_md_def tree = do
	err <- inside $ fsRef tree $ forest_err_def
	info <- inside $ fsRef tree $ fileInfo_def
	return $ Forest_md err info
...
fmd_symLink md = liftM symLink $ inside $ fsForce $ fileInfo md

-- we need to actually create new thunks to allow the modification to occur at the inner layer
modify_errors_under :: (IncK (IncForest fs) md,ForestMD fs md) => FSTree fs -> ForestFSThunk fs Inside md -> (Forest_err -> ForestI fs Forest_err) -> ForestO fs ()
modify_errors_under tree t newerrs = fsModify tree t $ \md -> replace_errors tree md newerrs
...
class (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs,ForestInput fs FSThunk Inside,ForestLayer fs Outside) => ForestMD fs md where
	...
	replace_fmd_header :: FSTree fs -> md -> (Forest_md fs -> ForestI fs (Forest_md fs)) -> ForestI fs md
	...
	replace_errors :: FSTree fs -> md -> (Forest_err -> ForestI fs Forest_err) -> ForestI fs md
	replace_errors tree md f = do
		replace_fmd_header tree md $ \fmd -> do
			t <- fsThunk tree $ f =<< get_errors fmd
			return $ fmd { errors = t }
	modify_errors :: FSTree fs -> md -> (Forest_err -> ForestI fs Forest_err) -> ForestO fs ()
	modify_errors tree md newerrs = do
		fmd <- get_fmd_header md
		let err_thunk = errors fmd
		fsModify tree err_thunk newerrs
	overwrite_errors :: FSTree fs -> md -> ForestI fs Forest_err -> ForestO fs ()
	overwrite_errors tree md newerrs = do
		fmd <- get_fmd_header md
		let err_thunk = errors fmd
		fsOverwrite tree err_thunk newerrs
	
	get_info :: ForestLayer fs l => md -> ForestL fs l FileInfo
	get_info md = inside . fsForce . fileInfo =<< get_fmd_header md
	get_fullpath :: ForestLayer fs l => md -> ForestL fs l String
	get_fullpath md = return . fullpath =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_owner :: ForestLayer fs l => md -> ForestL fs l String
	get_owner md = return . owner =<< inside . fsForce =<< return .  fileInfo =<< get_fmd_header md
	get_group :: ForestLayer fs l => md -> ForestL fs l String
	get_group md = return . group =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_size :: ForestLayer fs l => md -> ForestL fs l COff
	get_size md = return . size =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_access_time :: ForestLayer fs l => md -> ForestL fs l EpochTime
	get_access_time md = return . access_time =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_mod_time :: ForestLayer fs l => md -> ForestL fs l EpochTime
	get_mod_time md = return . mod_time =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_read_time :: ForestLayer fs l => md -> ForestL fs l EpochTime
	get_read_time md = return . read_time =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_mode :: ForestLayer fs l => md -> ForestL fs l FileMode
	get_mode md = return . mode =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_modes :: ForestLayer fs l => md -> ForestL fs l String
	get_modes md = return . modeToModeString . mode =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_symLink :: ForestLayer fs l => md -> ForestL fs l (Maybe FilePath)
	get_symLink md = return . symLink =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md
	get_kind :: ForestLayer fs l => md -> ForestL fs l FileType
	get_kind md = return . kind =<< inside . fsForce =<< return . fileInfo =<< get_fmd_header md

change_fmd_header :: (ForestLayer fs l,IncK (IncForest fs) (Forest_md fs, md),ICRep fs,ForestInput fs FSThunk Inside,ForestInput fs FSThunk l) => FSTree fs -> ForestFSThunk fs l (Forest_md fs,md) -> Forest_md fs -> ForestO fs ()
change_fmd_header tree t fmd' = fsModify tree t $ \(fmd,md) -> return (fmd',md)
...
instance (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs,ForestInput fs FSThunk Inside,ForestLayer fs Outside) => ForestMD fs (Forest_md fs) where
	...
	replace_fmd_header tree fmd f = f fmd
...
instance (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs,ForestInput fs FSThunk Inside,ForestLayer fs Outside) => ForestMD fs (Forest_md fs,b) where
	...
	replace_fmd_header tree (fmd,b) f = replace_fmd_header tree fmd f >>= \fmd' -> return (fmd',b)
instance (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs,ForestInput fs FSThunk Inside,ForestLayer fs Outside) => ForestMD fs ((Forest_md fs,bmd),b) where
	...
	replace_fmd_header tree ((fmd,bmd),b) f = replace_fmd_header tree fmd f >>= \fmd' -> return ((fmd',bmd),b)
...
instance (ForestMD fs (ForestFSThunk fs l a),Eq a,MData NoCtx (ForestL fs l) b,ForestMD fs a) => ForestMD fs (ForestFSThunk fs l a,b) where
	...
	get_fmd_header (a,b) = get_fmd_header a
	replace_fmd_header tree (t,b) f = replace_fmd_header tree t f >>= \t' -> return (t',b)
...
instance (ForestMD fs a) => ForestMD fs ((a,b),c) where
	...
	replace_fmd_header tree ((a,b),c) f = replace_fmd_header tree a f >>= \a' -> return ((a',b),c)
...
instance (IncK (IncForest fs) a,ForestMD fs a,ForestInput fs FSThunk Inside,ForestLayer fs Outside) => ForestMD fs (ForestFSThunk fs Inside a) where
	...
	get_fmd_header t = do
		md <- inside (fsForce t)
		get_fmd_header md
	replace_fmd_header tree t f = fsThunk tree $ fsForce t >>= \v -> replace_fmd_header tree v f
...
instance (ForestRep fs rep,ForestMD fs (ForestFSThunkI fs (ForestRepTy fs rep))) => ForestMD fs rep where
	...
	replace_fmd_header tree r (f :: Forest_md fs -> ForestI fs (Forest_md fs)) = do
		let fs = Proxy :: Proxy fs
		liftM (from $ iso_rep_thunk fs) $ replace_fmd_header tree (to (iso_rep_thunk fs) r) f
...
instance (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestMD fs rep) => ForestMD fs (Maybe rep) where
	get_fmd_header Nothing = forestM latestTree >>= \tree -> inside (cleanForestMD tree)
...
instance (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs) => ForestMD fs (FileInfo,b) where
	get_fmd_header (info,_) = forestM latestTree >>= \tree -> inside (fsRef tree info) >>= return . Forest_md (error "noForest_err")
instance (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs) => ForestMD fs ((FileInfo,bmd),b) where
	get_fmd_header ((info,_),_) = forestM latestTree >>= \tree -> inside (fsRef tree info) >>= return . Forest_md (error "noForest_err")
...
cleanForestMD :: (ForestLayer fs Inside,IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestInput fs FSThunk Inside) => FSTree fs -> ForestI fs (Forest_md fs)
cleanForestMD tree = do
	err <- fsRef tree $ cleanForestMDErr
	info <- fsRef tree $ fileInfo_def
	return $ Forest_md err info
cleanForestMDErr = Forest_err {numErrors = 0, errorMsg = Nothing }

errorForestMD :: (ForestLayer fs Inside,IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestInput fs FSThunk Inside) => FSTree fs -> ForestI fs (Forest_md fs)
errorForestMD tree = do
	err <- fsRef tree $ errorForestMDErr
	info <- fsRef tree $ errorFileInfo
	return $ Forest_md { errors = err, fileInfo = info }
missingPathForestMD tree path = do
	err <- fsRef tree $ missingPathForestErr path
	info <- fsRef tree $ mkErrorFileInfo path
	return $ Forest_md { errors = err, fileInfo = info }
missingDirForestMD tree path = do
	err <- fsRef tree $ missingDirForestErr path
	info <- fsRef tree $ mkErrorFileInfo path
	return $ Forest_md {errors = err, fileInfo = info }
fileMatchFailureForestMD tree path = do
	err <- fsRef tree $ fileMatchFailureForestErr path
	info <- fsRef tree $ mkErrorFileInfo path
	return $ Forest_md {errors = err, fileInfo = info }
systemErrorForestMD tree i = do
	err <- fsRef tree $ systemErrorForestErr i
	info <- fsRef tree $ errorFileInfo
	return $ Forest_md {errors = err, fileInfo = info }
notDirectoryForestMD tree path = do
	err <- fsRef tree $ notDirectoryForestErr path
	info <- fsRef tree $ mkErrorFileInfo path
	return $ Forest_md {errors = err, fileInfo = info }
constraintViolationForestMD :: (ForestLayer fs Inside,FSRep fs,IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestInput fs FSThunk Inside) => FSTree fs -> ForestI fs (Forest_md fs)
constraintViolationForestMD tree = do
	err <- fsRef tree $ constraintViolationForestErr
	info <- fsRef tree $ errorFileInfo
	return $ Forest_md {errors = err, fileInfo = info }
ioExceptionForestMD :: (ForestLayer fs Inside,FSRep fs,IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestInput fs FSThunk Inside) => FSTree fs -> ForestI fs (Forest_md fs)
ioExceptionForestMD tree = do
	err <- fsRef tree $ ioExceptionForestErr
	info <- fsRef tree $ errorFileInfo
	return $ Forest_md { errors = err, fileInfo = info }
ioExceptionForestMDwithFile :: (ForestLayer fs Inside,FSRep fs,IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ForestInput fs FSThunk Inside) => FSTree fs -> FilePath -> ForestI fs (Forest_md fs)
ioExceptionForestMDwithFile tree path = do
	err <- fsRef tree $ ioExceptionForestErr
	info <- fsRef tree $ mkErrorFileInfo path
	return $ Forest_md { errors = err, fileInfo = info }
...
cleanForestMDwithFile :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs,ForestLayer fs l,ForestInput fs FSThunk Inside) => FSTree fs -> FilePath -> ForestL fs l (Forest_md fs)
cleanForestMDwithFile tree path = inside $ do
	abspath <- forestM $ forestIO $ absolutePath path
	err <- fsRef tree $ cleanForestMDErr
	info <- fsRef tree $ fileInfo_def { fullpath = abspath }
	return $ Forest_md err info
...
cleanSymLinkForestMDwithFile :: (IncK (IncForest fs) FileInfo,IncK (IncForest fs) Forest_err,ICRep fs,ForestLayer fs l,ForestInput fs FSThunk Inside) => FSTree fs -> FilePath -> FilePath -> ForestL fs l (Forest_md fs)
cleanSymLinkForestMDwithFile tree path tgt = inside $ do
	abspath <- forestM $ forestIO $ absolutePath path
	err <- fsRef tree $ cleanForestMDErr
	info <- fsRef tree $ fileInfo_def { fullpath = abspath, symLink = Just tgt }
	return $ Forest_md err info
...
addMultipleMatchesErrorMD :: (ForestMD fs md) => FSTree fs -> FilePath -> [String] -> md -> ForestO fs ()
addMultipleMatchesErrorMD tree path names md = modify_errors tree md $ \olderrors -> do
...
addMultipleMatchesErrorMDInside :: (ForestMD fs md) => FSTree fs -> FilePath -> [String] -> md -> ForestI fs md
addMultipleMatchesErrorMDInside tree path names md = replace_errors tree md $ \olderrors -> do
	let errMsg' = case errorMsg olderrors of
		Nothing -> MultipleMatches path names
		Just e ->  e
	return $ Forest_err { numErrors = numErrors olderrors + 1, errorMsg = Just errMsg' }

updateForestMDErrorsWith :: ForestMD fs md => FSTree fs -> md -> ForestI fs [Forest_err] -> ForestO fs ()
updateForestMDErrorsWith tree md get_errs = modify_errors tree md $ \err0 -> get_errs >>= \errs -> return $ updateForestErr err0 errs

updateForestMDErrorsWithPadsMD :: (PadsMD pads_md,ForestMD fs md) => FSTree fs -> md -> ForestI fs pads_md -> ForestO fs ()
updateForestMDErrorsWithPadsMD tree md get_errs = modify_errors tree md $ \err0 -> get_errs >>= \errs -> return $ updateForestErr err0 [padsError $ get_md_header errs]

replaceForestMDErrorsWithPadsMD :: (PadsMD pads_md,ForestMD fs md) => FSTree fs -> md -> ForestI fs pads_md -> ForestO fs ()
replaceForestMDErrorsWithPadsMD tree md get_errs = modify_errors tree md $ \err0 -> get_errs >>= \errs -> return $ padsError $ get_md_header errs

updateForestMDErrorsInsideWith :: ForestMD fs md => FSTree fs -> md -> ForestI fs [Forest_err] -> ForestI fs md
updateForestMDErrorsInsideWith tree md get_errs = replace_errors tree md $ \err0 -> get_errs >>= \errs -> return $ updateForestErr err0 errs

updateForestErr :: Forest_err -> [Forest_err] -> Forest_err
updateForestErr err errs = List.foldl' mergeForestErrs err errs

updateForestMDErrorsInsideWithPadsMD :: (PadsMD pads_md,ForestMD fs md) => FSTree fs -> md -> ForestI fs pads_md -> ForestI fs md
updateForestMDErrorsInsideWithPadsMD tree md get_errs = replace_errors tree md $ \err0 -> get_errs >>= \errs -> return $ updateForestErr err0 [padsError $ get_md_header errs]

replaceForestMDErrorsWith :: (IncK (IncForest fs) Forest_err,ForestMD fs md) => FSTree fs -> md -> ForestI fs [Forest_err] -> ForestO fs ()
replaceForestMDErrorsWith tree md get_errs = overwrite_errors tree md $ do
	errs <- get_errs
	return $ mergeMDErrors errs

replaceForestMDErrorsInsideWith :: ForestMD fs md => FSTree fs -> md -> ForestI fs [Forest_err] -> ForestI fs md
replaceForestMDErrorsInsideWith tree md get_errs = replace_errors tree md $ \err0 -> get_errs >>= return . mergeMDErrors
...
getForestMDInTree path tree = forestM (pathInTree path tree) >>= getForestMD tree path
