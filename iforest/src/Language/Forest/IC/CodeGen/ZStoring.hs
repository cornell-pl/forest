...
zmanifestWithArgsE isTop ty_name [] pathE treeE dtaE manE = do
	...
	return $ appE5 manifest (TupE []) pathE treeE dtaE manE
zmanifestWithArgsE isTop ty_name argsE pathE treeE dtaE manE = do
	...
	let manifest = if isTop then VarE 'doZManifestNamed else VarE 'zupdateManifestScratchGeneric
	return $ appE5 manifest tupArgsE pathE treeE dtaE manE
...
zmanifestConstraint isTop treeE pat predE dtaE manE manifest = forceVarsZEnvQ predE $ \predE' -> do
	...
	if isTop
		then return $ appE5 (VarE 'doZManifestConstraint) predFnE treeE dtaE manifestAction manE
		else return $ appE5 (VarE 'doZManifestConstraintInner) predFnE treeE dtaE manifestAction manE
...
zmanifestSimple (internal, isForm, externalE, forestTy, predM) treeE parentPathE dtaE man0E man1P = do
	...
	let letRepS = [BindS varP (AppE (VarE 'lift) $ AppE (VarE 'inside) $ appE3 (VarE 'BX.getMTree) (VarE 'lens_content) treeE $ returnExp innerRepE),LetS [ValD repP (NormalB varE) []]]
...
zmanifestCompound isNested (CompField internal tyConNameOpt explicitName externalE descTy generatorP generatorTy generatorG predM) treeE parentPathE dtaE man0E man1P = do
	...
	let innerRepE = AppE (VarE 'lift) $ AppE (VarE 'inside) $ appE3 (VarE 'BX.getMTree) (VarE 'lens_content) treeE $ returnExp $ if isNested then AppE repE dtaE else dtaE
