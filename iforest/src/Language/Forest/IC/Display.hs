...
instance (FSRep fs,IncK (IncForest fs) a,Display Outside (IncForest fs) a,ForestInput fs FSThunk l) => Display Outside (IncForest fs) (FSThunk fs l (IncForest fs) a) where
	displaysPrec l inc t rest = outside (fsForce t) >>= \x ->  displaysPrec l inc x rest
	{-# INLINE displaysPrec #-}

instance (FSRep fs,IncK (IncForest fs) a,Display Inside (IncForest fs) a,ForestInput fs FSThunk Inside) => Display Inside (IncForest fs) (FSThunk fs Inside (IncForest fs) a) where
	displaysPrec l inc t rest = fsForce t >>= \x -> displaysPrec l inc x rest
	{-# INLINE displaysPrec #-}
