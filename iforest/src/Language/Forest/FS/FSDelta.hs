...
unfocusFSDelta :: FilePath -> FSDelta -> FSDelta
unfocusFSDelta rel (Add path dsk) = Add (makeRelative rel path) dsk
unfocusFSDelta rel (AddLink path tgt) = AddLink (makeRelative rel path) tgt
unfocusFSDelta rel (Rem path) = Rem (makeRelative rel path)
unfocusFSDelta rel (Move path1 path2 dsk) = Move (makeRelative rel path1) (makeRelative rel path2) dsk
unfocusFSDelta rel (ChgAttrs path dsk) = ChgAttrs (makeRelative rel path) dsk
...
fixFSTreeDelta root_td root_ts = fixFSTreeDelta' "" root_td root_ts
	...
	fixFSTreeSymNode' root rel (FSTreeSymLink tgt) = 
		let td = maybe Map.empty childrenFSTreeDeltaNode $ focusFSTreeDeltaByRelativePathMay root_td (root </> tgt)
		    ts = focusFSTreeSym (root </> tgt) root_ts
		in maybeFSTreeNop $ fixFSTreeDelta' (root </> rel) td ts
