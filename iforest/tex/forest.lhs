...
This wait-and-acquire step is performed atomically, without interruption from the Haskell scheduler, and can be implemented using GHC's lightweight concurrency substrate~\cite{HaskellLWC}.
...
Another difference is that locks must follow the hierarchical structure of paths, i.e., a lock on a path shall lock all its subpaths. For example, we should not be able to acquire a lock on a file while its parent directory is locked.

Furthermore, the concurrent handling of file paths becomes subtle in the presence of symbolic links --the identity of a path is not unique (as different paths may refer to the same physical address) nor stable (since the path resolution depends on the current symbolic link configuration)-- making it harder to identify conflicts between transactions. For example, one transaction may read a file whose path is concurrently modified by another transaction that changes a symbolic link.
...
The path locking scheme, however, does not need to take symbolic links into account.
...
When initialized, each transaction acquires the version number of the latest snapshot from the NILFS filesystem. Reads on the filesystem are instead performed against the initial snapshot; and groups of writes create advancing transaction-local snapshots over the initial physical snapshot.
In this variant,
Since the initial snapshot does not change throughout the lifetime of a transaction, we also do not need to know all the symbolic links in the system
On success, the transaction commits its modifications over the current filesystem (that may be at newer snapshot than the transaction's snapshot due to concurrent updates) and requests the creation of a new NILFS snapshot combining the concurrent and the transaction-local updates.
...
In all the previous iterations of TxForest, TxForest transactions are only serializable with respect to other Forest transactions, under the assumption that no other TxForest-oblivious processes are concurrently modifying the filesystem.
This is because our transactional commit phase is implemented in software and protected using in-memory TxForest locks.
