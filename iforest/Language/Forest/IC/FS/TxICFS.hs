...
instance ZippedICMemo TxICFS where
	...
	addZippedMemo path args (rep :: rep) mb_tree = debug ("added memo "++show path ++ show mb_tree) $ do
		...
		let load = do
			...
			Inc.get (to (iso_rep_thunk proxyTxICFS) t)
...
deleteTxICFS (rep :: rep) = do
	...
	case mb of
		...
		Just (args,path) -> do
			def_rep :: rep <- inside $ zdefaultScratchMemo proxyRep proxyTxICFS args path
			content <- inside $ BX.getM lens_content $ Inc.get $ to (iso_rep_thunk proxyTxICFS) def_rep
...
copyOrElseTxICFS proxy src tgt b f = do
	...
	case (mb_src,mb_tgt) of
		(Just (args_src,path_src),Just (args_tgt,path_tgt)) -> do
			...
			content' <- inside $ BX.getM lens_content $ Inc.get $ to (iso_rep_thunk proxyTxICFS) tgt'
