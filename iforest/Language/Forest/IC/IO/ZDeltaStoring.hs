...
doZDeltaManifestConstraint :: (Typeable rep,ForestContent fs rep,IncK (IncForest fs) (ForestFSThunkI fs Forest_err, rep),ForestMD fs rep,ICRep fs, toprep ~ ForestFSThunkI fs (ForestFSThunkI fs Forest_err,rep)) =>
	Bool -> (ForestContentTy fs rep -> ForestI fs Bool) -> FSTree fs -> toprep -> ValueDelta fs toprep
...
zskipManifestIf :: (Typeable rep,Typeable (ForestRepTy fs rep),IncK (IncForest fs) (ForestRepTy fs rep),ForestMD fs rep,ICRep fs,ForestRep fs rep,StableMD fs rep,DeltaClass d) =>
			...
			Writer.tell $ \latest_tree -> do
				...
				case mb_latest_df of
					Just latest_df -> do
						...
						return ()
					Nothing -> overwrite (to (iso_rep_thunk fs) rep) $ Inc.get =<< liftM (to (iso_rep_thunk fs)) (load latest_tree)
			return man
