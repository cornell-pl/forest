...
class ICRep fs => ForestContent (fs :: FS) var where
	...
	lens_content :: LensM (ForestI fs) var (ForestContentTy fs var)
...
class (Eq rep,ForestMD fs rep,IncK (IncForest fs) rep,Typeable (ForestIs fs (ForestArgs rep)),IncK (IncForest fs) Forest_err,ICRep fs,ZippedICMemo fs,ForestArgsClass fs (ForestArgs rep),MData NoCtx (ForestO fs) rep) => ZippedICForest fs rep  where
	...
	zdefaultScratchGeneric :: Proxy rep -> Proxy fs -> ForestIs fs (ForestArgs rep) -> FilePath -> ForestI fs rep

	zdefaultScratch :: Proxy rep -> Proxy fs -> ForestIs fs (ForestArgs rep) -> FilePath -> ForestI fs rep
	
	zdefaultScratchMemo :: FTK fs rep => Proxy rep -> Proxy fs -> ForestIs fs (ForestArgs rep) -> FilePath -> ForestI fs rep
	zdefaultScratchMemo proxy fs args path = do
		rep <- zdefaultScratch proxy fs args path
		addZippedMemo path args rep Nothing
		return rep

	zdefault :: FTK fs rep => ForestIs fs (ForestArgs rep) -> FilePath -> ForestI fs rep
	zdefault = zdefaultScratchMemo Proxy Proxy
