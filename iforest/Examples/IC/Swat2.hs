...
module Examples.IC.Swat2 where
...
basinFile :: SwatLines -> FilePath
--bsnFile l = "basins.bsn"
basinFile = getSwatStringVal 40
...
[txforest|
         ...
         data SwatDir = Directory 
	     { cio2 is "file.cio" :: FPreamble
         , pcps2 is [f :: FPCP | f <- matches (GL "*.pcp") ]
         , tmps2 is [f :: FTMP | f <- matches (GL "*.tmp") ]
         , fig is <| cioFile figFile cio2 |> :: FFIG
         , cst matches <| cioFileRE cstFile cio2 |> :: Maybe TextFile
         , wnd matches <| cioFileRE wndFile cio2 |> :: Maybe (FWND)
         , rh matches <| cioFileRE rhFile cio2 |> :: Maybe (FRH)
         , slr matches <| cioFileRE slrFile cio2 |> :: Maybe (FSLR)
         , bsn is <| cioFile basinFile cio2 |> :: FBSN
         , plant is <| cioFile plantFile cio2 |> :: FPLANT
         , till is <| cioFile tillFile cio2 |> :: FTILL
         , pest is <| cioFile pestFile cio2 |> :: FPEST
         , fert is <| cioFile fertFile cio2 |> :: FFERT
         , urban is <| cioFile urbanFile cio2 |> :: FURBAN
...
validSwat :: (FileInfo,SwatDir_innerEC TxVarFS) -> ReadOnlyFTM TxVarFS Bool
validSwat (swat_md, swat) = do
	(cio_md, cio_info) <- read $ cio2 swat
	(fig_md, fig_info) <- read $ fig swat
	pcp <- mapM (\(n,v) -> liftM (n,) $ readData v) $ pcps2 swat
	tmp <- mapM (\(n,v) -> liftM (n,) $ readData v) $ tmps2 swat
